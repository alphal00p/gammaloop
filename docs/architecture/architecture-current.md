...
The real LU generated-process end-to-end tests currently rely on a temporary
`GL_LU_E2E_HACK` evaluator-compilation bypass while the proper LU numerator /
theta / tree-denominator implementation is still being completed. The hack is
intentionally localized and documented in `differential_lu.md`, together with
