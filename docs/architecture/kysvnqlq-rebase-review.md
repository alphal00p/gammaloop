...
The original readiness checkpoint was recorded on 2026-09-01; the scoped
normalization, persistence, and export corrections below reflect the
2026-09-05 source review. Historical run counts and the recorded remote tip do
not constitute a current full-suite or release-matrix pass.

The original published feature-code parent was change
`lktlsypu`; the 2026-09-01 record placed the remote `raised_energy_cff_wip`
checkpoint at `5c3e7f`. That historical tip has not been reverified here; inspect
the current remote and local divergence before using it as a continuation base.
The local non-LTD continuation includes the
...
`test_gammaloop`; that remains historical evidence.

At the 2026-09-05 attempt07 checkpoint, the literal `just test_gammaloop`
passed all 1994 selected tests with zero selected skips, failures or retries
and normal RAM-guard completion. This included the six default scalar cases
and their direct-3D per-complete-key UV profiles. The subsequent dev-optim
166-case matrix completed 117 passing cases before the whole-system memory
cap stopped it; it is incomplete, with no recorded assertion failure. The
requested `just test_LU_scalar_xs` release target began compiling, but the
restart left no completed release-matrix verdict. A complete release 166-case
pass remains required. The four later tooling additions have separate command
and guard validation; the curated pass belongs to the unchanged core source.
This section records that checkpoint; later code or configuration changes need
their own validation. The following sections retain chronological evidence.
...
an EMR energy by signed integer `a*M`. Every evaluator receives the common
scale `M`, defaulting to one, and every process requires it to be nonzero;
results are invariant under changing its nonzero value.

In the live table below, the recorded 2026-08-31 `15/15` scalar-matrix result is
retained historical evidence. Coverage at that checkpoint is the recorded 1994-test
curated pass; the complete release scalar matrix remains a separate gate.
The four 10x physical rows below also retain their historical measurements.
...
| Raised-energy CFF core | **Repeated-channel, finite-pole EMR, factorized rank, source-backed dotted lifting, normalization, and lower-sector frame repairs implemented** | Physical edge-indexed bounds, source/local remapping, factorized analysis, and explicit `None` versus `Some([])` are retained. A55 routes the full occurrence graph through ordinary CFF or the restored channel-normal-form bounded builder. A56 restores independent finite-pole EMR sampling without rewriting diagnostic loop coordinates. A78 removes the bespoke contour certificate, retains the required physical-EMR rank envelope, and adds a source-integral hard gate from the existing all-cycle UV DOD with `DOD_E = DOD_4D - 3 L`. A53 excludes structural non-cut bridges from the CFF denominator domain and leaves their existing main-style 3D projection/mapping ownership intact. The corrected A79 path carries each original `EdgeIndex` through Taylor expansion so cograph/UV source minors can seed and validate the known skeleton, retain cut and physical-map provenance, and bookkeep repeated copies of one wrapper. The original graph and retained owners fix the UV/cograph skeleton; only derivative-created serial copies extend an owner's line. Denominator channels classify algebra on that already constructed scaffold, never infer its incidence or reassign an original numerator factor. Compatible owner-relabeling fixtures certify equal rational residues only after each source-backed lift is valid. Every projected discrepancy first requires exact source/reconstructed numerator and denominator certificates after the immutable assignment plan. One sign-safe minimax plan supplies exact bounds and factorized substitutions. A quotient-space solve chooses only a routing sign on the source-backed scaffold. Graphica canonically relabels this already-built topology for caching; no Kirchhoff/incidence reconstruction or balance synthesis remains. Two-pass batching normalizes each request's repeated-channel **totals** separately and reuses CFF only for equal topology and normalized capacity, while retaining every genuinely outer term's plan. The real DOD1 triangle retains its separate natural denominator topologies `(1,1,1)`, `(2,1,1)` and `(1,1,2)`, as exercised by `dod_one_triangle_keeps_separate_denominator_topologies`; it is not forced into a common-denominator source. A63's complete contour normalization remains valid, while its earlier same-owner/distinct-owner incidence distinction is superseded by this owner-invariant residue contract. A58 restores the validated full-rank top-level pure-CFF boundary while retaining contextual rank-deficient component/particular lifts only inside lower-sector construction. The historical shared-crate checkpoint was `98/100`; its private-selector/fixture failures are not the current test inventory. The retained 2026-08-31 campaign passed the four physical NLO acceptances; its scalar LU 15-case all-enabled rerun passed `15/15`, with `235` skipped, in `70.438 s` under `dev-optim` / `test_gammaloop`. The feature-gated diagnostic evaluator remains comparison evidence only; CLI-only failures are not a product gate unless they expose shared CFF mathematics used by GammaLoop. A64's numerator-call deduplication idea is retained as a deliberate post-rebase follow-up and is not a readiness blocker. |
| Production CFF carrier and prefactor bridges | **Implemented with focused and inclusive coverage** | Integrated finite UV terms retain their exact source-local EMR maps while each production selector identifies one complete generalized residue-map entry. Physical edge directions are metadata within that entry and never define the selector partition. The shared core carries component-local energy-factor ownership and source-frame metadata. GammaLoop applies the same bridge once for stored, reduced, and fresh exact sources: each component consumes its core or denominator factor according to its own ownership, while the parity `(-1)^(rho*N_c)` uses the whole expression's retained-positive-factor flag `rho`. Thus a mixed variant-local expression also supplies this parity to its ordinary components; the generalized core factor is not multiplied again. Recursive scalar bases separately convert their actual native rational prefactor into the enclosing source frame. The DDx and TTX three-route inclusive acceptances now exercise these bridges end to end. |
...
| UV profiling | **Six default per-key profiles passed in attempt07; complete release matrix pending** | `--selected-limits only-divergent` profiles every expected cycle union with DOD >= 0. It uses the generation LMB when suitable, otherwise the first suitable basis in the deterministically sorted complete LMB list; `all` remains opt-in. Graph and Cutkosky-cut selectors work for LU input, and failures are summarized in a colored table. Per-orientation profiling is valid only for orientation-parametric localized direct local3D; explicit-sum direct local3D and projected local4D are summed representations and reject it. The selected focused regression covers DOD0/1/2 bubbles, and every selected base scalar-matrix graph (GL00, GL02, GL04, GL08, GL09, and GL24) now invokes the localized-direct3D per-orientation profile. All six cases passed in the recorded 1994-test curated run with their existing profile assertions. The earlier isolated end-to-end CLI `2/2` and lower-level API/unit `15/15` counts remain historical inventories. |
| Direct-3D versus 4D-local-then-CFF parity | **Curated cases passed; complete release matrix pending** | Both direct forms apply every Taylor operator after complete/global CFF energy integration; their local bodies are identical and explicit-sum only omits selectors. A79's exact reconstruction and derivative-created minimax dispatch apply exclusively to projected local4D. A77 keeps the empty forest as the ordinary factorized production root in both routes. The four 10x physical DD/TT measurements and the 2026-08-31 scalar `15/15` run remain historical. The checkpoint source's selected cases passed in the 1994-test curated run; the larger dev-optim matrix stopped at the system-memory cap after 117 passes. Neither that partial run nor release compilation is a complete 166-case verdict. |
...
| Scalar snapshot audit | **Historical oracles retained; complete release matrix pending** | Historical proper-LTD-vs-CFF runs validate the live GL00 `Q7^2` magnitude `1.317240193281177e-9`; the negative-imaginary `.snap.new` remains stale because a fresh rebuild gives the positive phase. GL02 without a numerator is Arb-stable and equals the historical LTD value up to the current global `-i` convention at cancellation magnitude `4.2e-16`. The earlier GL24 investigation used a nonlocal global numerator and a handwritten contour calculation, so its mismatch claim was not authoritative. The historical replacement scalar LU 15-case matrix started from generated scalar graphs, retained unmodified unit-numerator lanes, and used companion Feynman-rule-local edge numerators. It compared all three routes at f64/native Arb with local UV, integrated UV, and threshold counterterms enabled. Its 2026-08-31 pre-reversal rerun passed `15/15`, with `235` skipped, in `70.438 s` under `dev-optim` / `test_gammaloop`; the current selected cases have since passed in the curated run, while the full release 166-case gate remains incomplete. Four historical near-zero cases used the authorized f64-input `1e-14` unit-scale fallback, while Arb-to-Arb still ran and reported non-scaling; this was test-oracle handling only and required no production change. No graph-specific production repair or graph-name branch is present. The protected GL11 expectations remain a separate test-intent issue. |
...
| Saved-state compatibility | **Top-level and integration-workspace boundaries implemented** | Top-level saved state writes manifest version 6 and rejects incompatible versions before decode or overwrite. Version 6 removes obsolete deferred-integrand fields from positional amplitude and cut-integrand layouts. It retains version 5's component-local generated-CFF energy-factor ownership and prefactor metadata, and version 4's typed core global-prefactor sign; version-3, version-4 and version-5 states must be regenerated. Integration workspaces independently write version 1 and reject missing, old, or future versions before either resume decoder, with contextual `integrate --restart` guidance and no positional-bincode migration. |
...
| Computed UV-forest export | **Exact stored-source ownership implemented (2026-09-05); final gates remain required** | Computed exports pair the selected persistent source graph with its complete stored production CFF and recorded generation settings. They reject missing settings/CFF data and a source/runtime graph-ID/name mismatch, then use exact maps in all three UV routes. Generation, save/load, and persistent selection/regeneration are covered; coarse direction metadata is never used to reconstruct an affine map. |
...
| Documentation, cleanup, and push | **Historical remote checkpoint retained; final local publication pending** | This ledger remains the authoritative chronological record and `raised-energy-cff-handoff.md` is its concise companion. The 2026-09-01 record placed the published `raised_energy_cff_wip` bookmark at `5c3e7f`; this review does not certify the current remote tip. Verify the current remote/local divergence before publication, retaining the recorded curated pass and requiring complete release-matrix validation plus the final artifact audit. `proper_ltd_core` remains a pushed sibling reserve. |
...
| `smlxpnry` | Computed UV-forest export | Originally deferred; exact-source ownership implemented in the 2026-09-05 continuation | Computed amplitude/cross-section exports now pair the selected persistent source graph with its stored complete production CFF and recorded generation settings. They reject missing source/settings and source/runtime graph mismatches, and use exact residue maps in all three UV routes. | The earlier coarse-direction export limitation is historical: persisted runtime directions cannot reconstruct affine maps. The continuation obtains those maps from the stored source expression instead. Generation, save/load, and persistent selection followed by regeneration are covered by the process-export regression; the final cleaned full suite and scalar matrix remain required. |
...
| A40 | Shared preprocessing retained; unused deferred representation removed during the optimized UV review | The earlier implementation stored finalized projected sums as compact calls plus serialized source-local bodies and supplied an alternate evaluator constructor. Review after ALL_GREEN found that only unit tests could seed that representation; production already used ordinary factorized integrands. Remove the sidecar, its private marker, duplicate consumers, and alternate constructor. | One `Integrands` map now owns each cut-indexed expression. `EvaluatorStack::preprocess_atom` retains the color, Dirac, metric, dot, tensor-network, aliasing, and contraction preprocessing used by ordinary evaluators. The closed bispinor trace regression remains on ordinary explicit sums, and a generic tagged-function regression preserves parameterless-body hyperdual coverage. Regenerate old serialized states under the repository's internal-state policy. |
...
| A81 | Maintainer-mandated projected-local4D reconstruction certificate | For every completed Taylor term, prove that its numerator in the original compatible hard sub-LMB and the proposed numerator in reconstructed UV-graph EMRs become the **same exact symbolic function** after substituting both into one neutral set of formal loop four-momenta and fixed external data. Separately prove equality of the denominator occurrence multiset, including momentum up to the explicitly even relation `D(Q)=D(-Q)`, mass, multiplicity, and topology domain. Original numerator factors retain their owners and are never dispatched. Only derivative-created numerator factors may be assigned among serial copies of their own physical line. Correctness precedes and is independent of minimax rank optimization. LMBs are coordinate charts for this proof only and never define EMR identity or capacity. Production numerators remain factorized; expansion is allowed only in the test/debug oracle. The certificate must consume the immutable production assignment plan and the full `H=hR`, `P=rR`, `H^0=hrP^0` occurrence conversion; a pre-plan comparison is invalid. | This is a mandatory stop rule for every projected-local4D discrepancy. A failed certificate localizes the defect before generalized CFF; a passing certificate moves the audit downstream to CFF normalization/component composition/residue aggregation. The GL04 `Q5_0^2` audit exonerates the no-derivative `8Ok/T0` sector. The live minimal discrepancy is `1zs/T2`: its exact post-T numerator, owner-5 multiplicity two, owner-6 multiplicity three, complete term-by-term occurrence plan, and hard/raw/parsed signs are written out explicitly. The executable oracle first keeps the five occurrence energies distinct and proves the production plan, then substitutes them into one common formal loop momentum and obtains exact zero numerator difference; the independent denominator certificate gives `D(q)^2 D(-q)^3`. Reconstruction therefore passes. Keeping each positive `GS.den` wrapper on one occurrence is a conservative convention but did not change the numerical A/B. A further exact audit proves that the five copies form one repeated rational channel and that every child generalized-CFF interpolation moment matches its analytic lower sector. The three observed projection waves are distinct LU cuts, the disabled-integrated first-level frozen factor is one, and the outer map leaves every provenance-wrapped child atom unchanged. Projected 60-record waves contain 40/48/48 exact zeros for cuts `[1,2]`/`[3,4]`/`[1,7,8]`, whereas the direct route prunes and coalesces a different keyed factorization; branch counts cannot be paired. The first unresolved boundary is the exact residue-summed projected/direct expression for one physical cut. Full invariant and worked algebra: [`exact-powered-denominator-cff-lifting.md`](exact-powered-denominator-cff-lifting.md). |
| A82 | Maintainer-clarified fixed-cut comparison contract | Compare direct local-3D and projected local-4D only after summing every generalized residue-map entry and assembling every raised-order derivative piece for one fixed physical Cutkosky cut. Individual map entries and `lu_cut_order` slots are decomposition coordinates and need not match: already for a double pole, `f2 -> f2 + eta h` together with `f1 -> f1 - h` leaves the assembled derivative functional invariant. Direct orientation-local per-key UV finiteness remains required, but it does not define a one-to-one comparison with projected local-4D. | Staged projected component maps must preserve factorization: each source-local map acts only on its owned numerator factor, mapped carrier/factor pairs flow to the next component, and the outer map consumes the remaining soft/cograph factors once. Cartesian products are formed only for genuinely independent unsummed component states; repeated production hosts of an already residue-summed child are mapping metadata, not additional factors. Tests and documentation must not promote fixture-specific per-order agreement into a route invariant. |
