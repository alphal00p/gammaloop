...
Last updated 2026-08-24 from local change `lktlsypu`. This section is the live
status summary; the later sections retain the chronological decision and
validation evidence.
...
| Raised-energy CFF core | **Repeated-channel, finite-pole EMR, D/N certificate, bridge, active-polynomial, topology, normalization, and top-level rank repairs implemented** | Physical edge-indexed bounds, source/local remapping, factorized analysis, and explicit `None` versus `Some([])` are retained. A55 routes the full occurrence graph through ordinary CFF or the restored channel-normal-form bounded builder. A56 restores independent finite-pole EMR sampling without rewriting diagnostic loop coordinates. A62 derives the contour certificate from active CFF denominator occurrences while keeping physical numerator EMR rows separate. A53 excludes structural non-cut bridges from that reduced domain and leaves their existing main-style 3D projection/mapping ownership intact. A74 rejects non-polynomial dependence in CFF-owned EMR variables while leaving cut/bridge factors caller-owned and factorized. A57 serializes same-owner exact powers while preserving occurrence IDs and distinct-owner incidence. A63 validates their contour normalization only after the complete explicit orientation sum. A58 restores the validated full-rank top-level pure-CFF boundary while retaining contextual rank-deficient component/particular lifts only inside lower-sector construction. GammaLoop-facing compilation and focused shared-core/GammaLoop tests pass. The feature-gated diagnostic evaluator inventory remains 53/59; it is evidence for comparison, not a product gate, and only regressions relative to the selected pre-rebase baseline or failures reaching GammaLoop affect readiness. A64's numerator-call deduplication idea is retained as a deliberate post-rebase follow-up and is not a readiness blocker. |
| `epem_a_ddx` NLO generation | **Exact reported command passes at the current tip** | After A49/A53/A62/A74, `./gammaloop --release --clean-state ./examples/cli/epem_a_ddx/NLO/epem_a_ddx_NLO.toml run generate_diagrams generate_integrands -c "quit -n"` completes, generating and compiling GL0 and GL2 with five evaluators each. This confirms the direct-3D production path no longer rejects the graph's raised physical-EMR numerator bounds, that strict active-variable analysis accepts its factorized numerator, and that its external-flow bridges remain outside CFF. The card has `generate_integrated = false`, so it still does not exercise the term-local integrated-capacity boundary. |
| Direct-3D versus 4D-local-then-CFF parity | **Pre-rebase regression isolated; restoration decision pending** | The focused raised GL00 `Q7^2` projected route currently stops before CFF generation at A52. The pre-rebase GammaLoop route did pass this comparison. Its physical bound `(7,2)` maps unambiguously to the current exact source's unique literal `Q7` occurrence `(15,2)`, and `ExactSourceEnergyMapper` already uses occurrence 15 to evaluate the factorized numerator. What is missing is the capacity-bound translation. A61 distinguishes restoring that validated literal case from inventing an owner-based or general affine mapping. The earlier zero-versus-nonzero trace separately documents A45's distinct-physical/coincident-denominator provenance problem, which will reappear once raised exact projection is representable. |
| Direct-3D historical parity | **Historical cause isolated; post-A55 run pending** | The authoritative pre-rebase GL00 `Q7^2` value agrees in the shared-core route and the integrated GammaLoop route, including the latter on Symbolica 2.2. The first relevant source delta is `58a1634e`: it replaces physical-EMR bounds and bounded finite-pole/contact completion with LMB sparse support/interpolation. This is the same rejected boundary repaired by A50/A55, while numerator mapping, threshold counterterms, the outer normalization, and Symbolica are ruled out. A fresh direct-only run is still required to prove that the current implementation restores the old value; the stale bad snapshot must not be refreshed or accepted meanwhile. |
| Scalar snapshot audit | **Open; no blanket updates** | Of the 198 live stored snapshots, 195 are byte-identical to the pre-rebase source. GL02 `Q1^4` is the maintainer-approved exact-zero change. GL02 without a numerator has an unapproved phase-rotated cancellation residue of magnitude about `4.2e-16`; GL00 `Q7^2` is the material unapproved change above. Current GL16 and GL02 route comparisons also expose cancellation-level raw-value differences. A global numerical clamp would affect many unrelated cases and is rejected. |
...
| Saved-state compatibility | **Top-level state resolved; integration-resume state still open** | Top-level saved state now writes manifest version 2 and rejects versions 0/1 before decode or overwrite, resolving the projected-body/explicit-sum positional-bincode boundary without a migration layer. Separately, `StatisticsCounter::numerical_stability` changes the bincode layout of `IntegrationState`, while integration workspaces persist an unversioned `manifest.json` plus `state.bin`; one resume path still expects decode success. A versioned workspace boundary and contextual rejection are pending audit/approval. |
| GammaLoop QoL and committed interfaces | **Audit complete; restoration decisions pending** | GammaLoop's live CLI behavior is ahead of the committed JSON schemas; one Rust example still pins the obsolete Symbolica repository; read-only guards/default-output fallbacks were dropped or not adapted to newer main; and the useful benchmark suite was not ported to the approved top-level GammaLoop `bench` command. The auxiliary `3Drep graph-from-signatures` command remains tracked because it existed and was validated before the rebase. A67/A69 record the mechanical repairs and the test-affecting placement/API choices. |
...
| Documentation, cleanup, and push | **In progress** | This ledger is in `lktlsypu` and is being updated with each decision. The remote `raised_energy_cff_wip` remains behind the current local WIP; the current tip is not pushed. `proper_ltd_core` is fully pushed as a sibling reserve. Obsolete source revisions and the detached old ledger revision remain until final cleanup. |
...
- GammaLoop is the product boundary and supplies the terminology used for
  ...
  GammaLoop calls it.
- The separate `3Drep` CLI and its feature-gated diagnostic evaluator affect
  rebase readiness only when behavior that worked before the rebase regresses.
  Pre-existing defects and new CLI-only enhancements are recorded as follow-up
  material, not GammaLoop completion blockers.
...
| `kqqlnrnx` | Main-owned model/state fixtures | Mechanical under A3/A13, resolved after final rebase | Preserve main's scalar-model parameter order, retain required `goldstone` schema fields, and regenerate `state_map.bin` against the final approved Symbolica/model tree. | The scalar JSON now differs from main only by the schema fields. Two exports produced the same 22,677-byte SHA-256 `651438715477c347f86e8900ef8af83cca336407337635f34fd7b6902feb7e94`. The pre-existing generator test then fails after export because it imports from its write-only `File::create` handle (`EBADF`); no test was changed. |
...
| `kqqlnrnx` | Exact-map UV localization carrier | Non-mechanical, maintainer-approved | Retain one UV forest traversal with its local-3D payload keyed by the full production expression's `OrientationID`. Project each contracted CFF map onto full-map IDs by exact affine equality on surviving edges, carry every compatible extension separately with main's existing localization weight, and collapse to ordinary integrands only after the complement/global numerator is attached under that map. Error when a raised reduced map has no exact full-map extension; never fall back to coarse orientation matching. | No existing carrier or reduced/full exact-map projection API exists. Reusing `OrientationExpression`, `LinearEnergyExpr`, `CFFTerm`, and `Local3DCts` avoids a second numerator or affine-map abstraction. Running the entire forest once per map would avoid a carrier but repeat map-independent Vakint/integrated work and scale poorly. `numerator_map_index` is expression-local diagnostic metadata, so the full expression's typed orientation ID is the stable identity during one orchestration. |
| `smlxpnry` | Exact-map implementation review | Non-mechanical under A17/A22, implemented and validated | Keep full production maps through `Graph::cff`, use map-keyed integrands with equal-ID zip operations only, defer full orientation-pattern filtering until exact projection, map each newly owned numerator fragment and integrated finite CT exactly once, leave root selector extensions unnormalized, and apply `1/N` when one collapsed local or integrated exact-4D residue is projected onto `N` compatible production extensions. Disable the final coarse sign fallback in exact mode. Restore an omitted contracted-parent map only from exact zero-inner source coordinates. | Independent review found and corrected root `1/N` scaling, premature reduced-pattern filtering/no-match conflation, unmapped finite CTs, and zero-filled reconstructible parent edges. The approved finite-CT fixture now contains a parallel loop plus a real external-flow bridge, asserts that the bridge is contracted, and verifies both exact localization maps; its focused test passes. |
...
| A17 | Decided | Add one private exact-map projection operation and retain map-indexed local-3D terms through a single UV forest traversal. | `Graph::numerator` remains the only physical-factor ownership mechanism. The carrier only prevents `Graph::cff`, localization, active-sector joins, and final assembly from losing or cross-multiplying existing `OrientationExpression` maps. Reduced maps match full maps by complete affine equality on uncontracted edges; root extensions retain their selector partition without normalization, while a single collapsed local or integrated exact-4D residue is averaged over selected compatible extensions by `1/N`. A genuinely missing extension is an error. This is preferable to recomputing the whole forest once per map. |
...
| A20 | Superseded by A52/A62 | Keep the nullity-aware certificate `N-2|A| < -d`, checked arithmetic, and typed arrangement-size errors, and fail conservatively when the energy-contour condition is not proved. A post-generation spatial UV profile is corroborating evidence, not a replacement for the energy-contour condition. | The maintainer approved conservative failure after reviewing the theory condition. A62 repairs the inputs to that hard gate by separating active denominator occurrences from cut-alias, bridge-owned, and physical numerator EMR rows. |
...
| A35 | Maintainer correction; ordinary-root proposal rejected | Both UV paths build the complete typed 4D forest because integrated counterterms require it. The direct-3D path then recomputes local subtraction on CFF expressions, taking limits with orientation localization. The projected path instead maps the completed 4D local counterterms **and root** to 3D and generates CFF only after every 4D series/limit operation. Do not replace the projected root with ordinary production CFF. | An ordinary-root special case would make the second path a hybrid and would no longer test the intended operation order. The distinction between the paths is where local CFF generation and limit-taking occur, not whether the 4D root/forest exists. |
...
| A45 | Corrected by A57/A63 after broader audit | Parser-expanded denominator powers are canonical serial chains. Exact energy identity remains occurrence-local, with an explicit many-to-one projection to physical source/cut ownership. | The earlier source-wide “exact occurrences are parallel” rule was too coarse. Multiple exact occurrences owned by one physical propagator also represent one power and need canonical serial incidence in the temporary CFF graph, while retaining distinct occurrence IDs. Distinct physical owners with coincident algebraic denominators must retain their own incidence and provenance. A global serial/parallel flag cannot encode both cases, and the new exact coefficient tests do not provide a validated normalization oracle. |
| A46 | Boundary retained; exact-parallel assertion superseded by A57 | Test only the live causal generator boundary, preserve topology meaning independent of list rotation, treat isolated zero-edge components as multiplicative identity, and make parser-powered repeated fixtures actual serial chains. | The serial assertion compares undirected endpoint sets and CFF recursion filters isolated vertices instead of manufacturing a zero-valued causal surface. The later exact-source “parallel occurrence endpoints remain unchanged” assertion encoded the insufficient source-wide topology flag and is reopened by A57. |
...
| A57 | Maintainer-approved and implemented; focused topology validation complete | Keep the validated pre-rebase shared CFF core's topology contract unchanged: local occurrence IDs plus explicit serial incidence, with no physical-owner field added to `ParsedGraph`. In GammaLoop's exact-4D source adapter, use the existing deterministic occurrence order and occurrence-to-physical map to subdivide repeated occurrences of one physical owner into a canonical serial chain. The safe chain key is physical `source_edge` plus UV/cograph coordinate domain plus canonical momentum signature and mass; opposite-sign members reverse their chain endpoints while retaining their signed EMR map. Distinct physical owners keep their original incidence even when their denominators are algebraically equal. Leave owner-blind signature/mass channel grouping unchanged. Remove the now-unused source-wide `RepeatedDenominatorTopology` enum/trait method. | The validated pre-rebase core (`b9c4e2ff`/`cc8c5979`) had no owner metadata: parser powers were already explicit serial chains, deletion contracted the removed segment, and `sub_to_orig` retained occurrence identity. The exact adapter now adds `n-1` fresh nodes per same-owner group, preserves deterministic occurrence/physical maps, separates UV/cograph and different-signature/mass groups, and balances opposite routing by reversing its segment. The dead global switch is removed and every lower-sector deletion again contracts its explicit edge incidence. Five focused exact-source tests pass serially, including independent same-domain/different-mass coverage; shared-crate nextest reports the same four already-catalogued A43/A59 expectation failures and 49 passes. A63's two exact-CFF tests now also validate the resulting serial-versus-parallel occurrence orientations and their complete contour-normalized sum. |
...
| A60 | Definite persistence boundary outside top-level state; decision pending | Version integration-workspace resume data independently of `state_manifest.toml`. Reject old, missing, or future workspace schema versions before decoding `state.bin`, return contextual errors instead of panicking, and do not add a positional-bincode migration unless explicitly requested. | The QoL line adds `StatisticsCounter::numerical_stability` inside serialized `IntegrationState`. Top-level state version 2 does not cover integration workspaces, which persist their own `manifest.json` and `state.bin`; at least one resume path still calls decode through `expect`. Old pre-rebase workspaces can therefore fail opaquely or panic. Exact call sites and the original QoL history are part of the continuing non-CFF delta audit; no persistence code has yet been changed. |
| A61 | Pre-rebase GammaLoop regression isolated; exact mapping policy decision pending | Restore the validated physical-EMR-to-exact-occurrence capacity translation without reverting to LMB bounds or copying ownership metadata as energy identity. The narrow proven rule is: map a physical bound to one unique exact occurrence only when that occurrence's momentum is literally the same `Q(edge)`; otherwise retain A52's hard error. A possible broader rule would select one deterministic representative when all same-physical literal copies belong to one logical repeated channel, because the bounded builder co-samples that channel and the degree must be counted once, not once per copy. That repeated-copy extension lacks the GL00 oracle and remains a separate approval/proof point. | Pre-rebase `expanded_4d.rs` from `b8bec5344` through integrated `cc8c5979` analyzed the factorized term numerator in physical EMR IDs, then copied each degree to every denominator occurrence with matching `source_edge`. Its GL00 `Q7^2` direct-versus-4D-CFF test passed at `-2.22459688841192310e-11 + 4.48261675062501648e-28 i`. In the current nine-edge GL00 source, literal occurrences are `Q1:9` through `Q8:16`, so `(7,2) -> (15,2)` is unique; the runtime mapper already evaluates Q7 from occurrence 15. The old general owner-based copying was not sound for repeated owners, shifted/UV denominators, or affine reconstruction and must not be restored wholesale. A full numerator-row API is still needed for bounded physical energies with no literal occurrence. No production code, battery assertion, or snapshot has changed under A61 yet. |
| A62 | Maintainer-approved active-rank and certificate-domain repair implemented | Separate the actual denominator arrangement from numerator-owned EMR capacity. Let `D` contain only active CFF denominator occurrences, excluding initial-state cuts and caller-owned bridges while retaining occurrence multiplicity; derive active rank/nullity from `D`. Let `N` contain physical EMR/source rows with degree bounds after cut-alias, bridge separation, and shrunken-edge semantics. For every denominator flat `F`, use `s = rank(D) - rank(F)` and certify `sum(active N bounds) + s < 2 * count(active D occurrences)`. | `energy_divergence_report` now receives the denominator occurrence multiset separately from edge-indexed numerator bounds. Denominator flats, rank, and suppression count use only `D`; active numerator degree is computed independently from bounded physical rows in `N`. Initial-cut rows are zeroed before certification, caller-owned or A49-preserved bridges are absent from the active parsed `D/N`, repeated denominator multiplicity remains in `D`, and a bounded nonzero numerator row outside `span(D)` returns a typed failure. This certificate checks scaling subspaces inside `span(D)`; it does not authorize a public source whose declared LMB has a direction outside that span. A58 now supplies that separate top-level full-rank prerequisite. Focused multiplicity/zero-row and outside-span regressions pass, as do formatting, checks, and warnings-denied core Clippy. |
| A63 | Maintainer-approved and implemented; focused normalization validation complete | After A57 restores owner-aware incidence, retain the ordinary pure-CFF duplicate-excess sign for algebraically repeated denominators and validate the **explicit orientation sum**, not each raw orientation against the total. Distinct physical ownership affects topology/provenance/numerator maps, not the identity `D(+Q)=D(-Q)`. | For either two occurrences of one owner or distinct `+Q/-Q` owners with equal mass, direct contour integration gives `integral dq0/(2 pi) D(q)^-2 = +i/(4 omega^3)`; including GammaLoop's spatial `(2 pi)^-3` factor gives `+i/(32 pi^3 omega^3)`. Ordinary pure CFF realizes this as two orientations of `+i/(64 pi^3 omega^3)`. The corrected tests require exactly those two orientations, distinguish same-owner serial incidence from distinct-owner parallel incidence, and keep the original physical-ID slots undirected. Their complete sums equal `+i/(32 pi^3 omega^3)` after replacing the intentionally deferred physical `OSE` by its exact square root. Both tests pass serially; production sign and normalization code are unchanged. |
...
| A65 | Uniform-sampling policy and runtime validation pending | Reject `M = 0` whenever a generated expression actually uses uniform sampling. Separately decide whether `NumeratorSamplingScaleMode::All` should follow the note exactly and activate only for repeated-channel degree `d_C > 1`, or keep the harmless but swell-inducing current `degree > 0` behavior. | The auxiliary feature-gated evaluator already rejects zero when the expression contains `M`, but GammaLoop's runtime `general.numerator_sampling_scale` is an unconstrained `f64` and its parameter setter accepts zero. The missing GammaLoop validation is a correctness defect. Activation at degree one predates the rebase and is algebraically unnecessary rather than numerically wrong for nonzero `M`; a current test explicitly blesses it, so policy alignment requires maintainer approval before changing the test. |
...
| A67 | QoL completeness audit; mixed mechanical and test-affecting decisions pending | Mechanically regenerate all committed schemas, update live Rust example pins to main's exact Symbolica revision, restore read-only guards for `renormalize --result-path` and main's newer `approach --output-results`, and remove only stale `#[allow(dead_code)]` attributes from live Python functions. Separately decide the read-only default-output and benchmark-suite adaptations before changing their tests. | `global.json`, `runtime.json`, and especially `runhistory.json` reject or misdescribe live settings and commands because they were not regenerated. The pre-rebase read-only fallbacks for default `save dot`/`save standalone` outputs were dropped even though their helper remains suppressed as dead code; main's newer `save uv-forest` has the same policy question. The 23-file validated benchmark suite should not be restored wholesale: retain live CFF/minimal/optimized-CFF profiling and resource/uncertainty tooling, adapt `inspect --bench` to the top-level GammaLoop `bench` command, require explicit orientation sum for 4D-local-CFF, and leave LTD/removed `3Drep evaluate` profiles deferred. BNL docs and one late-parsing CLI test contain obsolete benchmark syntax. Pure-tree zero-coordinate bench support is a separate API choice. No schema, source, documentation, or test has yet been changed under A67. |
| A68 | Auxiliary diagnostic mismatch recorded; nonblocking absent a pre-rebase regression | Possible follow-up: make `3Drep` detailed output display the same GammaLoop production numerator that its bound analysis and generator consume. | The command currently prints `Graph::full_numerator_atom()`, which includes initial-cut-owned material and the raw overall factor, while GammaLoop production uses `production_numerator_atom_for_full_3d_expression()`, which excludes the initial cut and evaluates the global factor. Generation mathematics and spinney ownership are unaffected, and no regression relative to the selected pre-rebase baseline has been established. No code or output expectation has been changed. |
| A69 | Validated pre-rebase auxiliary-CLI regression; tracked below GammaLoop product work | Restore `3Drep graph-from-signatures` as tooling independent of LTD, reusing the pre-rebase Symbolica `prop(q_,m_)` extraction and connected GammaLoop-DOT reconstruction rather than reimplementing it. Honor the command's `minimize_externals` value instead of the old hard-coded `true`. | The 1,200-line core module, CLI owner, five unit tests, and round-trip closure tests from one through five loops disappeared with no replacement. It is not required for GammaLoop CFF generation, but it is a demonstrated loss of validated pre-rebase QoL and must not be labelled dead or silently left to the future LTD change. Restoring it changes the auxiliary CLI surface and adds tests, so exact placement remains an approval point after GammaLoop product work. |
| A70 | Shared-package maintenance plus auxiliary regression gate | Restore a crate-local macOS `EXTRA_MACOS_LIBS_FOR_GNU_GCC` build hook using main's current standardized linker flags if the shared-package or GammaLoop build needs it. Correct the README to call `eval` a manual feature-gated oracle and document its serial command. Treat evaluator CI as a pre-rebase regression gate, not a GammaLoop product gate. | Cargo build scripts are package-scoped: the shared core directly links Symbolica and cannot inherit the root, `gammalooprs`, or API hook when tested alone. The repository still documents this opt-in macOS workaround. Separately, neither workspace nextest nor dependency feature unification currently enables the core's `eval` feature, so the README's integration-test claim is false. The linker hook and documentation are mechanical; evaluator CI is only required after classifying the catalogued failures against the pre-rebase baseline. |
...
| A72 | Stale public feature flag; cleanup decision pending | Decide whether to remove the no-op default Cargo feature `serde` or retain it as compatibility vocabulary. | Serde dependencies and derives are unconditional and no `cfg(feature = "serde")` exists, so `serde=[]` changes neither build nor API behavior. Removing a named public feature can still break downstream manifests; it is recorded rather than silently deleted under the no-dead-code policy. |
| A73 | Direct GL00 historical cause isolated; live post-A55 oracle pending | Treat the old direct-3D GL00 `Q7^2` value as the acceptance oracle and rerun it before changing any snapshot. Do not attribute the regression to Symbolica, numerator tensor mapping, thresholds, graph normalization, or distinct-owner grouping without new evidence. | Both `cc8c5979` (pre-rebase shared-core route) and `f46f9f56` (integrated GammaLoop route on Symbolica 2.2) give `re=-2.22459688841192310e-11`, `im=4.48261675062501648e-28`. The bad `114c5e158` trace is entirely in direct `Original`; its threshold term is zero, UV is off, its outer factor is unchanged, and Q7 maps as a factorized affine energy with no tensor concretization. No-numerator, Q1-squared, and quadratic-pair controls are unchanged. The first relevant mathematical delta is `58a1634e`, which converts EMR bounds to LMB monomial support and replaces bounded finite-pole/contact completion with residue-coordinate sparse interpolation in the repeated group containing edges `[2,3,6]`. Its old and bad snapshot revisions share the same source parent, so that snapshot transition itself proves nothing. A50/A55 reverse the implicated architecture, but a current release result has not yet been obtained; no snapshot or expected value has been changed. |
| A74 | Maintainer-approved active-variable analyticity repair implemented and focused-validation complete | CFF capacity analysis needs two distinct facts: a nonnegative polynomial degree bound for UV/contact reconstruction, and validation that every energy-dependent factor assigned to CFF has no additional numerator poles. Negative powers of cut/bridge energies may remain in their separately projected external sector, but a negative power of an active CFF EMR affine form must fail rather than be assigned degree zero. | The CFF-facing atom analysis now uses strict `EnergyPowerAnalyzer::with_internal_edges(active_edges)`. In one factorized traversal it rejects negative active powers, energy-dependent exponents, and opaque nonlinear active-energy functions, then returns the polynomial degree caps; it does not expand the numerator. Excluded cut/bridge `Q` factors and `K` factors whose explicit carrier edge is excluded remain caller-owned, factorized, and mapped later. The unchanged `typed_4d_projection_analyzes_each_term_owned_numerator` regression, the active-negative analyzer test, and a new excluded-Q/K-carrier regression pass, as do formatting, `cargo check -p gammalooprs --lib`, and `git diff --check`. Warnings-denied `gammalooprs` Clippy is currently blocked only by the separately existing `too_many_arguments` warnings in `local_3d::{t_tilde,start}`. No existing test changed. |
| A75 | Maintainer-approved product and regression boundary | Use GammaLoop terminology and judge completion by GammaLoop generation, projection, evaluation, persistence, and user-facing commands. Audit the shared CFF library wherever GammaLoop depends on it. Treat the separate `3Drep` CLI and diagnostic evaluator only as regression oracles against the selected pre-rebase baseline; do not make pre-existing auxiliary-CLI defects or prospective CLI-only improvements block this rebase. | This narrows the readiness interpretation without discarding source material. Demonstrated lost pre-rebase auxiliary-CLI behavior such as A69 remains a rebase regression and QoL restoration candidate; A64's pre-existing diagnostic-loop identity mismatch and other unproven CLI-only improvements remain documented follow-ups. Shared-library mathematical defects remain in scope when they can affect GammaLoop, regardless of where focused tests live. |
| A76 | Latest-main evaluator/archive replay resolved, validated, and squashed | Preserve main #95's unconditional evaluator construction, removal of `override_if`/override input, `0441bd7a` Symbolica stack with native-code support, and O2 compilation behavior. Layer the approved deferred explicit CFF orientation sum through evaluator-local tagged function maps. Explicit-sum stacks evaluate once from representative input, reject every individual-orientation request, and do not resurrect main-deleted conditional tests. | The only semantic source conflict in `kqqlnrnx` was resolved by retaining main's ordinary orientation guard and adding the explicit-sum dispatch. The top replay retained only the two new deferred-CFF evaluator tests, updated `collect_orientation_if` to main's one-argument API, removed all stale override references, and combined archive layouts as amplitude version 6 / cross-section version 9. Symbolica references use main's `0441bd7a`; O2 forwarding/defaults remain. Formatting, locked GammaLoop library compilation, both deferred-sum tests run serially, the explicit-sum runtime/LTD boundary, main's ordinary orientation-Monte-Carlo guard, offline metadata, Hakari verification, and an independent generated-amplitude-loader compile pass. GammaLoop's amplitude `save standalone --json` and cross-section archive/loader export tests also pass. Those two export tests do not perform typed archive reload/evaluation, so that remains an explicit coverage gap rather than inferred validation. The validated resolution was squashed into `lktlsypu`; no conflict remains in the selected ancestry. |
...
| `ltnoyvll` | Add LU scalar cross-section regression coverage. | Its two CFF routes and 198 live snapshot cases are ported in `lktlsypu`; semantic sign-off is blocked by GL00 and the proper-LTD third leg remains preserved and deferred. |
...
- The pre-battery raised-CFF scope audit found no scope blocker at that
  ...
  GL00 findings. `expanded_4d.rs` is replaced by
...
- `state_map.bin` was exported twice against the final tree with identical
  22,677-byte SHA-256
  `651438715477c347f86e8900ef8af83cca336407337635f34fd7b6902feb7e94`.
...
- Historical exact-normalization checkpoint, superseded by A55/A63: the
  ...
  broader audit now requires an owner-aware topology plus a derived explicit-sum
  normalization before either production code or expectations change. The
...
- Scalar-battery audit and implementation: source `b8bec534` added
  ...
  cancellation-scale phase change, and GL00 `Q7^2` has an unapproved material
  change.
- Scalar-battery implementation compiles cleanly against the rebased APIs. The
  ...
  sums those source-local maps explicitly after the complete 4D root/forest,
  rather than weakening exact matching or substituting an ordinary root.
...
- A38 projected-route validation: all six `typed_4d_projection` tests pass,
  ...
  contain the later GL00 distinct-physical/coincident-denominator group and
...
- A45 pure-case topology validation: parser-expanded serial powers remove
  ...
  assertion, and disconnected recursion receives the topology setting. GL00
  demonstrates that these modes are not exhaustive: distinct physical source
  edges with coincident algebraic denominators need a third provenance category
  that preserves every physical incidence while applying algebraic
  repeated-pole completion. No serial contraction is valid for that case.
...
- A51 architecture and test finding: pure CFF keeps repeated denominator powers
  ...
  residue engine.
- A51 UV-policy discrepancy: the reference note makes the loop-energy UV check
  ...
  radial scaling in an `r`-dimensional subspace: require `delta < -r`. Keep that
  correction regardless of hard-versus-advisory policy.
- A51 UV implementation audit corrects A20's wording: live generation does not
...
- A51 post-generation-profile audit: GammaLoop's existing `UVProfileable::profile`
  ...
  CFF generation has already discarded. A successful post-generation profile
  must not relabel a failed energy-contour certificate as proved. The two
  results should remain explicit and separate: an energy-contour
  certified/uncertified warning from the source numerator envelope, and an
  emitted-3D UV profile result. Automatically running the latter is also much
...
- A52 acceptance-route audit: the maintainer's exact
  `epem_a_ddx_NLO.toml` command selects `final_integrand = ThreeD` and
  `local_uv_cts_from_expanded_4d_integrands = false`. It constructs full 4D
  local counterterms, but local subtraction uses the direct 3D CFF path; it does
  not call `project_4d`, `cff_from_4d_denominators`, or the provisional exact
  source guard. The command does exercise the restored EMR-edge bounds, strict
...
- A52 direct reduced-CFF capacity audit: expression ownership remains
  ...
  once. However, a nonzero integrated counterterm currently generates its
  ...
  together with spinney-owned and outside/global factors. The current capacity
  can therefore over-reject a bound on a contracted edge or underbound new
  outer-energy dependence introduced by the integrated counterterm. The exact
  acceptance card has `generate_integrated = false` and returns before this
  path, so it cannot validate or invalidate this pending non-mechanical fix.
  A53 later resolves the graph-theoretic bridge subset by retaining main's
...
- Current projected-route boundary: the GL00 `Q7^2` comparison now stops at the
  approved A52 exact-source guard before CFF generation because its nonconstant
  physical-EMR numerator capacity has no numerator-only occurrence map. The
  earlier zero-versus-nonzero trace remains evidence for distinct-owner
  provenance once that map exists, but it is not the current failure and cannot
  establish route parity. A61 records the honest battery choices.
- A61 pre-rebase comparison proves that GL00's present stop is a GammaLoop
  ...
  owner. That was sufficient for GL00, whose `Q7` has the single literal exact
  occurrence 15, but it was not a general affine contract and overcounted
  repeated owner copies. The current runtime mapper already evaluates the
  factorized Q7 numerator from occurrence 15. Restoring only the unambiguous
...
- Independent direct-route oracle: GL00 `Q7^2` has the same pre-rebase value in
  the pre-rebase shared-core route and the integrated GammaLoop route already on
  Symbolica 2.2. The bad trace is wholly in direct `Original`; thresholds are
  zero, UV is off, the outer normalization and numerator affine map are
  unchanged, and the control numerators are stable. The first relevant source
  delta is `58a1634e`'s EMR-bound-to-LMB-support conversion and replacement of
  bounded finite-pole/contact completion in the repeated channel. A50/A55 undo
  that architecture, but a live current result is still required. The bad
  snapshot was refreshed without a source delta and is not an oracle. A73
  records the evidence and the required post-A55 check.
...
- Current DAG/push checkpoint: the ledger is included in `lktlsypu`. The
  fourteen retained changes have been replayed directly onto local/remote
  `main` at `39561014`; the obsolete selected dependency-pin root was abandoned.
  The evaluator/archive conflicts with main #95 are resolved, validated, and
  squashed into `lktlsypu`. The remote `raised_energy_cff_wip` remains behind;
  no push of the current tip is claimed.
  `proper_ltd_core` remains a fully pushed sibling source reserve, not a
  descendant. The unrelated conflicted bookmark `push-wwvzyzvysvro` remains
...
- A62's certificate rank and A58's source-rank boundary are now repaired as two
  ...
  result.
- A57/A63 replace the earlier binary topology model. The validated pre-rebase
  shared CFF core needs no new owner field: GammaLoop's exact-source adapter already owns the
  occurrence and physical-owner ID maps and must encode same-owner powers as
  canonical serial incidence before entering that core. Coincident denominators
  from distinct physical owners retain their original incidence. Algebraically,
  however, `D(+Q)=D(-Q)`, so the existing owner-blind channel grouping and ordinary
  duplicate-excess sign remain required. The contour-correct explicit sum is
  two `+i/(64 pi^3 omega^3)` orientations, or `+i/(32 pi^3 omega^3)` total; the
  corrected tests now assert the complete positive sum, distinguish same-owner
  serial incidence from distinct-owner parallel incidence, and pass serially.
...
- A66 records the missing validation evidence rather than claiming deleted Rust
  ...
  this audit.
- A73 narrows the direct GL00 `Q7^2` regression to the same generalized
  repeated-channel sector changed by `58a1634e`. The old result is stable across
  pre-rebase shared-core/integrated GammaLoop routes and Symbolica generations, while the
  bad snapshot refresh contains no source change. A50/A55 repair the implicated
  EMR-bound and finite-pole organization, but only a fresh direct evaluation can
  establish restoration; neither old nor bad snapshots may be rewritten to
...
- The GammaLoop ownership review found no second numerator-growth regression.
  ...
  × global factor` once; final assembly does not regrow it. The dual exact
  occurrence and physical-owner ID maps now retain A57's canonical incidence
  layer, and A63's explicit-sum normalization oracle passes for both same-owner
  powers and distinct `+Q/-Q` owners.
- A60/A67/A68 cover non-CFF integration mistakes: unversioned integration
  workspace bincode after the stability field addition; stale committed
  ...
  schema regeneration, exact dependency pin, guards, and diagnostic alignment
  are mechanical. Default-output, benchmark API, and every affected test remain
  approval-gated.
