...
Last updated 2026-08-26. The published feature-code parent was change
`lktlsypu`; the current local non-LTD continuation adds the
...
prefactor bridge. The standalone NLO normalization acceptance remains open.
...
| `epem_a_ddx` NLO normalization | **Open; no passing inclusive result recorded** | The standalone acceptance must sum GL0+GL2, orientations, and LMB channels and reproduce `|(alpha_s/pi)*LO|`; phase and overall sign are deliberately excluded for now. Generation and focused bridge tests are not a numerical pass; the empty `wszrkkok` change was abandoned rather than presented as completed acceptance. |
| Direct-3D versus 4D-local-then-CFF parity | **Exact EMR capacity mapping restored; empty-root identity corrected** | A61 maps a physical numerator bound to one unique literal exact occurrence and rejects missing, ambiguous, shifted, or general-affine cases without consulting LMB coordinates or ownership. A77 keeps the empty forest as the ordinary factorized production root in both routes and applies expanded-4D projection only to proper UV nodes; a focused true/false route regression verifies identical root local and final integrands. |
...
| Saved-state compatibility | **Top-level and integration-workspace boundaries implemented** | Top-level saved state writes manifest version 2 and rejects incompatible versions before decode or overwrite. Integration workspaces independently write version 1 and reject missing, old, or future versions before either resume decoder, with contextual `integrate --restart` guidance and no positional-bincode migration. |
...
| Documentation, cleanup, and push | **Final refresh and publication pending** | This ledger remains the authoritative chronological record and `raised-energy-cff-handoff.md` is its concise companion. The published `raised_energy_cff_wip` bookmark is still the starting checkpoint below the newer local continuation; move it only after final cleanup, fetch/rebase, validation, and artifact audit. `proper_ltd_core` remains a pushed sibling reserve. |
...
| A17 | Decided; source-local identity corrected after integrated-NLO diagnosis | Add one private exact-map projection operation and retain map-indexed local-3D terms through a single UV forest traversal. | `Graph::numerator` remains the only physical-factor ownership mechanism. Each additive carrier branch holds a production `OrientationID` for theta-sector selection and, when generated by a contracted source, the source-local `edge_energy_map` that owns numerator substitution. Distinct `(selector, source map)` pairs are unioned rather than cross-multiplied; the same pair may be zip-added. Root production-owned branches retain their selector partition and strict exact-map identity without normalization. A collapsed local or integrated source residue is averaged over its selected direction-compatible production sectors by `1/N`, but its affine/contact map need not equal any production map. This preserves maps through localization, active-sector joins, and final assembly without recomputing the forest once per map. |
...
- A45 pure-case topology validation: parser-expanded serial powers remove
  ...
  assertion, and disconnected recursion receives the topology setting. GL11
...
- 2026-08-26 production-bridge audit: integrated finite UV terms retain their
  exact source-local EMR maps; production selector IDs partition theta sectors
  without replacing source algebra. The shared connected-loop and pure
  duplicate-denominator global sign is typed metadata consumed exactly once for
  root, reduced, and exact GammaLoop CFF sources. Focused coverage and
  integrated generation establish those ownership boundaries, but inclusive
  `epem_a_ddx` NLO normalization remains open.
