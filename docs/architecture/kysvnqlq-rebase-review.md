...
Last updated 2026-08-24 on the `raised_energy_cff_wip` documentation tip; its
feature-code parent is change `lktlsypu`. This section is the live status
summary; the later sections retain the chronological decision and validation
evidence.
...
| Documentation, cleanup, and push | **Cross-machine handoff published** | This ledger remains the authoritative chronological record in `lktlsypu`; its concise continuation guide is `raised-energy-cff-handoff.md` on the `raised_energy_cff_wip` documentation tip. That bookmark is the canonical published continuation point. `proper_ltd_core` is fully pushed as a sibling reserve. Obsolete source revisions and the detached old ledger revision remain historical material only. |
...
| A60 | Definite persistence boundary outside top-level state; decision pending | Version integration-workspace resume data independently of `state_manifest.toml`. Reject old, missing, or future workspace schema versions before decoding `state/integration_state.bin`, return contextual errors instead of panicking, and do not add a positional-bincode migration unless explicitly requested. | The QoL line adds `StatisticsCounter::numerical_stability` inside serialized `IntegrationState`. Top-level state version 2 does not cover integration workspaces, which persist their own `manifest.json` and `state/integration_state.bin`; at least one resume path still calls decode through `expect`. Main and the selected pre-rebase WIP already share this unversioned/panicking boundary, so it is a product-relevant pre-existing persistence oversight rather than a replay regression. A required independent version-1 manifest checked before either normal or summary decode, contextual errors, and regeneration through `integrate --restart` are the minimal proposed fix; no persistence code has yet been changed. |
