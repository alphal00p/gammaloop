...
#figure(
  align(center)[#table(
    ...
    table.header([Public tensor-reduction mode], [Current conversion], [Freshening disabled],),
    table.hline(),
...
<behavioral-regression>
`tensor_reduction_of_powered_scalar_sum_matches_angular_average` in #link("../../crates/vakint/tests/tensor_reduction_tests.rs")[tensor\_reduction\_tests.rs] exercises both public modes against the same independent formula. It compares the complete rational residual after `together().cancel()`. It does not constrain dummy names, copy counts, intermediate storage or expression formatting, and does not expand the input numerator.
