...
The immutable pre-document source `392c98c3d2f70896ef94267addfcf610e2df2c88` and reconciled C8 `449a59f6f173d100c2da858c541faa1e0dccca6e` share tree `c87cc95044663a68413010aceb81f9475be342c1`: #strong[verified];. This is the exact split-preservation proof before final reports. Later documentation changes require a separate reviewed delta from this reconciled C8 and from the executed test candidate; the final full tree need not equal this pre-document tree. Untouched input duplication/squashing is recorded separately in `squash-tree-preservation.json`.

The observed eight boundaries contain #strong[647 commit/path records and 549 distinct paths];. Exact before/after Git blobs appear in `raised-energy-cff-stack-current-mapping.json`. These counts are an inventory, not a fresh review claim for unchanged earlier hunks.
...
#figure(
  align(center)[#table(
    columns: (25%, 25%, 25%, 25%),
    align: (auto,auto,auto,auto,),
    table.header([Checkpoint], [Corrected source], [Reconciled C8], [Equal tree],),
    table.hline(),
...
#figure(
  align(center)[#table(
    ...
    table.header([\#], [Source], [Kind], [Source description], [Owners], [Functional disposition],),
    table.hline(),
    ...
    [20], [`89a52f17`], [remote], [checkpoint: raised-energy UV/CFF handoff (not green)], [2, 3, 4, 8], [Shared evaluator/generation 2; source/UV correctness 3; profiling/state 4; durable handoff history 8. Temporary HANDOFF\_REMOVE\_FOR\_PR files are net-absent in source tip.],
    ...
    [32], [`91142139`], [remote], [ALL GREEN (ALL\_GREEN)], [none (empty)], [Empty remote marker: duplicated for provenance, contributes no content after squash.],
    [33], [`ac99d32d`], [remote], [ALL\_GREEN: preserve independent CFF occurrences and simplify UV generation], [1, 2, 3, 4, 8], [Contraction foundations 1; independent shared occurrences 2; optimized source/UV generation and consumers 3; profiling/CLI 4; historical integration narrative 8.],
...
<incoming-review-coverage>
Incoming `a1140c90c334ff58a3b040ff3eae06d3645bf0bb` is reviewed against `78395e3ab3ddd8d8f62b2f674d7488484eace197`: #strong[68 paths];, including 65 text and 3 binary artifacts. The retained ledger records 792 text hunks, 30321 added lines and 1718 deleted lines. The three serialized Symbolica blobs have byte/hash and source-provenance review; their contents were not semantically decoded. Bounded reviewer attribution is explicit; filename overlap alone does not assign functionality.

#figure(
  align(center)[#table(
    columns: (25%, 25%, 25%, 25%),
    align: (auto,auto,auto,auto,),
    table.header([Path], [Reviewed owners], [Reviewer(s)], [Review correction],),
    table.hline(),
    [`.github/workflows/continuous-integration.yml`], [C4], [/root], [no],
    [`.github/workflows/nix.yml`], [C4], [/root], [no],
    [`CONTRIBUTING.typ`], [C7], [/root], [no],
    [`SPEED_UP_UV_CTS_FROM_4D.typ`], [C7], [/root/latest\_cff\_review], [no],
    [`bin/README.md`], [C4], [/root], [no],
    [`bin/ram_watchdog.py`], [C4], [/root], [no],
    [`crates/gammalooprs/src/cff/generation.rs`], [C7], [/root/latest\_cff\_review], [yes],
    [`crates/gammalooprs/src/cff/mod.rs`], [C7], [/root/latest\_cff\_review], [yes],
    [`crates/gammalooprs/src/graph/lmb.rs`], [C7], [/root/latest\_cff\_review], [no],
    [`crates/gammalooprs/src/graph/three_d_source.rs`], [C7], [/root/latest\_cff\_review], [yes],
    [`crates/gammalooprs/src/integrands/process/evaluators.rs`], [C3], [/root], [yes],
    [`crates/gammalooprs/src/numerator/energy_degree.rs`], [C7], [/root/latest\_cff\_review], [yes],
    [`crates/gammalooprs/src/utils/symbols.rs`], [C3, C7], [/root/latest\_cff\_review], [no],
    [`crates/gammalooprs/src/uv/approx/direct_3d/branches.rs`], [C7], [/root/latest\_uv\_review], [no],
    [`crates/gammalooprs/src/uv/approx/direct_3d/kernel.rs`], [C7], [/root/latest\_uv\_review], [no],
    [`crates/gammalooprs/src/uv/approx/final_integrand.rs`], [C7], [/root/latest\_uv\_review], [no],
    [`crates/gammalooprs/src/uv/approx/integrated.rs`], [C6], [/root/latest\_uv\_review], [no],
    [`crates/gammalooprs/src/uv/approx/local_3d/residue_localizer.rs`], [C7], [/root/latest\_uv\_review], [no],
    [`crates/gammalooprs/src/uv/approx/local_3d/tests.rs`], [C7], [/root/latest\_uv\_review], [no],
    [`crates/gammalooprs/src/uv/approx/local_4d.rs`], [C7], [/root/latest\_uv\_review], [yes],
    [`crates/gammalooprs/src/uv/approx/mod.rs`], [C7], [/root/latest\_uv\_review], [no],
    [`crates/gammalooprs/src/uv/approx/projected_4d.rs`], [C7], [/root/latest\_uv\_review], [yes],
    [`crates/gammalooprs/src/uv/forest.rs`], [C7], [/root/latest\_uv\_review], [no],
    [`crates/gammalooprs/src/uv/hedge_poset.rs`], [C7], [/root/latest\_uv\_review], [no],
    [`crates/gammalooprs/src/uv/mod.rs`], [C7], [/root/latest\_uv\_review], [no],
    [`crates/gammalooprs/src/uv/orchestrator.rs`], [C7], [/root/latest\_uv\_review], [no],
    [`crates/gammalooprs/src/uv/tests.rs`], [C7], [/root/latest\_uv\_review], [no],
    [`crates/idenso/src/color/simplify.rs`], [C1], [/root/latest\_shared\_review], [no],
    [`crates/idenso/src/color/test/mod.rs`], [C1], [/root/latest\_shared\_review], [yes],
    [`crates/idenso/src/shorthands/chain.rs`], [C1], [/root/latest\_shared\_review], [no],
    [`crates/idenso/src/shorthands/schoonschip/normalize_dots.rs`], [C1], [/root/latest\_shared\_review], [no],
    [`crates/linnet/src/half_edge.rs`], [C1], [/root/latest\_shared\_review], [no],
    [`crates/linnet/src/half_edge/hedgevec.rs`], [C1], [/root/latest\_shared\_review], [yes],
    [`crates/linnet/src/half_edge/nodestore/test.rs`], [C1], [/root/latest\_shared\_review], [no],
    [`crates/linnet/src/tree/child_pointer.rs`], [C1], [/root/latest\_shared\_review], [yes],
    [`crates/spenso/src/network/contract.rs`], [C1], [/root/latest\_shared\_review], [no],
    [`crates/spenso/src/network/graph.rs`], [C1], [/root/latest\_shared\_review], [yes],
    [`crates/spenso/src/network/mod.rs`], [C1], [/root/latest\_shared\_review], [no],
    [`crates/spenso/src/network/parsing/tensor_from_expression.rs`], [C1], [/root/latest\_shared\_review], [no],
    [`crates/spenso/src/network/store.rs`], [C1], [/root/latest\_shared\_review], [yes],
    [`crates/spenso/src/network/tests.rs`], [C1], [/root/latest\_shared\_review], [yes],
    [`crates/spenso/src/shadowing/collect.rs`], [C1], [/root/latest\_shared\_review], [no],
    [`crates/spenso/src/shadowing/tests.rs`], [C1], [/root/latest\_shared\_review], [yes],
    [`docs/architecture/architecture-current.typ`], [C7], [/root], [yes],
    [`docs/architecture/exact-powered-denominator-cff-lifting.typ`], [C7], [/root], [yes],
    [`docs/architecture/local-4d-uv-benchmarks.json`], [C7], [/root/latest\_cff\_review], [no],
    [`docs/architecture/local-4d-uv-correctness.json`], [C7], [/root/latest\_cff\_review], [no],
    [`docs/architecture/local-4d-uv-dispatch-profile.json`], [C7], [/root/latest\_cff\_review], [no],
    [`docs/architecture/local-4d-uv-performance.typ`], [C7], [/root], [yes],
    [`flake.nix`], [C4], [/root], [no],
    [`tests/artifacts/aa_aa_uv_slowdown/symbolica_evaluator_mre/Cargo.lock`], [C7], [/root/latest\_cff\_review], [no],
    [`tests/artifacts/aa_aa_uv_slowdown/symbolica_evaluator_mre/Cargo.toml`], [C7], [/root/latest\_cff\_review], [no],
    [`tests/artifacts/aa_aa_uv_slowdown/symbolica_evaluator_mre/README.md`], [C7], [/root], [no],
    [`tests/artifacts/aa_aa_uv_slowdown/symbolica_evaluator_mre/evidence/original_cli21_failure.json`], [C7], [/root/latest\_cff\_review], [no],
    [`tests/artifacts/aa_aa_uv_slowdown/symbolica_evaluator_mre/evidence/original_cli21_failure.typ`], [C7], [/root/latest\_cff\_review], [no],
    [`tests/artifacts/aa_aa_uv_slowdown/symbolica_evaluator_mre/evidence/original_cli21_panic.txt`], [C7], [/root/latest\_cff\_review], [no],
    [`tests/artifacts/aa_aa_uv_slowdown/symbolica_evaluator_mre/examples/import_remap.rs`], [C7], [/root], [no],
    [`tests/artifacts/aa_aa_uv_slowdown/symbolica_evaluator_mre/import_remap_r1/receipt.json`], [C7], [/root/latest\_cff\_review], [no],
    [`tests/artifacts/aa_aa_uv_slowdown/symbolica_evaluator_mre/import_remap_r1/tiny.symbolica`], [C7], [/root/latest\_cff\_review], [no],
    [`tests/artifacts/aa_aa_uv_slowdown/symbolica_evaluator_mre/inputs/function_map_entries.symbolica`], [C7], [/root/latest\_cff\_review], [no],
    [`tests/artifacts/aa_aa_uv_slowdown/symbolica_evaluator_mre/inputs/params.symbolica`], [C7], [/root/latest\_cff\_review], [no],
    [`tests/artifacts/aa_aa_uv_slowdown/symbolica_evaluator_mre/inputs/provenance.json`], [C7], [/root/latest\_cff\_review], [no],
    [`tests/artifacts/aa_aa_uv_slowdown/symbolica_evaluator_mre/src/main.rs`], [C7], [/root], [no],
    [`tests/artifacts/aa_aa_uv_slowdown/symbolica_evaluator_mre/validation_summary.json`], [C7], [/root/latest\_cff\_review], [no],
    [`tests/tests/test_runs/amplitude_phase_conventions.rs`], [C5], [/root/latest\_uv\_review], [no],
    [`tests/tests/test_runs/scalar_3L_cross_section_inspects.rs`], [C7], [/root/latest\_uv\_review], [no],
    [`tests/tests/test_runs/scalar_phase_conventions.rs`], [C5], [/root/latest\_uv\_review], [no],
    [`tests/tests/uv.rs`], [C7], [/root/latest\_uv\_review], [yes],
  )]
  , kind: table
  )
...
<deliberate-review-corrections>
The current incoming-to-corrected delta contains #strong[21 files];. Independent review covers the 15 production/test correction files, followed by complete public node-set checks, C1 formatting and public sum-fixture construction, plus current documentation sources and any separately verified rendered artifact. Initial formatting and compiler failures remain in the validation attempt ledger. Test oracles are preserved or strengthened; complete values replace incidental cache/class/order assertions.

#figure(
  align(center)[#table(
    columns: (33.33%, 33.33%, 33.33%),
    align: (auto,auto,auto,),
    table.header([Path], [Semantic owner], [Review receipt],),
    table.hline(),
    ...
    [`docs/architecture/architecture-current.typ`], [C7], [docs-edits-receipt.json],
    [`docs/architecture/exact-powered-denominator-cff-lifting.typ`], [C7], [docs-edits-receipt.json],
    [`docs/architecture/local-4d-uv-performance.typ`], [C7], [docs-edits-receipt.json],
