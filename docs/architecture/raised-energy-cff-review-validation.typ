...
#figure(
  align(center)[#table(
    ...
    table.header([Execution], [Observed result], [Local evidence],),
    table.hline(),
...
#strong[Nested inline variables.] The direct control exited 0; wrapping the same invocation in an inline run exited 1 with `Missing command-block variable 'level'`. Logs: `/tmp/raised-review-api-direct.log`, `/tmp/raised-review-api-nested.log`.
...
#strong[Boot-card variable inheritance.] Save these cards as `/tmp/raised-review-api-boot-direct.toml` and `/tmp/raised-review-api-boot-nested.toml`, respectively:
...
#strong[Unsaved state writes.] This exited 0, without `save state` and with `quit -n`, but created `threed_workspace/latest_oriented_expression_path.txt` and the per-graph `oriented_expression.json` under `/tmp/raised-review-api-3drep`. The repository's explicit-save state contract is the expected behavior; the control here is the command's explicit no-save exit. Log: `/tmp/raised-review-api-3drep.log`. Adapt the repository path when reproducing elsewhere.
...
#strong[Diagnostic expression parser];, `/tmp/raised-review-eval-parser.rs`:
...
#strong[Malformed graph validation];, `/tmp/raised-review-validator.rs`:
...
#strong[Preserved-tree energy-bound validation];, `/tmp/raised-review-bounds.rs`:
