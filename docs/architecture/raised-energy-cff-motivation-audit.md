...
At the pinned local-UV measurement source, physical GL00/GL01 generation premiums over erased 3D are 4.45% and 0.82%, and sampling is faster under the reported settings. Remaining scalar setup/generation costs include GL21/base at 1.431860 times erased 3D, GL21/quadratic `q7` at 1.376004 and GL17/base at 1.368080. These are single-run observations with integrated and threshold subtraction enabled; they do not establish a general slowdown or a universal improvement. The [performance report](local-4d-uv-performance.md) retains full workload, binary, accounting and uncertainty details.
...
| Family | Recorded result | Complete-result evidence |
| --- | --- | --- |
