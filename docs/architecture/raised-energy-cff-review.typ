...
+ #strong[\[P2\] Nested inline runs expand variables before entering the inner scope.] #link("/common/dev/gammaloop/higher-power-energies-review/crates/gammaloop-api/src/commands/run.rs:268")[run.rs:268] sends any placeholder-containing inline command through template expansion before recognizing a nested `run`. A direct invocation with its own `-D level=warn` and a body using `$(level)` succeeds; wrapping that same invocation inside `run -c` fails with “Missing command-block variable 'level'”. Both cases were run through the real CLI. Source tracing also shows an outer definition can capture the inner placeholder before the inner override applies. Prepare the inner invocation in its own environment, as the ordinary block path already does at line 477. One environment-aware preparation boundary would remove this divergence.

+ #strong[\[P2\] Loading nested command blocks demands variables intended for invocation.] #link("/common/dev/gammaloop/higher-power-energies-review/crates/gammaloop-api/src/session.rs:238")[session.rs:238] skips static validation only when a command's own raw text contains a placeholder. For `outer = ["run inner"]` and a parameterized `inner`, it recursively prepares `inner` with an empty environment before the top-level `run outer -D level=warn` can execute. A real boot-card reproduction exits 1; the equivalent single-block control exits 0. Reusable nested block libraries are affected even before execution. Keep static name/cycle validation, but defer invocation-variable resolution until the inherited environment exists.

+ #strong[\[P2\] Default 3Drep output violates the explicit state-write policy.] #link("/common/dev/gammaloop/higher-power-energies-review/crates/gammaloop-api/src/commands/threedreps/mod.rs:476")[threedreps/mod.rs:476] selects `<active-state>/threed_workspace` in ordinary mode, and lines 395--410 save JSON and an output pointer by default. I imported a scalar box, ran `3Drep build --no-pretty`, and ended with `quit -n`; both files were created inside the state directory despite no save command. #link("/common/dev/gammaloop/higher-power-energies-review/CONTRIBUTING.typ:369")[CONTRIBUTING.typ:369] reserves state-directory writes for explicit save/quit-output operations and enabled logs. Use the existing cwd-output policy consistently, or obtain a deliberate repository-policy change. This is a filesystem behavior issue, not a CFF-value discrepancy.

+ #strong[\[P2\] The public diagnostic numerator parser silently changes arithmetic.] #link("/common/dev/gammaloop/higher-power-energies-review/crates/three-dimensional-reps/src/eval.rs:756")[eval.rs:756] bypasses its right binding power for exponentiation; #link("/common/dev/gammaloop/higher-power-energies-review/crates/three-dimensional-reps/src/eval.rs:777")[eval.rs:777] binds unary minus more tightly than powers; and #link("/common/dev/gammaloop/higher-power-energies-review/crates/three-dimensional-reps/src/eval.rs:836")[eval.rs:836] narrows an exponent to `i32` without checking its range. A compiled public `evaluate_expression` reproduction on a generated unit graph gives `-2**2 = 4`, `-(2**2) = -4`, `2**3**2 = 64`, and `2**4294967296 = 1`. The first and chained-power cases contradict conventional power precedence; the last silently wraps the exponent to zero. Use checked conversion and define or reject chained powers explicitly. Tests should compare evaluated arithmetic at this public boundary. This parser belongs to the shared crate's diagnostic evaluation feature; the production compiled GammaLoop integrand does not use it.

+ #strong[\[P2\] Route-comparison tolerances can accept order-one relative errors.] #link("/common/dev/gammaloop/higher-power-energies-review/tests/tests/test_runs/scalar_3L_cross_section_inspects.rs:969")[scalar\_3L\_cross\_section\_inspects.rs:969] applies an absolute `1e-14` floor whenever both Arb results have norm below one. The alternative precision-scaled relative assertion consequently does not constrain small nonzero results: `actual = 1e-18` and `reference = 2e-18` pass despite a 50% normalized difference. The comment describes cancellation to zero, but the exception is applied to every case. The common f64 event and total comparisons also use `max(norm, 1)` with `1e-10`, so they do not restore sensitivity at small scales (#link("/common/dev/gammaloop/higher-power-energies-review/tests/tests/test_runs/utils.rs:457")[utils.rs:457];, #link("/common/dev/gammaloop/higher-power-energies-review/tests/tests/test_runs/utils.rs:641")[utils.rs:641];). The same exception occurs in #link("/common/dev/gammaloop/higher-power-energies-review/tests/tests/test_gamma_star_ttx_nlo_acceptance.rs:315")[test\_gamma\_star\_ttx\_nlo\_acceptance.rs:315];. Use a relative criterion for nonzero results at identical inputs. Handle certified zeros separately, with a justified absolute or propagated error bound. Supplying coordinates as f64 does not by itself justify a universal absolute error in the observable.

+ #strong[\[P2\] Numerical failure predicates can accept NaN and infinity.] #link("/common/dev/gammaloop/higher-power-energies-review/tests/tests/test_runs/utils.rs:635")[utils.rs:635] only panics when `distance > tolerance`. With a NaN total, the distance is NaN and the comparison is false. With an infinite total, both distance and tolerance can be infinite, so the comparison is false again. This can pass when the remaining event data agree, including empty event groups. The explicit finite checks in the scalar test are inside the optional `localized_3d_results` branch; they do not protect every caller path. Require finite components before an affirmative `distance <= tolerance` assertion. This also affects failure-list aggregation in the independent shared-crate tests (#link("/common/dev/gammaloop/higher-power-energies-review/crates/three-dimensional-reps/src/eval.rs:2066")[eval.rs:2066];, #link("/common/dev/gammaloop/higher-power-energies-review/crates/three-dimensional-reps/src/generation.rs:6818")[generation.rs:6818];) and Arb raised-LU comparisons (#link("/common/dev/gammaloop/higher-power-energies-review/crates/gammalooprs/src/cff/mod.rs:4418")[cff/mod.rs:4418];, repeated at 4510, 4626, 4692 and 4791). A compiled reproduction confirms that the actual `F<ArbPrec>` NaN compares neither greater than nor less than or equal to a finite tolerance. An empty failure list therefore does not prove valid agreement. No current production NaN at those sample points is claimed. Using `hypot` would also avoid avoidable overflow in the norm calculation.

+ #strong[\[P2\] Multiple tests freeze private construction choices.] The following would fail under valid implementation simplifications:

  #figure(
    align(center)[#table(
      columns: (33.33%, 33.33%, 33.33%),
      align: (auto,auto,auto,),
      table.header([Location], [Incidental commitment], [Outward replacement],),
      table.hline(),
      [#link("/common/dev/gammaloop/higher-power-energies-review/crates/three-dimensional-reps/src/generation.rs:10639")[generation.rs:10639];], [A private component-product builder must return `None`.], [Generate through the public entry point and compare the complete residue against an independent reference.],
      [#link("/common/dev/gammaloop/higher-power-energies-review/crates/three-dimensional-reps/src/generation.rs:10912")[generation.rs:10912];], [Exactly one variant per orientation and a branching denominator tree.], [Compare the represented denominator/residue function after summing its representation.],
      [#link("/common/dev/gammaloop/higher-power-energies-review/crates/three-dimensional-reps/src/generation.rs:11503")[generation.rs:11503];], [The exact origin string `bounded_degree_known_factor_cff`.], [Check the high-power reconstruction identity; an internal strategy name is not a physics result.],
      [#link("/common/dev/gammaloop/higher-power-energies-review/crates/gammalooprs/src/uv/approx/local_3d/tests.rs:625")[local\_3d/tests.rs:625];], [A completed independent source sum is hosted at exactly `OrientationID(0)`.], [Verify compatible mapping, exactly-once selection, and unchanged complete sum under a different valid host.],
      [#link("/common/dev/gammaloop/higher-power-energies-review/crates/gammalooprs/src/graph/three_d_source.rs:6058")[three\_d\_source.rs:6058];], [Exact edge-count sequence and cache population of two.], [Compare cached/uncached values and sufficient capacity. Put an explicitly required cache resource bound in a separate performance check.],
      [#link("/common/dev/gammaloop/higher-power-energies-review/crates/gammalooprs/src/uv/approx/mod.rs:2066")[approx/mod.rs:2066];], [Exactly one sector/frame and particular loop-carrier IDs.], [Check coordinate compatibility and complete reconstructed value across valid charts.],
      [#link("/common/dev/gammaloop/higher-power-energies-review/crates/gammalooprs/src/graph/three_d_source.rs:3393")[three\_d\_source.rs:3393];], [Synthetic node-name prefixes; nearby tests fix complete parsed graphs and canonical occurrence maps.], [Check physical ownership, valid references and complete mapped values under relabeling.],
      [#link("/common/dev/gammaloop/higher-power-energies-review/crates/gammalooprs/src/cff/mod.rs:3122")[cff/mod.rs:3122];], [Zip two orientation lists and require equal individual maps and carriers.], [Compare complete owner-invariant sums; separately test any promised public selector semantics.],
      [#link("/common/dev/gammaloop/higher-power-energies-review/crates/gammaloop-api/src/commands/bench.rs:1041")[bench.rs:1041];], [Field-by-field settings changes, including a private sentinel.], [Execute benchmarking and subsequent evaluation; verify values and restoration on success and error.],
      [#link("/common/dev/gammaloop/higher-power-energies-review/crates/gammaloop-api/src/state.rs:3618")[state.rs:3618];, #link("/common/dev/gammaloop/higher-power-energies-review/crates/gammalooprs/src/processes/process.rs:1608")[process.rs:1608];], [Bincode equality of entire regenerated CFF, graph and cut-group structures.], [Compare semantic exports, ownership and evaluated save/load results. Byte equality unnecessarily fixes representation.],
      [#link("/common/dev/gammaloop/higher-power-energies-review/crates/spenso/src/network/tests.rs:311")[network/tests.rs:311];, #link("/common/dev/gammaloop/higher-power-energies-review/crates/spenso/src/tensors/parametric.rs:3347")[parametric.rs:3347];], [Exact internal tensor storage and allocation choices.], [Check contraction/evaluation and explicit resource budgets if those are required.],
    )]
    , kind: table
    )

  This does not mean every structural assertion is wrong. Original numerator ownership, denominator momentum/mass/multiplicity, and preserved factorization are explicit repository requirements. Test those semantic invariants without prescribing an arbitrary tree, cache layout, diagnostic label, or host. A deterministic result need not retain one particular lexicographic tie-break, and public threshold IDs need not be dense unless that is a documented API guarantee. Private-builder `None`, diagnostic origin names, host zero and storage layouts should not become indirect correctness requirements. Unit-test placement is also fine: a private module can still be tested by its observable result. Do not replace independent correctness tests with comparisons that call the same implementation twice.

+ #strong[\[P2\] The GL04 denominator certificate does not inspect reconstructed denominators.] #link("/common/dev/gammaloop/higher-power-energies-review/crates/gammalooprs/src/graph/three_d_source.rs:5208")[three\_d\_source.rs:5208] checks two owner-list lengths in a manually constructed mapper, then compares handwritten `D(q)^2 D(-q)^3` with handwritten `D(q)^5`. It never obtains denominators from the exact source builder. The assertion proves the evenness of that chosen denominator, but cannot detect a generated wrong mass, multiplicity or routing. The preceding numerator identity is useful within its narrower mapper scope. Derive one side from actual reconstructed source records and compare its complete denominator with the retained Taylor target. The earlier report overstated this test's denominator coverage; that claim is corrected here.

+ #strong[\[P2\] The 198 added scalar snapshots are not exercised.] The new scalar snapshot files under #link("/common/dev/gammaloop/higher-power-energies-review/tests/tests/test_runs/snapshots")[snapshots] have no snapshot assertion or file reader in #link("/common/dev/gammaloop/higher-power-energies-review/tests/tests/test_runs/scalar_3L_cross_section_inspects.rs")[scalar\_3L\_cross\_section\_inspects.rs];. The route loop ends with comparisons between generated routes at line 992. Editing those snapshot values cannot fail these tests. A common error shared by the compared routes can therefore pass even if it changes the recorded values. Restore a small, independently justified outward baseline where appropriate, or explicitly retain these files as historical artifacts rather than calling them regression coverage. These historical values were not treated as current numerical evidence in this review.

+ #strong[\[P3\] The preserved-tree fast path skips energy-bound validation.] #link("/common/dev/gammaloop/higher-power-energies-review/crates/three-dimensional-reps/src/generation.rs:668")[generation.rs:668] returns before checking the bound IDs at line 689. A public call with a one-edge, zero-loop `ParsedGraph`, preserved edge `[0]`, and bounds `[(999, 2)]` succeeds and retains those invalid source bounds. I reproduced this against the compiled library. `ParsedGraph` uses the default absent edge-index map, so the earlier optional remapping validation does not catch it. Validate all input bound IDs before the early return. This is an input validation inconsistency; no valid-input CFF algebra error is claimed.

+ #strong[\[P3\] The public graph validator panics on malformed signature dimensions.] #link("/common/dev/gammaloop/higher-power-energies-review/crates/three-dimensional-reps/src/validator.rs:39")[validator.rs:39] allocates vertex balances from declared loop/external counts, then indexes them using unchecked signature lengths at lines 52--53. A compiled public-API call with an edge signature `[1]` and no declared loops panics with a zero-length out-of-bounds index instead of returning an invalid `GraphValidation`. Public `ParsedGraph` fields allow this construction. Validate dimensions before balance accumulation and return a structured rejection. This concerns malformed external Rust input; no valid internally generated graph failure was demonstrated.

+ #strong[\[P3\] Arb stability overflow renders an impossible accuracy bound.] #link("/common/dev/gammaloop/higher-power-energies-review/crates/gammalooprs/src/integrands/evaluation.rs:443")[evaluation.rs:443] computes `10.0_f64.powf(-1000.0)` for the Arb histogram's overflow bound. That underflows to zero. An Arb stability result with estimated relative accuracy zero is placed in this overflow bin, and the displayed median becomes `<0.0e0` via lines 500--513. Keep the logarithmic exponent through formatting, or otherwise represent the bound without first converting it to an unrepresentable f64 number. This affects reporting, not the integrand.

#strong[Functional architecture and its evidence]
...
#figure(
  align(center)[#table(
    columns: (33.33%, 33.33%, 33.33%),
    align: (auto,auto,auto,),
    table.header([Route], [Ordered stages], [Correct comparison boundary],),
    table.hline(),
...
The physical graph, intended numerator and completed cut are shared inputs; local Taylor construction first differs at `Direct3dApproximation::run` versus `Local4dCts` reconstruction and `Projected4dCts` composition. The shared CFF engine and integrated add-back are downstream boundaries, so they should not be blamed for a route difference until their actual inputs have been compared. The review checked ownership/denominator contracts before assessing low-level algebra. It did not establish a new valid-input route mismatch.

Different per-key decompositions or individual `lu_cut_order` pieces are not, by themselves, evidence of a physical mismatch. The handoff correctly explains redistribution between derivative slots. Conversely, agreement of just one such piece is insufficient. Tests should reflect this boundary.
...
- #link("/common/dev/gammaloop/higher-power-energies-review/crates/three-dimensional-reps/src/eval.rs:3145")[eval.rs:3145] derives simple/double-pole residues independently and compares full CFF values.
- #link("/common/dev/gammaloop/higher-power-energies-review/crates/three-dimensional-reps/src/eval.rs:3256")[eval.rs:3256] compares independent bubble contour moments across seeds and numerator forms. Keep the value assertions while relaxing incidental branch-layout assertions.
- #link("/common/dev/gammaloop/higher-power-energies-review/crates/three-dimensional-reps/src/eval.rs:1255")[eval.rs:1255] checks a denominator-cancellation identity under a selected cut.
- #link("/common/dev/gammaloop/higher-power-energies-review/crates/three-dimensional-reps/src/eval.rs:1730")[eval.rs:1730] checks invariance under a nonzero auxiliary sampling scale.
- #link("/common/dev/gammaloop/higher-power-energies-review/crates/gammalooprs/src/graph/three_d_source.rs:4986")[three\_d\_source.rs:4986] checks a common-loop numerator identity for a manually constructed mapper. Its denominator section does not certify production reconstruction; see finding 8.
- #link("/common/dev/gammaloop/higher-power-energies-review/tests/tests/test_epem_a_ddx_nlo_acceptance.rs:30")[test\_epem\_a\_ddx\_nlo\_acceptance.rs:30] uses physical normalization and uncertainty requirements, supplying an oracle beyond agreement between implementations.
- #link("/common/dev/gammaloop/higher-power-energies-review/tests/tests/test_cli.rs:253")[test\_cli.rs:253] exercises command substitution, defaults, nested overrides, validation before partial execution, and persisted command history through outward behavior.

#strong[Rust patterns and effectiveness]

#figure(
  align(center)[#table(
    columns: (50%, 50%),
    align: (auto,auto,),
    table.header([Pattern and example], [Assessment],),
    table.hline(),
...
#strong[Documentation and generated-artifact consistency]

The architecture documents preserve useful mathematical contracts, but several “current” statements have drifted. #link("/common/dev/gammaloop/higher-power-energies-review/docs/architecture/architecture-current.typ:514")[architecture-current.typ:514] still names state schema version 4 while the API uses 5; #link("/common/dev/gammaloop/higher-power-energies-review/docs/architecture/kysvnqlq-rebase-review.typ:76")[kysvnqlq-rebase-review.typ:76] has readiness/A79 statements about a common-denominator DOD1 topology that conflict with the newer separate natural-topology implementation. The run-history schema lacks the new UV profile selection fields and enum. Reconcile current contracts and regenerate schemas; retain historical reports with a clear historical scope. These are consistency comments, not evidence of a numerical defect.

All 200 changed snapshots were audited as artifacts. The 198 scalar snapshots have the coverage problem in finding 9. The two other snapshots are referenced by assertions. JSON syntax and local schema references were checked; fixture text and source references were inspected. The changed `state_map.bin` was inventoried by size/hash, not semantically decoded. Historical acceptance values were not independently rederived from their publications.

#strong[Review coverage and executed validation]
...
#figure(
  align(center)[#table(
    columns: (50%, 50%),
    align: (auto,auto,),
    table.header([Check], [Observed result],),
    table.hline(),
...
Companion files: #link("/common/dev/gammaloop/higher-power-energies-review/docs/architecture/raised-energy-cff-review-validation.typ")[validation evidence] and #link("/common/dev/gammaloop/higher-power-energies-review/docs/architecture/raised-energy-cff-review-coverage.typ")[file coverage ledger];.
