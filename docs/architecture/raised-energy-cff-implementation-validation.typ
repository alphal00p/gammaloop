...
Production validation ran on 2026-09-08; approved test-oracle corrections were verified on 2026-09-09 for implementation change `wuuyylluomunwsqoonmvmqttszsnynvr`. The reviewed tip was `91142139e1fde44cc4709a35ad090631d27ca451`; the untouched pre-implementation baseline used for the independent pole check was `55db15c1c14ae34d1bdae8566c8e1c3c50a494e2`.

Commands ran from the repository root in its Nix development environment with the supplied Symbolica license available to the processes and `RUST_MIN_STACK=67108864`. The license is not included in this record. Independent tests may run concurrently with this license. The final main suites used Nextest profile `test_gammaloop`, four test threads and no retries. Existing tests retained their own execution-order annotations; data serialization and its behavioral tests remain in scope. Timings below exclude compilation and build-lock waits.

=== Coefficient migration: 2026-09-09
<coefficient-migration-2026-09-09>
The follow-up change `wuqmzzux` migrates the four `LinearEnergyExpr` coefficient fields and `CFFVariant::prefactor` to `Rational`, including constructors, consumers and native serde/bincode support. All #strong[805 distinct selected tests are verified passing] across the migrated production-code runs and the approved fixture's focused rerun. The original selection passed 804 tests; the one failed fixture is now corrected and passes. Repeated executions are counted once, and no automatic retries were used. The two unaffected Spenso tests from the earlier 807-test selection were not rerun. This is an affected-scope validation, not a full-workspace pass.

#figure(
  align(center)[#table(
    columns: (16.67%, 16.67%, 16.67%, 16.67%, 16.67%, 16.67%),
    align: (auto,auto,auto,auto,auto,auto,),
    table.header([Suite], [Executed], [Passed], [Failed], [Seconds], [Session log],),
    table.hline(),
...
Formatting and all-target checks/Clippy passed for the affected shared/core/API/integration packages. The shared crate passed all-target checks with both all features and default features disabled. The latter retains pre-existing unused imports/helpers in eval-gated test modules; `proc-macro-error2` also retains its existing future-compatibility notice. No new lint warnings remain in the all-feature shared or consumer checks.

The existing persistence test now exercises negative fractions and coefficients exceeding i64, then compares complete public expressions and energy maps after binary and JSON round-trips. Source-mapping fixtures express their arbitrary symbolic witnesses through existing energy substitutions; every original assertion, GL04 common-chart certificate and tolerance is unchanged. The corrected fixture substitution is recorded below. Native Rational serialization changes coefficient encodings; old-format compatibility is not retained under repository policy.

The first shared check found two Rust ownership/type errors, which were corrected before tests ran. A now-unused production import was moved into its test module before the final checks. The initial run failed `exact_taylor_vacuum_affine_circulation_preserves_energy_mapping`: its second, independently constructed pinched mapper lacked the newly introduced symbolic witness substitution. The expressions differed only by `OSE(7)` versus `affine_pole`. With explicit user approval, the fixture now shares that witness substitution between both mappers, retaining their own physical replacements and every original assertion. The focused rerun passed. Formatting, the core all-target check and Clippy also passed after the correction. Only this test fixture changed; the passing production-code results remain applicable.

The driver ran in the existing licensed Nix development shell with `RUST_MIN_STACK=67108864` and `INSTA_UPDATE=no`. Independent Nextest suites ran concurrently; Cargo's build-directory lock coordinated compilation. The commands and driver result are recorded below; no secret is included.
...
Initial driver result: `SHARED=0 CORE=100 API=0 INTEGRATIONS=0`. The approved fixture correction was then verified in the licensed Nix environment with the same stack setting and zero retries:
...
<earlier-implementation-results>
Before the coefficient migration, all 807 distinct selected tests passed after the approved test-only corrections: 106 shared, 276 core, 375 API, 48 integrations and two Spenso tests. The earlier core run passed 275 of 276; its sole cache-oracle failure is resolved by the passing focused rerun below, which also checks the analogous triangle case. The complete shared suite was rerun. Production code did not change in that test-only completion; the coefficient migration subsequently reran the affected shared, core, API and integration selections. Repeated tests and the untouched baseline are excluded from the total. A full workspace run is not claimed.

#figure(
  align(center)[#table(
    columns: (16.67%, 16.67%, 16.67%, 16.67%, 16.67%, 16.67%),
    align: (auto,auto,auto,auto,auto,auto,),
    table.header([Run], [Executed], [Passed], [Failed], [Seconds], [Session log],),
    table.hline(),
...
Formatting and affected all-target Cargo checks/Clippy passed. The test-only completion reran shared/core all-target checks and Clippy; its lint logs are `/tmp/raised-impl-approved-shared-clippy.log` and `/tmp/raised-impl-approved-core-clippy.log`. The production-code lint logs include `/tmp/raised-impl-owned-mass-clippy.log`, `/tmp/raised-impl-final-shared-clippy.log`, `/tmp/raised-impl-final-positive-sign-clippy.log` and `/tmp/raised-impl-spenso-clippy-final.log`. The only reported notice is the dependency `proc-macro-error2` future-compatibility warning.

=== Validation commands
<validation-commands>
The final-driver commands are transcribed below; equivalent shared/Spenso reproduction commands follow. Long nextest expressions preserve the selected scope. The shell drivers ran independent suites concurrently; the commands below can also be run individually.
...
The schema export completed successfully and all 205 local schema references resolve. Only `runhistory.json` changed, reflecting the template payload and refreshing existing UV-profile schema drift. Snapshot audit compared all 198 archived blobs to the reviewed originals; every file is identical. The audit ledger is `/tmp/raised-final-snapshot-move-audit.json`.

=== Approved test-oracle corrections
<approved-test-oracle-corrections>
The user authorized the prepared corrections on 2026-09-09 after the repository-required approval request. All affected checks now pass. The three original failures were mistakes in the replacement oracles; numerical tolerances remain unchanged.

#figure(
  align(center)[#table(
    columns: (50%, 50%),
    align: (auto,auto,),
    table.header([Test], [Corrected outward contract],),
    table.hline(),
...
No production code changed during this completion. Final formatting, whitespace, document compilation and visual PDF checks also passed.

=== Independent diagnosis of the mixed UV correction
<independent-diagnosis-of-the-mixed-uv-correction>
Unchanged GL04 strict Arb comparisons initially disagreed by roughly `8.5749e-21` in the imaginary part. Captured complete evaluator inputs and normalized function maps were identical. Independent 90, 180 and 330 digit rational-program replays reproduced the discrepancy; coefficient extraction isolated it to a one-loop integrated localization factor. Varying its localization scale showed that the small default-scale error was systematic, not an Arb precision limit.

The direct 3D active coefficient lacked the vacuum-mass hard scaling already present in the four-dimensional forest Taylor operation when that operation encloses the coefficient owner. Normalized localization kernels stay frozen; a disjoint coefficient stays fixed for that particular Taylor operation but may become active for a later enclosing one. The final direct correction retains the owner of each connected coefficient using transient `mUV(owner)` applications. The Taylor operation scales contained owners, leaves disjoint ones fixed, and rejects partial overlaps. Output copies restore ordinary `mUV`; stored sectors retain their owners for later enclosing steps. Four-dimensional Taylor and integrated coefficient code retain their established mass scaling.

A blanket direct mass substitution also failed GL00 q7-squared and the GL04 quadratic-pair numerator, both of which contain disjoint divergent components. Fresh GL00 capture isolated the mismatch to the bare cut contribution: threshold terms agreed, all paired Arb inputs were identical, and all 43 normalized function definitions matched. Independent program replay reproduced the generated-function difference. The scoped correction preserves the component information needed at this boundary. Individual LU-order coefficient comparisons were treated as diagnostics; they were not substituted for the complete-cut oracle.

The independent two-loop renormalization test was run unchanged on an untouched archive of the pre-implementation baseline with `--locked --offline` and `INSTA_UPDATE=no`; it passed. It also passes on the final code, together with depth three and all six discriminating GL00/GL04 cases. Incorrect four-dimensional alternatives were discarded. Their interrupted broad runs are not included in the successful validation totals, and no snapshots were accepted or updated.
