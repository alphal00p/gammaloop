...
- A67: the mechanical schema, live-example, explicit read-only-output, Python
  ...
  caller is inside the legacy commented method. The review implementation now
  places default 3Drep artifacts outside active state, executes the late-parsing
  fixture through the CLI, and checks benchmark value/restoration behavior.
- A65's zero-scale rejection is now unconditional; nonzero-scale invariance is
...
- Computed UV-forest export uses the persisted exact production CFF catalogue
  and map IDs. It does not regenerate or infer affine shifts from coarse stored
  orientations. Persistence tests compare selector-weighted residue expressions.
...
Earlier checks encountered a restricted-license/global-initialization boundary
when identical binaries ran concurrently; those tests passed individually.
With the supplied license, independent tests may run concurrently. Retain existing
shared-state locks and resource limits. Rerun the final suite after the newer exact
...
4. Do not change the protected GL11 tests or snapshots while their
   ...
   unresolved. If approval is obtained, run its battery:
