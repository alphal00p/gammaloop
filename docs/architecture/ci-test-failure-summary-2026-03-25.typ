...
<ci-test-failure-summary-2026-03-25>
#quote(block: true)[
#strong[Lifecycle:] Archived investigation captured 2026-03-25

This record predates later fixes and does not identify one immutable source revision for every
run. Test names, snapshots, and environment behavior may have changed. Preserve it as triage
evidence; use the current CI workflow and test results for the present contract.
]
...
+ `SYMBOLICA_LICENSE=${SYMBOLICA_LICENSE:-GAMMALOOP_USER} just test-ci`
+ `SYMBOLICA_LICENSE=${SYMBOLICA_LICENSE:-GAMMALOOP_USER} just test-nix`
...
+ Done on this branch: fix `linnet-py` default-feature behavior so the workspace test build can start.
+ Done on this branch: move Python API subprocess tests behind an explicit feature so `just test-ci` does not report unexecuted tests as passing.
+ Fix the obvious non-snapshot runtime regressions:
...
+ Fix the Nix-local invocation so `SYMBOLICA_LICENSE` is visible to the flake checks.
+ Triage the remaining broad `SIGABRT` set in `gammalooprs`, `idenso`, `spenso`, and related crates after the first blockers are reduced.
