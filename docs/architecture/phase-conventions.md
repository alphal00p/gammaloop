...
Cut-side preprocessing removes a fixed-momentum attachment vertex only when it
is incident to the initial-state cut. A fixed-momentum bridge joining internal
components remains in the physical side: removing its endpoint can erase a loop
and make the cut-side loop-momentum bases incomplete. Channel construction uses
the complete retained sides.
