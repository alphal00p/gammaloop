...
This companion records the implementation following the #link("raised-energy-cff-review.typ")[historical review] of `main...raised_energy_cff_wip`. The original findings and their reproduced failures remain historical evidence; this document describes the resulting changes. The #link("raised-energy-cff-review-validation.typ")[validation appendix] records the earlier review commands and reproductions. The #link("raised-energy-cff-implementation-validation.typ")[implementation validation record] gives the actual post-change commands, counts and verification scope.
...
#figure(
  align(center)[#table(
    columns: (50%, 50%),
    align: (auto,auto,),
    table.header([Review finding], [Resulting behavior and verification boundary],),
    table.hline(),
...
The direct route now tags each connected coefficient's vacuum mass as transient `mUV(owner)` before multiplying coefficients. A Taylor operation scales that mass only when its subgraph contains the owner; it leaves disjoint owners fixed and rejects partial overlaps. A later enclosing operation can scale those retained owners. `DirectSector::combine` restores physical `mUV` on output copies, preserving the stored owners for further forest steps. Normalized localization kernels stay frozen throughout, and the auxiliary OSE deformation mass retains its separate role.
...
<rust-patterns-and-kiss>
#strong[Reuse the owner of an invariant.] `PreparedCommand::prepare_with_environment` now owns inline and named command preparation. Clap uses the existing `RunVariable::from_str` implementation directly. Graph-name lookup delegates to graph lookup. These changes remove parallel paths that had drifted in variable expansion and storage selection.

The existing surface interner becomes an owned `ExpressionAssembler`. The three CFF builders reuse its surface copying, interning, variant insertion and numerator-label finalization. Moving these operations together removes repeated assembly code without adding a second builder framework. `Direct3dApproximation::run` similarly reuses its existing local and integrated execution methods, keeping Taylor transitions and frozen-factor handling in their established owners.

`DirectSector` keeps the active CFF and coefficient separate from normalized localization kernels. The direct correction also carries each integrated factor with its existing subgraph owner, using a tuple at the existing execution boundary. A transient one-argument `mUV(owner)` application preserves that ownership after symbolic multiplication; existing subgraph encoding and containment checks determine which masses scale. This adds a small internal symbolic convention, with validation at Taylor expansion and removal on output copies. It resolves the disjoint-versus-enclosing distinction without adding a helper, Rust type, mode or symbol, or changing four-dimensional integration.

#strong[Use types for real alternatives.] `CommandTemplate(String)` carries the text required to expand a template; a missing optional history string can no longer turn into a synthetic command. It is skipped by Clap. Template equality is ordinary command equality, and the existing history serialization path reads the payload. The late-parsing test uses a real boolean `quit` option and verifies its executed exit behavior after save/load.

Private exact-frame and sub-LMB inputs travel as one `Option` tuple, and exact projection options belong to the existing `Exact` variant. These representations prevent mismatched optional inputs. The generic energy-candidate classification was only used with `()`; its validation now lives directly at the existing source-occurrence boundary.
...
`EvaluatorStack::from_integrand_with_timings` selects the existing orientation-local, explicit-sum or deferred-body constructor. Four callers supply their data instead of repeating three branches. Borrowed optional bodies avoid copies, and a supplied orientation catalog remains distinguishable from explicit mode. The invalid combination of deferred bodies and a catalog returns an error. Existing catalog-length checks still apply. This consolidation preserves the counterterm callers' existing `unwrap` behavior; it does not create a universal recoverable-error API for evaluator construction.

#strong[Express rational invariants in the types.] The four `LinearEnergyExpr` coefficient fields and `CFFVariant::prefactor` now use `Rational`, including public constructors, arithmetic, fusion, cut handling and adapters. Symbolic energies remain indexed terms; constants are rational numbers. Generated coefficients already obey this restriction. Mapping tests retain their arbitrary symbolic witnesses through existing energy substitutions.

Native rational arithmetic replaces runtime rationality assertions, repeated `Atom` conversion and coefficient string parsing. Integer-only surface conversion still rejects fractions and out-of-range integers. `Atom::num` marks the symbolic output boundary; numerical evaluation calls `Rational::to_f64` at its existing f64 boundary. Existing scalar constructors and scaling methods share their rational implementations. This removes helper layers and invalid coefficient states without introducing a new abstraction.
...
- `internal_signature_dimension_violations` identifies internal edges whose loop or external signature lengths disagree with the graph's declared dimensions.
- `external_signature_dimension_violations` identifies external edges with a mismatched external coefficient length.

Both contribute to `ok`. They also appear in serialized diagnostic output. Rust callers constructing this public struct must provide the fields; no compatibility fallback for older serialized validation objects is added.

`NumericalStabilityMedian` retains its three variants, with their payloads now expressed in base-10 logarithms: `Underflow { log10_lower_bound }`, `InRange { log10_relative_accuracy }` and `Overflow { log10_upper_bound }`. Rust callers matching or constructing these fields must use the new names and units. `formatted_relative_accuracy()` remains the presentation boundary and does not require materializing an unrepresentably small f64 accuracy.

The hidden Rust `Commands::CommandTemplate` variant now requires a `String` payload and is excluded from command-line parsing. Command-history text remains the persistence boundary; there is no new template serialization format or public sentinel command.
...
<post-change-verification>
All 805 distinct selected tests are verified passing for the `Atom` → `Rational` migration. The total combines the concurrent production-code runs with the approved fixture's focused rerun; repeated executions are counted once. The suites used four workers and the supplied license, retaining the repository's per-test resource limits. No assertion or numerical tolerance was weakened. Exact commands, results and scope are in the #link("raised-energy-cff-implementation-validation.typ")[validation record];.

#figure(
  align(center)[#table(
    columns: (50%, 50%),
    align: (auto,auto,),
    table.header([Executed check], [Observed result],),
    table.hline(),
...
Earlier checks verified schema export from an unsaved state, 205 internal schema references and all 198 byte-preserving snapshot moves. The untouched baseline also passed the independent `finite_part_ghost_2loop` oracle in 69.121 seconds, helping reject the discarded four-dimensional rescaling change.
