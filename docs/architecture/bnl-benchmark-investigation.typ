...
<fixed-point-benchmark-timings>
These archived timings used the since-removed `inspect --bench` mode. The current equivalent is the top-level `bench` command with `--duration 5s`, `--n-batches 10`, and `--minimal-integrand`. The original fixed x-space point was not recorded, and `BNL.toml` does not define the referenced `bench_bnl_reference` command block, so the numbers below are historical rather than exactly reproducible from the committed files. The reported uncertainty is the standard error over 10 batches.

#figure(
  align(center)[#table(
    columns: (16.67%, 16.67%, 22.22%, 22.22%, 22.22%),
    align: (auto,auto,right,right,right,),
    table.header([state], [threshold runtime], [total time / sample], [evaluator time / sample], [samples],),
    table.hline(),
...
For a new measurement, set `BNL_POINT` to the complete fixed x-space coordinate vector for the generated integrand and invoke `bench` directly:
