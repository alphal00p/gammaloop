...
// Current implementation, source-pinned measurements, and coverage limits.
#set document(date: datetime(year: 2026, month: 9, day: 14), title: "Raised-energy CFF motivation audit", author: "Lucien Huber",
  description: "Current eight-commit change motivations, source-pinned physical benchmarks, reproduced defects, Rust patterns, KISS and coverage limits.")
...
#v(3mm)
#text(size: 9pt, fill: rgb("667085"))[14 September 2026 · Seven functional commits and documentation]
#v(4mm)
...
#text("The current stack has seven functional owners and an eighth documentation commit. Its motivations include correctness, supported capabilities, simplification and measured performance; a successful current test alone is not evidence that an earlier implementation failed. The retained benchmark inventory covers ")#strong("73 logical categories, 579 commit/path records and 505 distinct paths at its pinned source")#text(". It does not claim complete path coverage of the current eight-commit stack. The ")#link("raised-energy-cff-stack-source-mapping.md")[#text("stack source mapping")]#text(" owns current source attribution and review coverage.")

#text("The foundation/shared-CFF/GL04 benchmark tables and 579-record path map are pinned to ")#text(font: "DejaVu Sans Mono", size: 0.86em, "d55a0f3e9d25e5c64bdb9ef9fb57749ecf680390")#text(" (functional parent ")#text(font: "DejaVu Sans Mono", size: 0.86em, "1a747039")#text("); the tensor baseline is ")#text(font: "DejaVu Sans Mono", size: 0.86em, "39561014")#text(". “Current” in those measured A/B tables means that pinned intervention source. Most experiments restore one method or disable one optimization on its dependencies, so they do not compare complete releases. The separate GL00/GL01 local-UV measurements imported with ")#text(font: "DejaVu Sans Mono", size: 0.86em, "a1140c90")#text(" use numerical source ")#text(font: "DejaVu Sans Mono", size: 0.86em, "911f1824f994611d723dff0f50416cb301718617")#text(" and independently pinned binaries. Neither measurement family certifies the current reconstructed stack; fresh outcomes belong to the ")#link("raised-energy-cff-stack-review-validation.md")[#text("stack validation receipt")]#text(".")

#text("Current Rust dependencies pin Symbolica 2.2.0, Graphica and Numerica to ")#text(font: "DejaVu Sans Mono", size: 0.86em, "https://github.com/alphal00p/symbolica")#text(" revision ")#text(font: "DejaVu Sans Mono", size: 0.86em, "4d0a833eb8e059d1f95bdae5abed2559830b235f")#text(". A locked build uses that revision, not a moving dev head. Each retained benchmark's dependency identity remains in its own receipt; a dependency change must be included when interpreting comparisons.")
...
= #text("Pinned GL04 caches and assignment scoring")
...
#text("Both GL04 inputs have exact equality of all three complete inspection JSON values and all nine computed forest/node DOT exports across variants. Their small real values are compared directly, without a unit-sized absolute tolerance floor. All alternative proposals lose: eleven GL04 groups have map counts ")#text(font: "DejaVu Sans Mono", size: 0.86em, "[14,20,20]")#text("; seven temporal-square groups have ")#text(font: "DejaVu Sans Mono", size: 0.86em, "[32,56,44]")#text(". Count-only disabling retains the same hit counts as the measured implementation, so these inputs do not demonstrate losing-count reuse. They exercise scoring overhead but do not justify its benefit. A partial GL16 trace does reach a winning third proposal: ")#text(font: "DejaVu Sans Mono", size: 0.86em, "[1200,1224,1104]")#text(" reduces the rank-first map count by 8%. All four GL16 variants time out at 180s before inspection or computed exports; none supplies full-value parity or a total-runtime comparison. The traced peak process-tree RSS is 545MB, so this is a time limit, not a reproduction of the source's 30GB failure. Matching diagnostic prefixes show far fewer generated contenders with memoization, and 52 paired parsed-source/bounds records match exactly. Cache hits omit the source and some canonical-key fields, so the traces are not an independent proof of every hit's key identity.")

= #text("Canonical local-4D UV motivation and measured limits")

#text("The canonical projection view separates reusable rational algebra from raw Taylor recursion. Raw sectors retain their physical owners for enclosing operations. Completed hard roles zero and one may use occurrences of a class certified by component, signed routing, mass and full denominator polynomial; physical-source and soft factors retain their restrictions. The retained physical incidence witness constructs the source graph. A signature match alone neither supplies topology nor authorizes a numerator sign change.")

#text("Rational-shell grouping and component-wave composition remove duplicate symbolic work without expanding graph numerators or imposing a global common denominator. Graph-owned preparation, winning CFF and row-mapping caches retain immutable source/template data within explicit byte/entry budgets. Native map counts select among at most three certified proposals; all discarded generation and preparation count as dispatch cost. These mechanisms have distinct responsibilities, but none is justified merely by cache hits or fewer rows. Their combined physical measurements do not isolate each mechanism's speed contribution.")

#text("At the pinned local-UV measurement source, physical GL00/GL01 generation premiums over erased 3D are 4.45% and 0.82%, and sampling is faster under the reported settings. Remaining scalar setup/generation costs include GL21/base at 1.431860 times erased 3D, GL21/quadratic ")#text(font: "DejaVu Sans Mono", size: 0.86em, "q7")#text(" at 1.376004 and GL17/base at 1.368080. These are single-run observations with integrated and threshold subtraction enabled; they do not establish a general slowdown or a universal improvement. The ")#link("local-4d-uv-performance.md")[#text("performance report")]#text(" retains full workload, binary, accounting and uncertainty details.")

#text("Same-frame ")#text(font: "DejaVu Sans Mono", size: 0.86em, "+N/-N")#text(" cancellation exposes a concrete projection bug: a zero created by grouping must return the existing typed zero before requiring energy maps. The complete final cut-order regression covers that behavior without fixing sector layout. Other projection regressions reconstruct complete rational values, preserve frozen-localizer contributions and compare cold/warm/disabled/evicted results. The byte ceiling remains a configured resource contract; preparation counts, class IDs and traversal order do not.")

#text("The allocator regression uses independent complete signed sunset contours, including all 126 degree-five distributions over five equivalent occurrences. At energies ")#text(font: "DejaVu Sans Mono", size: 0.86em, "(2,3,7)")#text(", the degree-five and degree-seven answers are ")#text(font: "DejaVu Sans Mono", size: 0.86em, "1213/16387080192")#text(" and ")#text(font: "DejaVu Sans Mono", size: 0.86em, "-365/47775744")#text(". The oracle tests the contour value and sign independently of native row counts. Physical route comparisons require finite, nonzero complex values at three points and a fixed ")#text(font: "DejaVu Sans Mono", size: 0.86em, "1e-9")#text(" relative complex-norm tolerance. A reported accuracy estimate cannot relax that acceptance threshold. Fresh executions are recorded separately from these contract descriptions.")

#text("GL262 has no complete matched evaluator/runtime result in the retained evidence. Its direct diagnostic reaches a Symbolica evaluator-construction panic with compilation disabled; the paired erased run stops during preprocessing. No confirmed standalone reproducer exists for the large evaluator panic. The tracked nonsymmetric ")#text(font: "DejaVu Sans Mono", size: 0.86em, "f(x,y)")#text(" import MRE demonstrates a distinct argument-order defect and does not establish the panic's cause. Manual PySecDec integrations remain outside automatic acceptance.")
...
[#text("Immutable assignment plans and typed occurrence/owner/class IDs")],
[#text("Keep capacity, routing and selected expression together. Raw Taylor owners and completed certified classes serve different boundaries; their explicit types prevent accidental interchange. LMB coordinates certify identity but do not own capacity.")],
[#text("Immutable ")#text(font: "DejaVu Sans Mono", size: 0.86em, "Arc")#text(" templates with graph-owned mutable retention")],
[#text("Prepared incidence and mappings can be reused without borrowed adapters or global graph caches. Row memoization is separate and cleared per component wave. Byte budgets bound retention, not RSS; complete-value parity under eviction tests the semantic contract.")],
[#text("Typed zero and checked bulk iterator sums")],
[#text("Group first, return the existing zero, and validate every cut-key shape before bulk addition. This removes an error path and repeated growing-sum copies without introducing a compatibility layer or distributing numerator factors.")],
[#text("Existing scalar alias owner at analytic spin expansion")],
[#text("Temporarily protect completed spin-independent scalar products while expanding open tensor contractions. It reuses established collision-safe restoration and prevents bound-index capture before Vakint.")],
[#text("Sparse incidence lists and existing graph operations")],
[#text("Dense edge swaps and touched-node membership remove graph-wide scans and masks while retaining slot/flow/forest semantics. Complete contractions and graph-access behavior are the correctness contracts; isolated performance gains for each bookkeeping change are not established here.")],
...
[#text("C1-25 — Preserve requested odd tensor powers across all five leaf forms")],
[#text("Repeatedly squaring an already squared tensor computes the wrong exponent. Multiplying the fixed paired contraction by the remaining base preserves odd powers and their free indices. Complete execution-value regressions protect the exponent; no isolated speedup is claimed.")],
[#text("C1-26 — Preserve slot, edge-flow and forest identity during graph rewiring")],
[#text("Existing dense swaps and touched-incidence membership avoid repeated full-graph scans. Outward contractions, ancestry and supported graph-access operations constrain behavior; exact node-set representation, cache counts and traversal order do not.")],
)
]
...
[#text("C6-09 — Protect completed scalar products during analytic spin expansion")],
[#text("Source comparison isolates ")#text(font: "DejaVu Sans Mono", size: 0.86em, "(p.k)^2")#text(" becoming ")#text(font: "DejaVu Sans Mono", size: 0.86em, "p^2*k^2")#text(" before Vakint through bound-index reuse. Existing aliases protect completed spin-independent products while open tensor slots remain visible to expansion. Complete open/closed Dirac oracles and physical analytic checks motivate the fix; it does not establish a Vakint integration error.")],
)
]
...
#table(columns: (0.35fr, 0.65fr),
table.header([#text("Change")], [#text("Evidence and conclusion")]),
[#text("C7-01 — Canonical completed UV classes with physical incidence certificates")],
[#text("Equal completed hard channels can share algebra and occurrence capacity while raw Taylor owners remain intact. Exact numerator and denominator certificates guard signs, masses, prescriptions and component domains. This is a certified algebra boundary, not permission to infer topology.")],
[#text("C7-02 — Group rational shells and complete component-wave states")],
[#text("Merge only compatible denominators, requests and frozen bindings while preserving numerator factors. Complete values justify regrouping; native state counts are diagnostics. Combined physical measurements do not isolate the regrouping speedup.")],
[#text("C7-03 — Return typed zero after projection-sector cancellation")],
[#text("Same-frame opposite sectors can cancel only after grouping. Returning the existing zero before energy-map validation preserves every final cut-order value; the regression covers the complete builder result.")],
[#text("C7-04 — Compressed bounded occurrence allocation")],
[#text("Cyclic baseline, optional packed excess and one placement challenger bound native trials. Independent signed sunset values replace a native row-count golden. The policy is not an exhaustive minimum, and rejected candidates count toward dispatch cost.")],
[#text("C7-05 — Graph-owned source/template and row-mapping retention")],
[#text("Reuses prepared immutable data within explicit byte/entry limits and clears row values per component wave. Cold/warm/disabled/evicted complete outputs are equal by contract. Cache-hit observations alone do not prove a performance gain.")],
[#text("C7-06 — Factor-preserving color preparation and bulk final sums")],
[#text("Existing color simplification and Symbolica bulk addition avoid repeated growing-expression work. Factorized momentum coefficients and complete cut-key validation remain required. Timings cover the combined workflow, not an independent contribution from every moved call.")],
[#text("C7-07 — Account for full dispatch cost and preserve physical comparison strength")],
[#text("Winning and discarded preparation/generation intervals are distinguished. Physical route comparisons use fixed relative complex-norm tolerance; reported accuracy cannot weaken it. Source-pinned GL00/GL01 gates pass, while scalar regressions and GL262 limits remain explicit.")],
)
]

= #text("Commit 8: motivation rows")

#block[
#set text(size: 8.6pt)
#table(columns: (0.35fr, 0.65fr),
table.header([#text("Change")], [#text("Evidence and conclusion")]),
[#text("C8-01 — Current reports, Typst/PDF and durable source/validation provenance")],
[#text("Requested documentation and provenance. The report records current mechanisms, complete-result contracts, source-pinned costs and unsupported claims. Fresh reconstructed-stack receipts own execution results. No runtime speedup applies.")],
)
]
...
table.header([#text("Family")], [#text("Recorded result")], [#text("Complete-result evidence")]),
