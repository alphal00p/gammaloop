...
<raised-energy-cff-change-motivation-and-benchmarks>
Current mechanisms, physical inputs and source-pinned measurements · 14 September 2026
...
The current stack has seven functional owners and an eighth documentation commit. Its motivations include correctness, supported capabilities, simplification and measured performance; a successful current test alone is not evidence that an earlier implementation failed. The retained benchmark inventory covers #strong[73 logical categories, 579 commit/path records and 505 distinct paths at its pinned source];. It does not claim complete path coverage of the current eight-commit stack. The #link("raised-energy-cff-stack-source-mapping.typ")[stack source mapping] owns current source attribution and review coverage.

The foundation/shared-CFF/GL04 benchmark tables and 579-record path map are pinned to `d55a0f3e9d25e5c64bdb9ef9fb57749ecf680390` (functional parent `1a747039`); the tensor baseline is `39561014`. “Current” in those measured A/B tables means that pinned intervention source. Most experiments restore one method or disable one optimization on its dependencies, so they do not compare complete releases. The separate GL00/GL01 local-UV measurements imported with `a1140c90` use numerical source `911f1824f994611d723dff0f50416cb301718617` and independently pinned binaries. Neither measurement family certifies the current reconstructed stack; fresh outcomes belong to the #link("raised-energy-cff-stack-review-validation.typ")[stack validation receipt];.

Current Rust dependencies pin Symbolica 2.2.0, Graphica and Numerica to `https://github.com/alphal00p/symbolica` revision `4d0a833eb8e059d1f95bdae5abed2559830b235f`. A locked build uses that revision, not a moving dev head. Each retained benchmark's dependency identity remains in its own receipt; a dependency change must be included when interpreting comparisons.
...
#figure(
  align(center)[#table(
    ...
    table.header([Strategy], [Double triangle], [ttH], [BNL],),
    table.hline(),
...
#figure(
  align(center)[#table(
    columns: (25%, 25%, 25%, 25%),
    align: (auto,auto,auto,auto,),
    table.header([Strategy], [Double triangle s], [ttH s], [BNL s],),
    table.hline(),
...
#figure(
  align(center)[#table(
    columns: 4,
    align: (auto,auto,auto,auto,),
    table.header([Input], [Old network ms], [Current network ms], [Old / current],),
    table.hline(),
...
#figure(
  align(center)[#table(
    columns: 4,
    align: (auto,auto,auto,auto,),
    table.header([Controlled input], [Old ms], [Current ms], [Old / current],),
    table.hline(),
...
On BNL, direct registration changes the alias-registration stage from #strong[4.669 ms to 0.079 ms] (58.8×), with four aliases and equivalent complete results. That stage is a small part of total runtime; the total-wall distributions do not establish an end-to-end speedup. The double triangle and ttH create no aliases, so their old-registration flag timings are inactive controls.
...
#figure(
  align(center)[#table(
    columns: 4,
    align: (auto,auto,auto,auto,),
    table.header([Operation/input], [Old µs], [Current µs], [Old / current],),
    table.hline(),
    [chain\_physical], [14483.235], [83.504], [173.44×],
    [chain\_scalar], [31.408], [0.239], [131.19×],
    [prune\_nozero], [7430.241], [360.509], [20.61×],
...
#figure(
  align(center)[#table(
    columns: 4,
    align: (auto,auto,auto,auto,),
    table.header([Variant], [Network ms], [Whole wall s], [Child RSS MB],),
    table.hline(),
...
#figure(
  align(center)[#table(
    ...
    table.header([Input], [Current], [Eager layers], [Eager fanout], [No base cache], [No compaction],),
    table.hline(),
...
#figure(
  align(center)[#table(
    columns: 4,
    align: (auto,auto,auto,auto,),
    table.header([Input / variant], [Wall mean ± SD s], [Wall median s], [Peak child MB],),
    table.hline(),
...
Both GL04 inputs have exact equality of all three complete inspection JSON values and all nine computed forest/node DOT exports across variants. Their small real values are compared directly, without a unit-sized absolute tolerance floor. All alternative proposals lose: eleven GL04 groups have map counts `[14,20,20]`; seven temporal-square groups have `[32,56,44]`. Count-only disabling retains the same hit counts as the measured implementation, so these inputs do not demonstrate losing-count reuse. They exercise scoring overhead but do not justify its benefit. A partial GL16 trace does reach a winning third proposal: `[1200,1224,1104]` reduces the rank-first map count by 8%. All four GL16 variants time out at 180s before inspection or computed exports; none supplies full-value parity or a total-runtime comparison. The traced peak process-tree RSS is 545MB, so this is a time limit, not a reproduction of the source's 30GB failure. Matching diagnostic prefixes show far fewer generated contenders with memoization, and 52 paired parsed-source/bounds records match exactly. Cache hits omit the source and some canonical-key fields, so the traces are not an independent proof of every hit's key identity.

=== Canonical local-4D UV motivation and measured limits
<canonical-local-4d-uv-motivation-and-measured-limits>
The canonical projection view separates reusable rational algebra from raw Taylor recursion. Raw sectors retain their physical owners for enclosing operations. Completed hard roles zero and one may use occurrences of a class certified by component, signed routing, mass and full denominator polynomial; physical-source and soft factors retain their restrictions. The retained physical incidence witness constructs the source graph. A signature match alone neither supplies topology nor authorizes a numerator sign change.

Rational-shell grouping and component-wave composition remove duplicate symbolic work without expanding graph numerators or imposing a global common denominator. Graph-owned preparation, winning CFF and row-mapping caches retain immutable source/template data within explicit byte/entry budgets. Native map counts select among at most three certified proposals; all discarded generation and preparation count as dispatch cost. These mechanisms have distinct responsibilities, but none is justified merely by cache hits or fewer rows. Their combined physical measurements do not isolate each mechanism's speed contribution.

At the pinned local-UV measurement source, physical GL00/GL01 generation premiums over erased 3D are 4.45% and 0.82%, and sampling is faster under the reported settings. Remaining scalar setup/generation costs include GL21/base at 1.431860 times erased 3D, GL21/quadratic `q7` at 1.376004 and GL17/base at 1.368080. These are single-run observations with integrated and threshold subtraction enabled; they do not establish a general slowdown or a universal improvement. The #link("local-4d-uv-performance.typ")[performance report] retains full workload, binary, accounting and uncertainty details.

Same-frame `+N/-N` cancellation exposes a concrete projection bug: a zero created by grouping must return the existing typed zero before requiring energy maps. The complete final cut-order regression covers that behavior without fixing sector layout. Other projection regressions reconstruct complete rational values, preserve frozen-localizer contributions and compare cold/warm/disabled/evicted results. The byte ceiling remains a configured resource contract; preparation counts, class IDs and traversal order do not.

The allocator regression uses independent complete signed sunset contours, including all 126 degree-five distributions over five equivalent occurrences. At energies `(2,3,7)`, the degree-five and degree-seven answers are `1213/16387080192` and `-365/47775744`. The oracle tests the contour value and sign independently of native row counts. Physical route comparisons require finite, nonzero complex values at three points and a fixed `1e-9` relative complex-norm tolerance. A reported accuracy estimate cannot relax that acceptance threshold. Fresh executions are recorded separately from these contract descriptions.

GL262 has no complete matched evaluator/runtime result in the retained evidence. Its direct diagnostic reaches a Symbolica evaluator-construction panic with compilation disabled; the paired erased run stops during preprocessing. No confirmed standalone reproducer exists for the large evaluator panic. The tracked nonsymmetric `f(x,y)` import MRE demonstrates a distinct argument-order defect and does not establish the panic's cause. Manual PySecDec integrations remain outside automatic acceptance.
...
#figure(
  align(center)[#table(
    columns: (25%, 25%, 25%, 25%),
    align: (auto,auto,auto,auto,),
    table.header([Comparison], [Old ms/call], [Current ms/call], [Paired old/current ratio \[range\]],),
    table.hline(),
...
#figure(
  align(center)[#table(
    ...
    table.header([Mode / workers], [Current s], [Old comparator s], [Global lock s], [Both s],),
    table.hline(),
...
#figure(
  align(center)[#table(
    columns: (25%, 25%, 25%, 25%),
    align: (auto,auto,auto,auto,),
    table.header([Mode / workers], [Old/current ratio], [Global/current ratio], [Both/current ratio],),
    table.hline(),
...
#figure(
  align(center)[#table(
    columns: (50%, 50%),
    align: (auto,auto,),
    table.header([Pattern], [Effectiveness supported by the audit],),
    table.hline(),
    ...
    [Immutable assignment plans and typed occurrence/owner/class IDs], [Keep capacity, routing and selected expression together. Raw Taylor owners and completed certified classes serve different boundaries; their explicit types prevent accidental interchange. LMB coordinates certify identity but do not own capacity.],
    [Immutable `Arc` templates with graph-owned mutable retention], [Prepared incidence and mappings can be reused without borrowed adapters or global graph caches. Row memoization is separate and cleared per component wave. Byte budgets bound retention, not RSS; complete-value parity under eviction tests the semantic contract.],
    [Typed zero and checked bulk iterator sums], [Group first, return the existing zero, and validate every cut-key shape before bulk addition. This removes an error path and repeated growing-sum copies without introducing a compatibility layer or distributing numerator factors.],
    [Existing scalar alias owner at analytic spin expansion], [Temporarily protect completed spin-independent scalar products while expanding open tensor contractions. It reuses established collision-safe restoration and prevents bound-index capture before Vakint.],
    [Sparse incidence lists and existing graph operations], [Dense edge swaps and touched-node membership remove graph-wide scans and masks while retaining slot/flow/forest semantics. Complete contractions and graph-access behavior are the correctness contracts; isolated performance gains for each bookkeeping change are not established here.],
...
<per-change-motivation-inventory>
Rows C1--C6 retain the pinned catalog identifiers and state current justification or measurement limits. Additional rows describe current mechanisms; C7 owns canonical UV optimization and C8 owns documentation. The retained machine-readable inventory preserves its source path ownership and `closeout_disposition` evidence; the #link("raised-energy-cff-stack-source-mapping.typ")[stack source mapping] supplies current ownership. Recorded counterexamples are not relabelled as fresh executions. Capability, policy and consumer migrations need not invent a motivating bug.
...
#figure(
  align(center)[#table(
    columns: (50%, 50%),
    align: (auto,auto,),
    table.header([Change], [Evidence and conclusion],),
    table.hline(),
    ...
    [C1-25 --- Preserve requested odd tensor powers across all five leaf forms], [Repeatedly squaring an already squared tensor computes the wrong exponent. Multiplying the fixed paired contraction by the remaining base preserves odd powers and their free indices. Complete execution-value regressions protect the exponent; no isolated speedup is claimed.],
    [C1-26 --- Preserve slot, edge-flow and forest identity during graph rewiring], [Existing dense swaps and touched-incidence membership avoid repeated full-graph scans. Outward contractions, ancestry and supported graph-access operations constrain behavior; exact node-set representation, cache counts and traversal order do not.],
  )]
  , kind: table
  )
...
#figure(
  align(center)[#table(
    ...
    [C3-13 --- Contract finite tensor sums at the numerical evaluator boundary], [Numerical preprocessing accepts complete finite tensor results while preserving free slots and deferred scalar coefficients. Local Taylor algebra retains spin structure; integrated analytic expansion has its own owner. This is a boundary/capability change, with no isolated kernel speedup established.],
  )]
  , kind: table
  )
...
#figure(
  align(center)[#table(
    ...
    [C6-06 --- Canonical backend serialization and post-restoration notation], [Source gives corrupt FORM input (1+2i)#emph[(a+b)];tensor losing parentheses, canonical-name corruption and invalid decimal-complex tokens. Adapter correctness, not speed, motivates the fixes.],
    ...
    [C6-09 --- Protect completed scalar products during analytic spin expansion], [Source comparison isolates `(p.k)^2` becoming `p^2*k^2` before Vakint through bound-index reuse. Existing aliases protect completed spin-independent products while open tensor slots remain visible to expansion. Complete open/closed Dirac oracles and physical analytic checks motivate the fix; it does not establish a Vakint integration error.],
  )]
  , kind: table
  )
...
#figure(
  align(center)[#table(
    ...
    table.header([Change], [Evidence and conclusion],),
    table.hline(),
    [C7-01 --- Canonical completed UV classes with physical incidence certificates], [Equal completed hard channels can share algebra and occurrence capacity while raw Taylor owners remain intact. Exact numerator and denominator certificates guard signs, masses, prescriptions and component domains. This is a certified algebra boundary, not permission to infer topology.],
    [C7-02 --- Group rational shells and complete component-wave states], [Merge only compatible denominators, requests and frozen bindings while preserving numerator factors. Complete values justify regrouping; native state counts are diagnostics. Combined physical measurements do not isolate the regrouping speedup.],
    [C7-03 --- Return typed zero after projection-sector cancellation], [Same-frame opposite sectors can cancel only after grouping. Returning the existing zero before energy-map validation preserves every final cut-order value; the regression covers the complete builder result.],
    [C7-04 --- Compressed bounded occurrence allocation], [Cyclic baseline, optional packed excess and one placement challenger bound native trials. Independent signed sunset values replace a native row-count golden. The policy is not an exhaustive minimum, and rejected candidates count toward dispatch cost.],
    [C7-05 --- Graph-owned source/template and row-mapping retention], [Reuses prepared immutable data within explicit byte/entry limits and clears row values per component wave. Cold/warm/disabled/evicted complete outputs are equal by contract. Cache-hit observations alone do not prove a performance gain.],
    [C7-06 --- Factor-preserving color preparation and bulk final sums], [Existing color simplification and Symbolica bulk addition avoid repeated growing-expression work. Factorized momentum coefficients and complete cut-key validation remain required. Timings cover the combined workflow, not an independent contribution from every moved call.],
    [C7-07 --- Account for full dispatch cost and preserve physical comparison strength], [Winning and discarded preparation/generation intervals are distinguished. Physical route comparisons use fixed relative complex-norm tolerance; reported accuracy cannot weaken it. Source-pinned GL00/GL01 gates pass, while scalar regressions and GL262 limits remain explicit.],
  )]
  , kind: table
  )

=== Commit 8: motivation rows
<commit-8-motivation-rows>
#figure(
  align(center)[#table(
    columns: (50%, 50%),
    ...
    table.header([Change], [Evidence and conclusion],),
    table.hline(),
    [C8-01 --- Current reports, Typst/PDF and durable source/validation provenance], [Requested documentation and provenance. The report records current mechanisms, complete-result contracts, source-pinned costs and unsupported claims. Fresh reconstructed-stack receipts own execution results. No runtime speedup applies.],
  )]
  , kind: table
  )
...
#figure(
  align(center)[#table(
    ...
    table.header([Family], [Recorded result], [Complete-result evidence],),
    table.hline(),
