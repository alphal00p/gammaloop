...
<uv-scalar-profile-benchmark>
#quote(block: true)[
#strong[Lifecycle:] Archived investigation captured 2026-05-18

#strong[Reproducibility:] The named `.sym` fixture remains in the repository, but the extraction
environment variable and focused regression-test target recorded below no longer exist. The
capture does not identify a source revision, hardware model, or toolchain lock. Treat its timing
and hotspot conclusions as historical evidence, not a current performance contract.
]
...
#table(
  columns: (2fr, 0.6fr, 1.4fr),
  table.header([*Test*], [*Result*], [*Observed runtime*]),
  [`uv::tests::scalars_profile`], [passed], [about 101 s in a clean timing run; a later rerun under noisier load took about 123 s],
  [`uv::tests::scalars_profile_new`], [passed], [about 99.5 s],
  [`uv::tests::spinney_partial_cmp_is_equal_for_identical_subgraphs`], [passed], [about 84.7 s],
  [`processes::amplitude::test::generation_orientation_pattern_filters_evaluator_orientations`], [passed], [about 0.31 s],
)
...
#table(
  columns: (1.8fr, 0.8fr, 2fr),
  table.header([*Phase*], [*Approximate time*], [*Notes*]),
  [UV expression / forest preprocessing], [67–69 s], [Dominant cost],
  [Evaluator construction], [17–20 s], [Second-largest cost],
  [Actual UV profile evaluation loop], [14–19 s], [Smaller than construction],
...
#table(
  columns: (3fr, 1fr),
  table.header([*Expression file*], [*Size*]),
