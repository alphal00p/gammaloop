...
1. A single radial map around **H in cut-1 kinematics** can give finite local variance at the measured hard H/Z corner. A signed quadratic distance map, giving density proportional to `|H|^(-1/2)`, is sufficient under the measured regular codimension-two model. It does not bound the leading maximum weights.
2. H and Z occupy **the same three-dimensional p block**, not two independent loop subspaces. To compensate their combined inverse-radius growth, use one joint chart with coordinates `(H,Z,tangent)`, with either a polar normal radius sampled uniformly or two signed-quadratic normal coordinates. Calling this an independent product of two three-dimensional radial E-surface maps would double-count that block.
3. A raw H channel alone does not cover an integrated CT singularity at `H(A-star)=Z(A-star)=0`. However, for native A in this GL638 graph, there is a particularly simple exact conditional map: **at fixed s, the A-star projection is affine in p**, so H(A-star) is another ellipsoid in p. The API need only express the target frame and exact native variant/center to expose this useful special case. A full six-dimensional star-first chart remains a valid more general alternative.
4. Focusing the global radial distance to a Cutkosky shell is generally not a solution to LU-projected singularities: the physical cut-prepared point is invariant under global scaling of the original input. Targeting a cut's own equation after its own LU projection is identically zero. Similarly, A evaluated at its own A-star is identically zero. The target and its evaluation frame are indispensable parts of the channel.
5. This is useful new sampler functionality, but a channel must include the graph partition/energy equation, signed-cycle coordinate embedding, evaluation frame, conditional dependencies, normalized radial law, support/inverse branches and exact physical density. An edge-list-only shorthand is appropriate only when these can be resolved unambiguously.

## Physics and measured evidence being targeted

The published current prescription uses generation LMB `[3,4,7,10]`, where `K=(p0,p0+a0,s0,t0)` and the gluons are `q13=p0-t0`, `q14=s0-p0`. The native cut-1 parent is `[3,6,7,10]`; its two-loop active block `[3,7]` varies p and s while preserving a and t. Native one-loop `[7]` varies s. These statements and the current WH/WF metadata are recorded in [the tracked report](../gl638/README.md).

The six physical cut equations and the full threshold catalogue are recorded in [the detailed assessment](/common/dev/gl638_integration_validation/GL638_CURRENT_PHYSICS_ASSESSMENT.md:11). For physical cut 1, edges `(2,6,10)`, the LU rescaling is
...
The earlier scratch H proposal already passed forward/inverse and independent finite-difference checks. It changed the measured hard-ray second-moment exponent from about -1 to about -0.54 but its 256-draw full-graph pilot showed no global gain. This separates demonstrated local density behavior from unestablished global efficiency; see [the earlier proposal evidence](../gl638/README.md).

## Exact A-star targeting with another one-block ellipsoid

Let c=(cp,cs) be the **actual selected native-two-loop common center**, and `RA=sqrt((Q/2)^2-m_t^2)`. At fixed original a0,t0,s0, tau, c and prepared s are fixed. Define v=s-cs and D=RA^2-|cs|^2>0. The A-star cone coordinate is
...
**Since ell is independent of p, H(Astar) is an affine pullback of the H ellipsoid.** Its prepared-p center is
...
Along the **whole native six-dimensional cone** `(p,s)=c+ell*(pstar-cp,sstar-cs)`, the projected star is independent of ell. Thus changing only that cone's overall radial sampling cannot concentrate on Hstar. A-star's own E-surface equation is zero by definition. This is why selecting a surface, its frame and the block to vary matters.

## Two-normal channel in the same p block

At the recorded hard intersection, the new arithmetic check gives `|grad H x grad Z|=1.73380575` and cosine 0.777009: these are regular independent normals in p-space. They are not independent *three-dimensional subspaces*.
...
The same (h,z,phi) chart can be used for pstar in the A pullback. With s held fixed, returning to p is affine and adds ell^3. Alternatively, a complete star-first A cone chart has Jacobian `ell^5 RA^2(RA-cs.n)`, followed by tau^(-6) for the two active momenta. [Exact star-cone formula](TWO_NORMAL_PROPOSAL.md).
...
The full H ellipsoid fits strictly inside this pstar ball because of the Higgs mass gap. The coarea/cone Jacobian and boundary cusp need to be retained; [the prior U-star audit](/common/dev/gl638_integration_validation/twenty-core/a-star-pullback/U_STAR_PROPOSAL.md:5) gives details. U-star coverage is motivated by WH appearing there; current maxima do not prove U dominance.

## LU and product-channel traps

For this e+e- CM graph, globally scaling the original input by lambda changes tau to tau/lambda; prepared `tau*K` is unchanged. New arithmetic checks for lambda=.07,.3,2,17 reproduced the same physical cut-1 point within 1.14e-13 GeV. The actual owner [rescaling all loop momenta](../../../crates/gammalooprs/src/integrands/process/cross_section/mod.rs) and [preparing all cut points before group geometry](../../../crates/gammalooprs/src/integrands/process/cross_section/mod.rs) are consistent with this reasoning.

Consequently, placing more raw hyperradial samples at a cut's own shell may help match the auxiliary LU localization, but does not by itself change the sampled physical event shape or tackle the H/Z shape corner. A sampler targeting a threshold *after* LU must either vary a cut-preserving block with fixed tau, as above, or explicitly pull back the threshold equation and use its real shape derivatives. It must not replace the physical projected residual by an unprojected one for convenience.
...
| Check | Why it matters for GL638 |
|---|---|
...
| Analytic ellipsoid volume at fixed complements | For H, transverse semiaxis is `a_perp=sqrt(B^2-|a|^2-4m^2)/2`, longitudinal semiaxis `a_parallel=a_perp/sqrt(1-|a|^2/B^2)`; volume `4pi a_perp^2 a_parallel/3`. Raw H volume is divided by tau^3; A-star H pullback volume gets an additional ell^3. Normalize these regions to unit volume for an exact regression. |
