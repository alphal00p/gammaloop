...
<what-soper-supplies-beyond-an-isotropic-shell-map>
#link("https://arxiv.org/pdf/hep-ph/0103262")[Soper (2001)] supplies several complementary improvements:

#figure(
  align(center)[#table(
    columns: 2,
    align: (auto,auto,),
    table.header([Location], [Improvement],),
    table.hline(),
...
<generic-extension-proposed-for-gammaloop>
The prior proposal already includes conditional contexts, joint surface normals, multiple patches and inverse density evaluation. It should additionally make #strong[tangential targets and normal widths conditioned on tangential distance] explicit first-class ingredients. Uniform angular sampling, or an ordinary adaptive grid on unmodified angles, cannot guarantee good weights in a narrowing anisotropic region.

A generic local construction is:

+ Resolve a target surface and a distinguished soft, collinear, intersection or near-tangency locus from the actual graph energy equations and routing. These targets are geometric, so they apply to amplitudes and cross sections alike.
+ Choose a regular chart `(delta, z, y)`, with surface-normal coordinate `delta`, tangential coordinates `z`, and complement `y`. More than one normal is permitted. Rank loss requires another patch or a dedicated degenerate chart.
+ Sample the complement, then tangential coordinates, then the normal. The exact conditional density is `q(y) q(z|y) q(delta|z,y)` with the chart determinant. Angularly dependent widths do not generate an omitted Jacobian: the ordered conditional map is triangular, provided that this dependency order is obeyed.
+ Supply normalized normal profiles whose widths can depend on distances measured in `z`, finite gaps, and relevant scales. Exponents and width laws are configurable and can be tuned from the actual subtracted integrand.
+ Add full-support components and a declared fallback where a regular surface fiber disappears. Retain explicit forward/inverse branch accounting and the same raw-space density in the multichannel partition.
