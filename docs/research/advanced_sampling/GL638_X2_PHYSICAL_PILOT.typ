...
<gl638-x2-direct-h-physical-pilot>
The generic conditional H chart passes normalization, native inverse-density and physical replay checks with the full #strong[936-orientation] GL638 state. This small untrained pilot gives encouraging imaginary-tail results for power 2, but does #strong[not] establish a reliable global efficiency gain: real errors are essentially unchanged against the matched power-1 chart, the mixed catalogue does not improve, and rare excursions dominate several estimates. The direct-H proposal remains unbounded on the measured H/Z rays. It is neither the joint two-normal H/Z chart nor an affine-star pullback.

All results use frozen source `f2f64fb17dd5dab588eca298dc61f1100fa4962d`, optimized CLI SHA256 `a37b5b27e1fde706e585be9f9c5e10894152cb02aab962ef13ff2dd751f0ee8f`. Build/profile and exact library fingerprints are in the #link("GL638_X2_PHYSICAL_PILOT.json")[machine artifact];. The #link("GL638_CHECKPOINT06D_BASELINE.json")[fresh checkpoint state];, all six physical cuts, original threshold metadata, 3D local UV and integrated UV are retained. State hashes are unchanged. Unlike the earlier reference-only result, the raw replays and integrations here evaluate the complete physical orientation sum.
...
#figure(
  align(center)[#table(
    ...
    table.header([Fixed direction], [beta\_F], [beta\_q], [beta\_W],),
    table.hline(),
...
The JSON also retains R=0.2 GeV and four-point fits. Thus the remaining approximately `R^-1/2` weight is measured using the #strong[actual full physical sum];. At R=0.0002 GeV, the two rays have complex weight magnitudes `|F/q|=229.77` and `264.94 pb`, much larger than the random pilot\'s extrema below. The pilot has not explored this rare tail adequately. These two fixed-angle directions do not establish global variance or control tangencies, soft limits or affine-star images.

== Fixed-budget pilot
<fixed-budget-pilot>
Each proposal uses seeds 1337, 7331 and 424242, 2,048 draws per seed, one fresh untrained iteration and 20 cores: #strong[24,576 physical draws];. All twelve runs finish with zero final NaN/unstable samples and no Arb rescues. The one loaded CLI batch takes #strong[565.70 seconds];, including setup and settings changes, excluding the surrounding file-hash audits. No proposal was enlarged after seeing its result.

#figure(
  align(center)[#table(
    columns: 6,
    align: (auto,auto,right,right,right,right,),
    table.header([Proposal], [Phase], [Pooled signed estimate ± SE \[pb\]], [Pooled absolute estimate ± SE \[pb\]], [Largest absolute weight \[pb\]], [Recorded total / map time \[ms per draw\]],),
    table.hline(),
...
Absolute monitors are `|Re I|` and `|Im I|` separately. For equal per-seed size n=2048, pooled mean mu and reported seed means mu\_i/errors e\_i, the empirical error shown is
