...
The before build is 1f83233dd79b5c7b8b09aef38ad5acdd5fde932c. The after build is optimized build 3, with the exact base/patch/build records retained in comparison\_1f\_certificate.json. Both use the actual configured ordinary stability stack, zero recorded previous maximum and the same 320 representative source draws repeated three times per mode. Forced-Arb diagnostic controls do not enter these timing denominators.

#figure(
  align(center)[#table(
    columns: 6,
    align: (auto,right,right,right,right,right,),
    table.header([Mode and set], [Before S/P], [After S/P], [Before upper/P], [After upper/P], [After invalid/count],),
    table.hline(),
    [optimized\_lmb: all\_representative], [3.69%], [4.16%], [4.32%], [4.59%], [0/960],
    [optimized\_lmb: valid\_representative\_only\_diagnostic], [3.69%], [4.16%], [4.32%], [4.59%], [0/960],
    [optimized\_lmb: selected\_hard], [2.32%], [2.41%], [3.89%], [4.76%], [0/180],
    [optimized\_lmb: bare\_hard\_control], [0.03%], [0.02%], [1.77%], [1.00%], [0/180],
    [optimized\_lmb: representative\_maxima], [6.14%], [6.78%], [6.42%], [7.05%], [0/60],
    [joint\_hz\_plus\_lmb: all\_representative], [39.48%], [35.37%], [39.76%], [35.67%], [3/960],
    [joint\_hz\_plus\_lmb: valid\_representative\_only\_diagnostic], [45.95%], [41.45%], [46.27%], [41.79%], [0/957],
    [joint\_hz\_plus\_lmb: selected\_hard], [18.08%], [16.86%], [18.33%], [17.05%], [0/180],
    [joint\_hz\_plus\_lmb: bare\_hard\_control], [0.03%], [0.02%], [0.22%], [0.21%], [0/180],
    [joint\_hz\_plus\_lmb: representative\_maxima], [47.51%], [47.77%], [47.82%], [48.10%], [0/60],
  )]
  , kind: table
  )
...
#figure(
  align(center)[#table(
    ...
    table.header([Joint selected hard point], [Before upper/P], [After upper/P], [After mean S, ms],),
    table.hline(),
    [hz\_00\_stack], [44.39%], [45.36%], [18.790],
    [hz\_03\_stack], [43.33%], [44.76%], [18.836],
    [soft22\_ct\_on\_stack], [8.19%], [7.20%], [16.843],
  )]
  , kind: table
  )
...
#figure(
  align(center)[#table(
    ...
    table.header([Joint set], [Before p99/max upper/P], [After p99/max upper/P],),
    table.hline(),
    [all\_representative], [159.32% / 199.04%], [146.78% / 159.36%],
    [selected\_hard], [73.86% / 81.20%], [54.23% / 54.93%],
    [representative\_maxima], [91.34% / 91.34%], [79.21% / 79.21%],
  )]
  , kind: table
  )
...
Inputs: results-performance-timing1 and results-certificate-timing1. Exact hashes, call counts, per-channel means, per-point ratios and unchanged invalid-source identities are retained in #link("comparison_1f_certificate.json");.
