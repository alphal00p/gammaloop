...
getcontext().prec=380
folder=Path(sys.argv[1]); out=Path(sys.argv[2]) if len(sys.argv)>2 else folder
budgets={'Double':D('1e-6'),'Quad':D('1e-10'),'Arb':D('1e-12')}
eps={'Double':D(2)**-52,'Quad':D(2)**-112,'Arb':D(2)**-999}
def num(v):
