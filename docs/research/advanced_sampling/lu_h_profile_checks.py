#!/usr/bin/env python3
"""Dependency-free quadrature checks for LU_H_MATCHED_SAMPLING.typ.
...
def profile(family, power):
    ...
    density = lambda y: (
        shape / (4 * math.exp(y) * math.cosh(shape * (y - mode) / 2) ** 2)
    )
    return (
        h,
        density,
        {"log_median": mode, "shape": shape, "h_normalization": normalization},
    )
...
def main():
    ...
    comparisons = [
        row("h_matched", h, h),
        row("log_logistic", h, density, **parameters),
    ]
    ...
    for family in ["poly_exponential", "poly_left_right_exponential"]:
        for power in powers:
            ...
            families.append(
                row("log_logistic", hi, qi, family=family, power=power, **params)
            )
