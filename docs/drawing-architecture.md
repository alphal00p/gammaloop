...
For the user-facing import contract, see [GammaLoop DOT input format](dot-input.md).
...
-  ext1 [style=invis];
  ext [style=invis];
  ext -> v0:0 [is_cut=0];
  v0 -> v1 [id=1, particle="d", lmb_id=0];
  v1 -> ext [particle="a"];
 }
 ```
 ...
 - An edge from a real node to an invisible node becomes an outgoing dangling
   half-edge at the real node.
- Reuse one `ext [style=invis]` node for all external legs; each connection
  creates a separate dangling half-edge. Keep the shared node attribute-free
  except for `style=invis`; dangling-node attributes are copied to every
  external edge.
 - An edge between two invisible nodes is invalid.
- Numeric half-edge ports and edge `id` attributes are optional and independent.
  Assign only the indices that matter; the parser assigns omitted indices.
