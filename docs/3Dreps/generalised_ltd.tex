...
branch-local Cutkosky residue bridge.  A cut with \(n\) simultaneously on-shell
propagators contributes the generic parity \((-1)^{n-1}\) when the branch-local
dual residue is mapped back to GammaLoop's simultaneous Cutkosky residue
normalization.  Source-side cut-edge orientation is already encoded in the
generated \(E\)-surface convention and in the sign accumulated when that
surface is canonicalized; multiplying it again in the Cutkosky bridge would
double-count reversed initial-state cut edges.  The resulting sign is stored
on the generated Cutkosky \(E\)-surface id before raised-cut alternatives are
merged, so different alternatives in the same raised group can keep their
correct residue orientation.  The remaining representation bridges are global
convention factors: the LTD LU loop-measure parity \((-1)^{L-1}\), with \(L\)
the forward-scattering loop count after removing initial-state cut edges, and
the full-source versus reduced-source CFF sign exponent used for
expanded-four-dimensional UV counterterms.  None of these signs depends on a
graph label or on a fitted sign from tests.
