...
normalization.  This is multiplied by the product of source-side cut-edge
orientation signs that converts the graph edge flow to the left-to-right
positive-energy Cutkosky convention; in the current graph convention a
default-oriented cut edge is opposite to that positive-energy flow and a
reversed cut edge is aligned with it.  The resulting sign is stored on the
generated Cutkosky \(E\)-surface id before raised-cut alternatives are merged,
so different alternatives in the same raised group can keep their correct
residue orientation.  The remaining representation bridges are global
...
the simultaneous-residue and cut-edge-flow parity described above.  CFF
expanded-source UV terms instead receive the difference between the full-graph
and reduced-source generalized-CFF sign exponents; this accounts for
duplicate-signature excess lost or gained when a local UV source is projected
out of the original graph.  These signs are representation-convention factors,
not graph-name patches.
