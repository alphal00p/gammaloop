...
The production dispatcher uses one repeated-channel construction for every
true repeated denominator channel.  Thus a requested \texttt{CFF}
representation with repeated denominators is no longer forced through an
independent repeated-pole CFF formula: it is built by the same derivative-free
confluent repeated-channel normal form used by the LTD branch, with the
requested CFF representation retained in the source metadata.  This is a
representation choice, not a sign convention.  Non-repeated CFF sectors still
use the ordinary CFF residue formula, and generalized bounded non-repeated
sectors use the finite-pole completions described below.
...
For non-confluent CFF sectors the implementation exposes the following
higher-power cases:
\begin{enumerate}[leftmargin=2em]
  \item arbitrary quadratic-or-lower caps \(d_e\le2\), on one-loop,
        multiloop, and split-mass graphs, provided the loop-energy UV check is
        satisfied;
  ...
  \item multi-active or initial-state-cut bounded sectors without duplicate
        energy channels, using the tensor-product interpolation
        \eqref{eq:multi-active-cff-interpolation}.
\end{enumerate}
In these accepted CFF cases every denominator tree is built only from
...
multiplies them in the numerator.  If the graph contains a true repeated
denominator channel, the dispatcher instead enters the shared confluent
repeated-channel builder described in Sec.~1.2.2.

\subsection{Tensor-Product Bounded CFF Interpolation}

Some bounded CFF graphs have more than one active edge-energy bound, or have
initial-state cut edges that make the lower-contact recursion depend on several
sampled edge energies at once.  Let
\[
  A_d=\{e\mid d_e>0\},\qquad
  X_e=\Samples_{d_e}=(1,-1,0,2,-2,\ldots)\big|_{d_e+1\ {\rm nodes}}
\]
be the active bounded edges and their deterministic interpolation nodes.  The
black-box numerator is reconstructed as
\begin{equation}
  N(\{q_e^0\})
  =
  \sum_{x\in\prod_{e\in A_d}X_e}
  N\!\left(q_e^0=x_e\omega_e\ \hbox{for }e\in A_d\right)
  \prod_{e\in A_d}
  \prod_{\substack{y\in X_e\\y\ne x_e}}
  \frac{q_e^0-y\omega_e}{(x_e-y)\omega_e}.
  \label{eq:multi-active-cff-interpolation}
\end{equation}
Each factor \(\omega_e^{-1}\) is serialized as one ordinary half-edge
\((2\omega_e)^{-1}\), and the explicit coefficient contributes
\(2/(x_e-y)\).  The linear numerator factor \(q_e^0-y\omega_e\) is then passed
to the same known-factor CFF recursion as any other numerator-side linear
factor.  Before that recursion starts, the consumed active bounds are cleared;
the tensor-product interpolation has already accounted for the complete
black-box degree.  This gives one generic path for initial-state-cut bounded
CFF and for bounded CFF with several active non-duplicate edge energies.
...
This channel normal form is an algebraic way of viewing repeated CFF powers.
The production dispatcher now uses the shared derivative-free confluent
repeated-channel builder whenever the requested CFF graph has a true repeated
denominator channel.  The ordinary finite-pole/contact recursion is still the
\(\nu=1\) limiting case: for one denominator copy,
\eqref{eq:channel-normal-form} is precisely the division of a single
higher-power edge into a pole remainder and a lower contact sector.
...
\(\Samples_4=(1,-1,0,2,-2)\).  In the algebraic pure-CFF channel normal form,
the first numerator map has orientation label \(\texttt{-+++++}\).  For its
sample \(a=1\), the variants are:
\[
...
canonical repeated energy \(y_C\) in that orientation.  This table is kept as
the channel-normal-form reference.  The production CFF path for true repeated
denominators uses the shared derivative-free repeated-channel expression, so a
bounded repeated CFF graph may intentionally serialize to the same expression
as the corresponding LTD construction.
...
Mathematically, repeated propagators can also be treated as repeated
denominator factors in the CFF contour integral.  The channel normal form above
performs the finite polynomial division at the level of the repeated
denominator power before CFF denominator trees are generated.  The production
implementation nevertheless chooses the shared derivative-free repeated-channel
construction for true repeated denominator channels, because it gives one
confluent limit for CFF and LTD and avoids a second repeated-pole sign path.
No graph-dependent sign or loop-number correction is introduced in this
dispatch.
...
\paragraph{Production representation names.}
The implementation-level family names above separate the mathematical building
blocks.  In gammaLoop user settings the production choices are only
\texttt{CFF} and \texttt{LTD}.  Both public paths use the derivative-free
repeated-channel construction for true repeated denominator channels, and
therefore agree with the split-mass LTD limit without differentiating the
numerator.  A repeated high-power CFF graph can consequently have the same
serialized expression as the LTD graph; the difference between CFF and LTD is
then only the requested production role and any auxiliary projection metadata
used by gammaLoop.  Ordinary non-repeated CFF still uses the pure CFF residue
formula.  \texttt{pure\_ltd} remains a diagnostic/testing object for the
3D-representation CLI and is not the production formula for unsplit repeated
propagators with non-trivial numerator dependence.  The feature-gated
\texttt{old\_cff} object is kept only inside the \texttt{three-dimensional-reps}
crate for legacy parity tests.

\paragraph{Uniform \(E\)-surface characterization for LU cuts.}
Local unitarity cut selection and threshold counterterm construction must not
depend on whether the original integrand is eventually evaluated as CFF or LTD.
GammaLoop therefore treats the causal \(E\)-surface groups as the
representation-neutral catalogue of physical threshold and Cutkosky cut
surfaces.  CFF expressions expose this catalogue directly.  LTD expressions may
contain additional \(H\)-surfaces in intermediate dual denominators, but after
the explicit sum over orientations those surfaces are spurious for the
threshold problem and are not used as threshold-counterterm surfaces.

For a forward-scattering graph, initial-state cut edges are first externalized
as correlated external energy shifts and external tree denominators are kept
four-dimensional.  GammaLoop then builds a companion CFF projection expression
with the same reduced graph data and uses its canonical \(E\)-surface groups to
identify raised LU cuts and left/right threshold residues.  The selected
residue data are mapped back to the requested representation before numerator
energy maps are localized and event weights are assembled.  This keeps CFF and
LTD aligned at the level of event groups, individual event weights, and
additional counterterm weights.

Canonicalization fixes the sign convention of a surface such as
\(-\omega_{i_1}-\cdots-\omega_{i_r}+\Delta\) by replacing it with
\(\omega_{i_1}+\cdots+\omega_{i_r}-\Delta\) and carrying the corresponding
residue orientation in the surface map.  The LTD projection also carries a
Cutkosky residue bridge for simple LU cuts.  GammaLoop determines this bridge
from the generated graph and surface data:
the selected denominator sign is the product of the positive-energy Cutkosky
edge-flow orientation and the determinant sign of the resolved LU coordinate
basis \((B_{\rm left},B_{\rm cut},B_{\rm right})\) in the chosen loop-momentum
basis.  Simple poles also carry the orientation of the full LU surface family,
defined as the product of the unit external-energy coefficients used to
parametrize the generated LU-cut \(E\)-surfaces.  This converts the local LTD
surface variables to GammaLoop's simultaneous positive-energy Cutkosky
variables without referring to graph labels or to the number of selected cut
groups as an independent rule.

Raised or confluent LU cuts do not use the simple-pole selected-variable
bridge.  Their selected local variable is kept in the generated LTD convention;
the remaining basis factor is carried once as a residue prefactor containing
the repeated-pole derivative parity and the product of the positive-energy
orientations of the collapsed Cutkosky alternatives.  The remaining
representation bridges are global convention factors: the LTD LU loop-measure
parity \((-1)^{L-1}\), with \(L\) the forward-scattering loop count after
removing initial-state cut edges, and the full-source versus reduced-source CFF
sign exponent used for expanded-four-dimensional UV counterterms.  None of
these signs depends on a graph label or on a fitted sign from tests.

There is one full-source bridge which is local to ordinary simple direct
residues rather than to a selected repeated pole.  If an active repeated
channel is not selected and all of its copies have the same canonical routing,
the LTD full-source residue still differs from the CFF projection convention by
the duplicate-collapse parity of that same-routing block.  Mixed-routing
channels do not use this bridge: their source orientation is already present in
the generated repeated-LTD routing, and selected repeated channels carry their
remaining orientation through the repeated-pole derivative and normalized
edge-support bridges above.
...
removing initial-state cut edges; its branch-local Cutkosky residue bridge is
the simultaneous-residue parity described above.  CFF expanded-source UV terms
instead receive the difference between the full-graph and reduced-source
generalized-CFF sign exponents; this accounts for duplicate-signature excess
lost or gained when a local UV source is projected out of the original graph.
LTD expanded-source UV terms do not receive this reduced-source CFF exponent:
their reduced source is evaluated directly in the generated dual convention,
and the full LTD measure bridge is applied once when the local UV forest term
is embedded back into the original graph.  These signs are
