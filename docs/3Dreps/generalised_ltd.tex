...
There is one sign convention distinction between the pure-CFF contact lift and
the LTD-style diagnostic finite-pole lift.  The extra one-edge pinch sign above
belongs to the CFF minor orientation convention: it is part of lifting a lower
CFF denominator tree back into the original CFF source/sink orientation.  If the
lower sector is instead generated as an LTD object, the contact quotient in
\eqref{eq:quadratic-contact} enters with its algebraic sign only.  Its
orientation is already carried by the LTD residue determinant and cut
half-edge factors, so reusing the pure-CFF pinch sign would double count a CFF
orientation convention in an LTD lower-sector comparison.
...
\texttt{CFF} and \texttt{LTD}.  The public \texttt{LTD} path uses the
derivative-free repeated-channel construction where needed, and therefore
agrees with the split-mass LTD limit without differentiating the numerator.
\texttt{pure\_ltd} remains a diagnostic/testing object for the 3D-representation
CLI and is not the production formula for unsplit repeated propagators with
non-trivial numerator dependence.  The feature-gated \texttt{old\_cff} object is
kept only inside the \texttt{three-dimensional-reps} crate for legacy parity
tests.

\paragraph{Diagnostic CFF/LTD comparison normal form.}
The production CFF representation and the diagnostic CFF/LTD comparison basis
are intentionally different objects for true repeated denominator channels.
Production CFF keeps the pure-CFF source basis and its channel/contact normal
form, because that basis is the local-UV object used by gammaLoop.  A diagnostic
comparison, however, should compare the two representations after projecting
both sides onto a common repeated-channel coordinate system.  The implementation
therefore uses a single comparison entry point: for non-repeated graphs it is
the ordinary representation generator, while for repeated CFF diagnostics it
returns the derivative-free confluent repeated-channel normal form also used by
the LTD-like comparison basis.

In that common projection the pure-CFF duplicate-signature excess factor is not
applied.  That sign is part of the independent pure-CFF contour/source
orientation convention.  It is required when the pure-CFF expression is used as
its own production basis, but it would be counted against the wrong basis in a
shared CFF/LTD diagnostic projection.  This separation is what allows repeated
high-power CFF/LTD diagnostics to be byte-for-byte or pointwise comparable
without changing gammaLoop's production CFF local-UV construction.
...
An edge marked as an initial-state cut is therefore not treated as an ordinary
internal denominator in the local coordinate problem.  It is a pair of
correlated external half-edge coordinates, one incoming and one outgoing, with
the same momentum.  If a generated LU \(E\)-surface and the omitted
positive-energy Cutkosky edge have identical unit derivatives with respect to
several such half-edge coordinates, those coordinates are equivalent local
charts for the same simple pole.  The implementation chooses one canonical
representative in the loop-momentum-basis external coordinate order; the
resolved LU coordinate determinant remains the source of the Jacobian sign.
...
The same separation applies to direct LTD original/root LU residues.  Those
terms are extracted from the requested LTD production expression, while the
companion CFF expression is only a surface-identification and canonicalization
catalogue.  Their local prefactor therefore contains the Cutkosky
local-coordinate orientation and any full-source repeated-channel source
bridge, but not the auxiliary CFF full-graph projection bridge.  Including that
projection bridge would count a CFF comparison convention inside an LTD
production residue; the LTD loop-measure bridge already accounts for the
representation-level measure convention when the branch is embedded.
