...
#let auto-root-node-corner-radius = 0.5
...
#let basketball-highlight-subgraphs(approx, given) = {
  ...
  if approx != none and approx != 0 {
    highlights.push((subgraph: approx, edge-style: (stroke: rgb("#3b82f6") + 1.45pt)))
  }
  if given != none and given != 0 {
    highlights.push((subgraph: given, edge-style: (stroke: rgb("#e4504f") + 1.65pt)))
  }
  highlights
}
...
#let label-scope = (
  ...
  S: S,
  T: T,
  expr: box(inset: -2mm, scale(42%, draw(
    g,
    unit: 1.0,
    node-label: none,
    node-style: basketball-node-style,
...
#let auto-root-cover(node) = {
  let fields = node.at("fields", default: (:))
  let statements = node.at("statements", default: (:))
  display-statement(
    node.at("cover", default: fields.at("cover", default: statements.at("cover", default: ""))),
    fallback: "",
  )
}

#let auto-root-node-style(node) = {
  let cover = auto-root-cover(node)
  (
    fill: if cover == "0" {
      rgb("#fff2b8")
    } else if cover == "GS" {
...
#let cover-size(cover) = {
  if cover == "" or cover == "0" {
    ...
    subgraph.hedges(subgraph.label(basketball, cover)).len()
  }
}

#let sorted-covers(covers) = {
  let sorted = ()
  let used = ()
  for _ in covers {
    let best = none
    let best-size = none
    for cover in covers {
      let size = cover-size(cover)
      if not used.contains(cover) and (best == none or size < best-size) {
        best = cover
        best-size = size
      }
    }
...
#let cover-rank-map(g) = {
  let covers = ()
  for node in graph.nodes(g) {
    let cover = auto-root-cover(node)
    if cover != "" and not covers.contains(cover) {
      covers.push(cover)
    }
  }
  let ranks = (:)
  for (rank, cover) in sorted-covers(covers).enumerate() {
    ranks.insert(cover, rank)
  }
  ranks
}

#let graph-with-auto-root-layout-hints(g) = {
  let ranks = cover-rank-map(g)
  graph.map(
    g,
    node: node => (statements: (layout-rank: ranks.at(auto-root-cover(node), default: 0))),
    edge: edge => (statements: ("source-route-exit": "east", "sink-route-exit": "west")),
  )
}
...
#let draw_auto_root_dot(doc) = {
  show raw: it => {
    ...
    for g in graph.parse(it.text) {
      ...
      let g = graph-with-auto-root-layout-hints(g)
