...
#{
  let xbox = {
    ...
    let edges = (
      ...
      D5: (spring-length: .2, crossing-under: <D6>, crossing-gap: 0.9)
        + mom(
          side: "right",
          shift: 1.5,
          label: (gap: .05),
  ...
  let xbox-opened2 = {
    ...
    let edges = (
      ...
      "D2.1": mom(side: "right", length: .7, shift: -.4, label: (
        gap: .2,
        shift: -.75,
        anchor: "south-west",
      )),
      ...
      D4: (spring-length: .1)
        + mom(side: "right", shift: -.4, label: (gap: .1)),
      D5: (spring-length: .5)
        + mom(
          side: "right",
          label: (gap: .05),
      ...
      "D6.0": (spring-length: .3, bend: -0.55)
        + mom(side: "left", length: 1.5, shift: .8),
  ...
  let xbox-cut = {
    ...
    let edges = (
      ...
      "D2.0": mom(side: "right", shift: .4, label: (
        anchor: "south-east",
        gap: .2,
      )),
      ...
      D5: (crossing-under: <D6.1>, crossing-gap: 1.5, spring-length: .2)
        + mom(side: "right", shift: -1., label: (gap: .01)),
      "D6.0": mom(side: "left", shift: .5, label: (gap: .1)),
      "D6.1": (spring-length: 2.)
        + mom(side: "left", shift: 3, label: (shift: 2.5, gap: .1)),
    ...
    let g = graph.map(
      ...
      edge: e => (
        (if e.origin.name != <D6> { flatten-boundary(e) })
          + mom(label: (gap: .4))
      ),
    )
    graph.map(g, node: nodes, edge: edges)
