...
fn canonicalization_keeps_explicit_dummies_distinct_from_compact_dot_indices() {
    ...
    for dots in 1..=2 {
        ...
        for reserved in [17, 1_000_000, 1_000_001] {
            ...
            let actual = input.canonize(AbstractIndex::Dummy).unwrap();
...
fn canonicalization_keeps_external_dummy_names_distinct_from_canonical_dummies() {
    ...
    for reserved in [0, 1] {
        ...
        let canonical = expression(17).canonize(AbstractIndex::Dummy).unwrap();
        assert_eq!(
            canonical,
            expression(18).canonize(AbstractIndex::Dummy).unwrap()
        );
        assert_eq!(canonical, canonical.canonize(AbstractIndex::Dummy).unwrap());
        assert_eq!(
...
fn canonicalization_ignores_indices_from_canceled_representation_groups() {
    ...
    for canceled in 0..2 {
        ...
        let canonical = input.canonize(AbstractIndex::Dummy).unwrap();
        assert_eq!(canonical, survivor.canonize(AbstractIndex::Dummy).unwrap());
        assert_eq!(canonical, canonical.canonize(AbstractIndex::Dummy).unwrap());
        assert!(
            canonical
