 ...
 use linnet::half_edge::subgraph::{subset::SubSet, SubSetLike};
 ...
 use super::{
     utils::{
         disable_direct_sum_contractions, distribute_smallest_expanded_sum_side, expression_size,
         is_sum, multiplicative_factors, product_excluding, trace_contraction_ordering,
         trace_direct_sum_term_expressions, trace_direct_sum_terms, trace_finish_contracts,
         trace_sum_contractions, TRACE_SCHOONSCHIP,
     },
     ...
     for (term_index, term) in terms.into_iter().enumerate() {
             ...
             factors.iter().zip(parsed_factors.iter()).enumerate()
         {
             ...
             match matched_pairs.as_slice() {
                 ...
                 [(first_pair_index, (_, first_target_slot)), (second_pair_index, (_, second_target_slot))]
                     if factor_tensor.is_metric && factor_tensor.structure.order() == 2 =>
                 {
