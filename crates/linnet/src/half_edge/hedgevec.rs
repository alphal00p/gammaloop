    ...
    fn set_flow_updates_dangling_and_paired_endpoints_consistently() {
        ...
        for flow in [Flow::Sink, Flow::Source] {
            ...
            for hedge in [dangling, source, sink] {
                assert_eq!(edges.data(hedge), unchanged.data(hedge));
                assert_eq!(edges.flow(hedge), unchanged.flow(hedge));
                assert_eq!(edges.orientation(hedge), unchanged.orientation(hedge));
                assert_eq!(edges.pair(hedge), unchanged.pair(hedge));
            }
        }
        for hedge in [dangling, source, sink] {
            assert_eq!(edges.data(hedge), original.data(hedge));
            assert_eq!(edges.flow(hedge), original.flow(hedge));
            assert_eq!(edges.orientation(hedge), original.orientation(hedge));
            assert_eq!(edges.pair(hedge), original.pair(hedge));
        }
    }
}
