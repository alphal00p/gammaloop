...
mod test {
        ...
        for forest_code in 0..720 {
            ...
            for a in 0..6 {
                for b in 0..6 {
                    ...
                    actual.swap(a, b);
                    oracle.swap(a, b);
                    assert_eq!(actual, ParentChildStore::from(oracle));
                    actual.validate().unwrap();

                    serial.swap(a, b);
                    serial_oracle.swap(a, b);
                    assert_eq!(serial, ParentChildStore::from(serial_oracle.clone()));
    ...
    fn deletion_matches_child_vec_for_every_subset() {
        ...
        for mask in 0..512 {
            ...
            assert_eq!(actual, oracle.cast::<ParentChildStore<usize>>());
