...
#[test]
#[ignore = "large UV scalar hotspot fixture; run under dev-optim with RUST_MIN_STACK=67108864"]
fn loads_uv_scalar_profile_hotspot_dump_and_simplifies_color() {
    ...
    let simplified = expression.simplify_color();
    eprintln!(
    ...
    assert_ne!(
        ...
        "color simplification unexpectedly left the UV hotspot fixture unchanged"
    );
    assert_eq!(
        simplified.simplify_color(),
        simplified,
        "color simplification did not reach a fixed point for the UV hotspot fixture"
    );
}
