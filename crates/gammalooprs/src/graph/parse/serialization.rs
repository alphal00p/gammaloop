                    ...
                    |_, _, p, e| {
                        if let HedgePair::Unpaired { flow, .. } = p {
                            e.map(|e| {
                                ...
                                match flow {
                                    Flow::Source => {
                                        dot.add_statement("pos", "x:@-left!".to_string());
                                    }
                                    Flow::Sink => {
                                        dot.add_statement("pos", "x:@+right!".to_string());
                                    }
                                }
                                dot
                            })
...
mod tests {
    ...
    fn generated_threshold_counterterms_are_exported_only_when_requested() {
        ...
        assert!(normalized.contains("threshold_counterterms = \"\nschema_version = 1\n"));
        assert!(!normalized.contains(r"schema_version = 1\n"));
    ...
    fn explicit_threshold_counterterms_round_trip_embedded_strings() {
        ...
        let dot_graph = DotGraph::from_string(&serialized).unwrap();
        let parse_graph =
            ParseGraph::from_parsed(dot_graph, &load_generic_model("scalars")).unwrap();
        let parse_graph_serialized = DotGraph::from(&parse_graph).debug_dot();
        assert!(
            parse_graph_serialized.contains("threshold_counterterms = \"\nschema_version = 1\n")
        );
        assert!(!parse_graph_serialized.contains(r"schema_version = 1\n"));

        let reparsed: Graph = parse_graph_serialized
            .into_graph(&load_generic_model("scalars"))
            .unwrap();
        assert_eq!(reparsed.threshold_counterterms.value, spec);
    }
