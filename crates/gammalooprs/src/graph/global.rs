...
use crate::{
    ...
    graph::{GroupId, threshold_counterterms::ThresholdCountertermSpec},
};
...
pub struct ParseData {
    ...
    pub threshold_counterterms: Option<ThresholdCountertermSpec>,
}
...
impl ParseData {
    pub(crate) fn with_overall_factor(self, overall_factor: Atom) -> Self {
        ParseData {
            ...
            threshold_counterterms: self.threshold_counterterms,
        }
    }
...
impl TryFrom<linnet::parser::GlobalData> for ParseData {
    type Error = color_eyre::Report;

    fn try_from(value: linnet::parser::GlobalData) -> color_eyre::Result<Self> {
...
impl Graph {
    pub(crate) fn global_data(&self, include_autogenerated_fields: bool) -> GlobalData {
        ...
        // Preserve native implicit defaults on group members; serializing them as explicit
        // master-topology directives would change their meaning on reimport.
        if !self.threshold_counterterms.autogenerated
            || (include_autogenerated_fields && (self.group_id.is_none() || self.is_group_master))
        {
            let threshold_counterterms = self
...
impl ParseGraph {
    pub(crate) fn global_data(&self) -> GlobalData {
        ...
        if let Some(threshold_counterterms) = &self.global_data.threshold_counterterms {
            let threshold_counterterms = threshold_counterterms
                .to_toml()
                .expect("validated threshold_counterterms should serialize to TOML");
            g.statements.insert(
                "threshold_counterterms".to_string(),
