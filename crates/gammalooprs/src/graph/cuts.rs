...
pub struct ResidueSelector {
    ...
    /// LTD-only bridge for the direct local-series extraction path. That path
    /// expands the fully assembled production expression in the selected
    /// Cutkosky variable, so it does not need the additional full-graph
    /// projection convention carried by `ltd_lu_cut_residue_prefactor_sign`.
...
impl ResidueSelector {
    pub(crate) fn ltd_threshold_residue_prefactor_sign(&self) -> i64 {
        // Threshold residues are selected and localized with the same
        // canonical E-surface orientation used by the generated 3D expression.
        // The side-dependent threshold prefactors carry the radial orientation,
        // so no additional LTD-only parity is needed here.
        1
    }
    ...
    pub(crate) fn ltd_local_series_residue_prefactor_sign(&self) -> i64 {
        self.ltd_lu_cut_local_series_prefactor_sign * self.ltd_threshold_residue_prefactor_sign()
    }
}
