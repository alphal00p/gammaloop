...
mod tests {
    ...
    fn autogenerated_model_dod_counts_raw_ufo_momenta() {
        ...
        assert_eq!(model.get_vertex_rule("V_35").dod, 1);
        assert_eq!(model.get_vertex_rule("V_36").dod, 1);
    ...
    fn sm_lorentz_structures_use_the_scalar_and_vector_axial_basis() {
        ...
        for lorentz in &model.lorentz_structures {
            for projector in ["ProjM", "ProjP"] {
                let projector = parse_python_expression(&format!("UFO::{{}}::{projector}"))
                    .get_symbol()
                    .unwrap();
                assert!(
                    !lorentz.structure.contains_symbol(projector),
                    "{} retains a chiral projector: {}",
                    lorentz.name,
                    lorentz.structure
                );
            }
