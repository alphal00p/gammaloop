...
impl VertexRule {
    ...
    pub(crate) fn from_serializable_vertex_rule(
        ...
        let dod = lorentz_structures
            ...
            .and_then(|dods| dods.into_iter().max());
        VertexRule {
            name: vertex_rule.name.clone(),
            particles: vertex_rule
                .particles
...
impl Propagator {
    pub(crate) fn from_serializable_propagator(
        ...
        let dod = (&numerator / &denominator).all_dod().ok();
        Propagator {
            name: propagator.name.clone(),
...
impl LorentzStructure {
    ...
    pub(crate) fn dod(&self) -> Option<i32> {
        self.structure.all_dod().ok()
    }
}
...
mod tests {
    ...
    #[test]
    fn autogenerated_model_dod_counts_raw_ufo_momenta() {
        let model = load_generic_model("sm");

        assert_eq!(model.get_vertex_rule("V_35").dod, Some(1));
        assert_eq!(model.get_vertex_rule("V_36").dod, Some(1));
        assert_eq!(model.get_propagator_for_particle("ghG").dod, Some(-2));
        assert_eq!(model.get_propagator_for_particle("g").dod, Some(-2));
        assert_eq!(model.get_propagator_for_particle("d").dod, Some(-1));
    }
