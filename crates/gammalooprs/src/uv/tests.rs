...
use crate::integrands::process::OrientationProfileMode;
...
#[test]
fn scalars_profile_respects_graph_lmb_and_subset_filters() {
    let (mut amp, model) = build_uv_scalars_amplitude(UVgenerationSettings {
        generate_integrated: false,
        softct: false,
        add_marker: true,
...
fn four_d_renormalization_without_cff_preserves_source_energy_gate() {
    ...
    for orchestrator in [
        ...
        if let Some(expected) = &common_part {
            assert_eq!(&part.expression.collect_factors(), expected);
        } else {
            common_part = Some(part.expression.collect_factors());
        }
