...
impl Graph {
    pub(crate) fn uv_rescaled(
        ...
        // Free public mUV occurrences are left untouched here: with the
        ...
        // Free mUVI occurrences come from previously integrated CT coefficients.
        ...
        atomarg = atomarg
            .replace(GS.m_uv_int)
            .with(Atom::var(GS.m_uv_int) / GS.rescale);

        let tsquare = Atom::var(GS.rescale).pow(2);
        let m_uv_sq = Atom::var(GS.m_uv).pow(2);
        let m_uv_int_sq = Atom::var(GS.m_uv_int).pow(2);
        ...
        atomarg = atomarg
            ...
            .with(
                GS.den(
                    ...
                    Atom::var(W_.prop_) * &tsquare + m_uv_sq.clone() * &tsquare
                        - m_uv_int_sq.clone(),
                ) / &tsquare,
            )
