...
fn finalize_expanded_expression_atom(atom: Atom, source: &Expanded4DParsedSource) -> Result<Atom> {
    ...
    let loop_count = source.parsed.loop_names.len();
    let normalization =
        (-Atom::i()).pow(loop_count as i64) / (Atom::var(GS.pi) * 2).pow(3 * loop_count as i64);
    Ok((atom * normalization)
...
fn denominator_uses_uv_loop_basis(denominator: &ExpandedDenominator) -> bool {
    denominator.mass_squared == Atom::var(GS.m_uv_expansion).pow(2)
}
