...
impl<'a> OrientationProjection<'a> {
    ...
    pub(crate) fn cff_options(self) -> Result<Generate3DExpressionOptions> {
        match self.source {
            OrientationProjectionSource::Exact { options, .. } => Ok(options.clone()),
            OrientationProjectionSource::FourDOnly => Err(eyre!(
                "four-dimensional-only renormalization has no 3D projection options"
            )),
        }
    }
...
mod tests {
    ...
    fn expanded_4d_setting_does_not_change_the_empty_forest_root() -> Result<()> {
        ...
        assert_eq!(
            projected
                .final_integrand(&projected_graph)?
                .iter()
                .collect::<BTreeMap<_, _>>(),
            direct
                .final_integrand(&direct_graph)?
                .iter()
                .collect::<BTreeMap<_, _>>(),
            "the empty forest must preserve every finalized residue value and selector"
        );
        Ok(())
    }
    ...
    fn gl24_direct_3d_modes_preserve_orientation_selector_contracts() -> Result<()> {
        ...
        let check = |explicit_orientation_sum_only| -> Result<Atom> {
        ...
        let ordinary_integrand = check(false)?;
        let explicit_integrand = check(true)?;
        assert!(!ordinary_integrand.is_zero());
        assert!(!explicit_integrand.is_zero());
    ...
    fn direct_nested_scalar_banana_replays_complete_cff_at_depth_three() -> Result<()> {
        ...
        for depth in [2, 3] {
            ...
            let mut filter = graph.get_edge_subgraph(EdgeIndex(1));
            let mut spinneys = Vec::new();
            for edge in 2..=depth + 1 {
                filter = filter.union(&graph.get_edge_subgraph(EdgeIndex(edge)));
                let subgraph =
            ...
            let build = |mut route_graph: Graph, from_expanded_4d| -> Result<Vec<Atom>> {
                let settings = UVgenerationSettings {
                    generate_integrated: false,
                    local_uv_cts_from_expanded_4d_integrands: from_expanded_4d,
                    ..Default::default()
                ...
                for spinney in &spinneys {
                    let mut current = Approximation::new(spinney.clone());
                    current.simple_approx = Some(
                        parent
                            .simple_approx
                            .as_ref()
                            .expect("the parent approximation is initialized")
                            .dependent(current.spinney.subgraph.clone()),
                    );
                    current.compute_4d(
                        &route_graph,
                        (crate::utils::vakint()?, &vakint_settings),
                    ...
                    if let Local3DCts::Direct(direct) = local {
                        ...
                        assert!(
                            !value.contains_symbol(global_marker),
                            "the untouched global numerator stays outside every local Taylor operation"
                        );
                        let negated = (-local.clone()).map(|atom| Ok(atom.clone()))?;
                        assert!(
                            (negated.direct()?.branches()?.factorized_sum() + value)
                                .expand()
                                .is_zero(),
                            "mapping a negated forest must negate its complete materialized value"
                        );
                    }
                    let final_integrand = current
                        .final_integrand(&route_graph)?
                        .iter()
                        .find(|(index, _)| *index == CutCFFIndex::new_all_none())
                        ...
                        .clone();
                    assert!(
                        !final_integrand.contains_symbol(OrientationID::symbol()),
                        "explicit final assembly must have summed all selectors"
                    );
                    let without_global = final_integrand.replace(global_marker).with(1);
                    assert!(
                        !without_global.is_zero(),
                        "the subtraction fixture must be nonzero"
                    );
                    assert!(
                        (&final_integrand - Atom::var(global_marker) * without_global)
                            .expand()
                            .is_zero(),
                        "final assembly restores the outside global numerator exactly once"
                    );
                    finals.push(final_integrand);
                    parent = current;
                }
                Ok(finals)
            };

            let direct = build(graph.clone(), false)?;
            let projected = build(graph.clone(), true)?;
            ...
            let expressions = direct
                .into_iter()
                .zip(projected)
                .flat_map(|(direct, projected)| [normalize(direct), normalize(projected)])
                .collect::<Vec<_>>();
            ...
            let loop_moms: LoopMomenta<F<ArbPrec>> = [
                ...
                ThreeMomentum::new(rational(43, 100), rational(29, 100), rational(-53, 100)),
            ]
            .into_iter()
            .take(depth)
            .collect();
            ...
            for (stage, pair) in values.chunks_exact(2).enumerate() {
                ...
                assert!(
                    ...
                    "{label} must produce finite values: direct={direct:e}, projected={projected:e}"
                );
        ...
        for outer_label in outer_factors {
            ...
            let build_child = |mut route_graph: Graph,
             ...
             -> Result<BTreeMap<CutCFFIndex, Atom>> {
                ...
                // The complete Taylor value keeps the natural propagator
                // powers, factorized numerator, and physical edge owners.
