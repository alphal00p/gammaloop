...
pub(crate) fn select_lu_cut_residue_for_representation<O>(
    ...
    ltd_simple_lu_cut_esurface_signs: &[(EsurfaceID, i64)],
...
where
    ...
    usize: From<O>,
{
    if representation == ThreeDRepresentation::Ltd && lu_cut.max_occurence == 1 {
        // A simple Cutkosky cut is a simultaneous residue in independent
        // propagators, while LTD evaluates branch-local dual residues. The
        // (-1)^(n-1) bridge for an n-propagator cut is therefore applied only
        // to simple LU cuts. Raised/confluent LU residues are generated by the
        // repeated-pole LTD formula itself; applying this bridge to them again
        // would double-count the derivative residue convention.
