...
mod tests {
    ...
    fn generation_cache_obeys_byte_entry_and_access_order_limits() {
        ...
        assert_eq!(cache.retained_bytes(), 7);
        cache.clear();
    ...
    fn canonical_sunset_allocator_matches_native_small_oracle() -> Result<()> {
        ...
        for a in 0..=5 {
            for b in 0..=5 - a {
                for c in 0..=5 - a - b {
                    for d in 0..=5 - a - b - c {
                        let loads = vec![a, b, c, d, 5 - a - b - c - d];
                        ...
                        let rows =
                            three_dimensional_reps::generate_3d_expression(&parsed, &options)?
                                .expression
                                .orientations
                                .len();
                        inventory.insert(loads, rows);
                    }
                }
            }
        }
        assert_eq!(inventory.len(), 126);
        let minimum = *inventory.values().min().unwrap();
        assert_eq!(minimum, 62);
        assert_eq!(
            inventory.values().filter(|rows| **rows == minimum).count(),
            1
        );
        assert_eq!(inventory[&vec![1, 1, 1, 1, 1]], minimum);
        assert_eq!(inventory[&vec![5, 0, 0, 0, 0]], 346);
        assert!(
            inventory[&vec![0, 0, 1, 1, 3]] < inventory[&vec![0, 0, 1, 2, 2]],
            "maximum rank is not the native row objective"
        );
        assert_ne!(
            inventory[&vec![0, 0, 0, 2, 3]],
            inventory[&vec![2, 0, 0, 0, 3]],
            "ordered capacities are part of the exact cache key"
        );
        ...
        let affine = analyzer.plan_atom_assignment_proposals(&(&q0 * q1.pow(5)), &candidates)?;
        assert_eq!(affine.len(), 1);
        options.energy_degree_bounds = Some(affine[0].energy_degree_bounds().to_vec());
        assert_eq!(
            three_dimensional_reps::generate_3d_expression(&parsed, &options)?
                .expression
                .orientations
        ...
        let nonlinear = analyzer.plan_atom_assignment_proposals(&(&q0 * q1.pow(7)), &candidates)?;
        let mut rows = Vec::new();
        for plan in &nonlinear {
            options.energy_degree_bounds = Some(plan.energy_degree_bounds().to_vec());
            rows.push(
                three_dimensional_reps::generate_3d_expression(&parsed, &options)?
                    .expression
                    .orientations
        ...
        assert_eq!(rows, vec![136, 124]);
        let seen = nonlinear
            .iter()
            .map(|plan| plan.energy_degree_bounds().to_vec())
            .collect::<Vec<_>>();
        let third = nonlinear[1].placement_challenger(&seen)?.unwrap();
        options.energy_degree_bounds = Some(third.energy_degree_bounds().to_vec());
        assert_eq!(
            three_dimensional_reps::generate_3d_expression(&parsed, &options)?
                .expression
                .orientations
    ...
    fn bounded_exact_cff_dispatch_preserves_owner_contour() -> Result<()> {
        ...
        for dotted_owner in [0, 1] {
            ...
            for phase in ["fresh", "cached", "uncached", "disabled", "evicted"] {
                ...
                assert_eq!(
                    selected_plan.get_or_insert_with(|| plan.clone()),
                    &plan,
                    "retention must not change factor assignments in {phase}"
                );
                let rows = generated.expression.orientations.len();
                assert_eq!(
                    *selected_count.get_or_insert(rows),
                    rows,
