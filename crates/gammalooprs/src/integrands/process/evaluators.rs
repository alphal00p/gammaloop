...
type ParsingTensorMap<'a> = dyn Fn(
        MixedTensor<F<f64>, ShadowedStructure<Aind>>,
    ) -> Result<MixedTensor<F<f64>, ShadowedStructure<Aind>>>
    + 'a;
...
impl EvaluatorStack {
    ...
    fn preprocess_tensor<A: AtomCore>(
        ...
        let integer_norm = |atom: AtomView<'_>| -> Option<u64> {
            let AtomView::Num(number) = atom else {
                return None;
            };
            let parts = match number.get_coeff_view().to_owned() {
                symbolica::coefficient::Coefficient::Complex(value) => [value.re, value.im],
                symbolica::coefficient::Coefficient::Float(value) => {
                    [value.re.try_to_rational()?, value.im.try_to_rational()?]
                }
                _ => return None,
            };
            parts
                .into_iter()
                .try_fold(0u64, |norm, value| {
                    if !value.is_integer() {
                        return None;
                    }
                    norm.checked_add(value.numerator_ref().to_i64()?.unsigned_abs())
                })
                .filter(|norm| *norm <= 1u64 << f64::MANTISSA_DIGITS)
        };
        let constant_factor =
            |factor: AtomView<'_>| -> Result<Option<MixedTensor<F<f64>, ShadowedStructure<Aind>>>> {
                ...
                while let Some(atom) = pending.pop() {
                    match atom {
                        ...
                        AtomView::Num(_) if integer_norm(atom).is_some() => {}
                ...
                let mut bounds: HashMap<_, (usize, usize, u64)> = HashMap::new();
                    ...
                    .rev()
                {
                    ...
                    let (entries, bytes, norm) = match &constant.graph.graph[node] {
                        NetworkNode::Op(op @ (NetworkOp::Sum | NetworkOp::Product)) => {
                            children.into_iter().fold(
                                (1usize, 0usize, u64::from(matches!(op, NetworkOp::Product))),
                                |(entries, bytes, norm), child| {
                                    let (child_entries, child_bytes, child_norm) = bounds[&child];
                                    (
                                        ...
                                        if matches!(op, NetworkOp::Sum) {
                                            norm.saturating_add(child_norm)
                                        } else {
                                            norm.saturating_mul(child_norm.max(1))
                                        },
                                    )
                                },
                            )
                        ...
                        NetworkNode::Leaf(NetworkLeaf::Scalar(scalar)) => {
                            ...
                            let Some(norm) = integer_norm(scalar) else {
                                return Ok(None);
                            };
                            (
                                1,
                                scalar.get_byte_size() + std::mem::size_of::<Atom>(),
                                norm,
                            )
                        }
                        NetworkNode::Leaf(
                            ...
                            tensor.to_param();
                            let MixedTensor::Param(tensor) = tensor else {
                                unreachable!()
                            };
                            let mut bytes = std::mem::size_of::<Atom>();
                            let mut norm = 0u64;
                            for (_, value) in tensor.iter_flat() {
                                let Some(entry_norm) = integer_norm(value) else {
                                    return Ok(None);
                                };
                                norm = norm.saturating_add(entry_norm);
                                bytes =
                    ...
                    // Total component l1 norms bound every contraction sum, product,
                    // and partial reduction. Gaussian integers remain exact in f64
                    // below this limit; max(1) above prevents a zero operand from
                    // masking a large partial product evaluated before it.
                    if norm > 1u64 << f64::MANTISSA_DIGITS {
                        return Ok(None);
                    }
                    bounds.insert(node, (entries, bytes, norm));
                }
                ...
                let mut checked = tensor.clone();
                checked.to_param();
                let MixedTensor::Param(checked) = checked else {
                    unreachable!()
                };
                if checked
                    .iter_flat()
                    .any(|(_, value)| integer_norm(value).is_none())
                {
                    return Ok(None);
                }
                Ok(Some(tensor))
    ...
    fn preprocess_numerator_families<A: AtomCore>(
        ...
        let (shadow_symbol, component_symbol, coefficient_symbol) = loop {
        ...
        for (family_index, definition) in definitions.iter().enumerate() {
            ...
            let shadow = Self::preprocess_tensor(
                ...
                None,
                |net, scalar_aliases, term_index| {
                    let mut tensor = match net.result_tensor(TENSORLIB.read().unwrap().deref())? {
        ...
        let lower_shadow_components = |tensor: MixedTensor<F<f64>, ShadowedStructure<Aind>>| {
            let MixedTensor::Param(param) = tensor else {
                return Ok(tensor);
            };
            let spenso::tensors::parametric::ParamTensor {
                mut tensor,
                param_type,
            } = param;
            let tensor = {
                let data = spenso::tensors::data::StorageTensor::map_data_ref_mut_result(
                    &mut tensor,
                    |value| lower_components(std::mem::take(value)),
                )?;
                spenso::tensors::data::SparseOrDense::to_sparse(data)
            };
            Ok(MixedTensor::Param(
                spenso::tensors::parametric::ParamTensor { tensor, param_type },
            ))
        };
        let atoms = atoms
            .iter()
...
mod tests {
    ...
    fn shared_numerator_outer_projectors_prune_unreachable_component_dependencies() {
        ...
        assert!(generated.iter().any(|entry| entry.tags[0] == Atom::num(0)
            && entry.rhs.as_view().get_byte_size() >= NETWORK_SCALAR_ALIAS_MIN_BYTES));
        assert!(
            generated
                .iter()
                .filter(|entry| entry.tags[0] == Atom::num(1))
                .all(|entry| entry.rhs.is_one() && entry.args.is_empty())
        );
    ...
    fn shared_numerator_components_prune_only_unused_formals_through_aliases() {
        ...
        for entry in &prepared.reps {
            if entry.tags[0] == Atom::num(1) {
                assert!(entry.args.is_empty());
            } else {
                assert!(entry.args.iter().all(|arg| arg == &q1));
                if entry
                    .lhs
                    .as_fun_view()
                    .unwrap()
                    ...
                    .starts_with("gammalooprs::numerator_component_")
                {
                    assert_eq!(
                        entry.args,
                        vec![Indeterminate::try_from(q1.clone()).unwrap()]
                    );
                }
            }
        }
    ...
    fn evaluator_preprocess_constant_tensor_factors_preserve_numeric_domains() {
        ...
        for (a, b) in [
            ...
            // admitted integers and declined rational, large and symbolic data.
