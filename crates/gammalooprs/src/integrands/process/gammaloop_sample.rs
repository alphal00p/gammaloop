...
pub enum DiscreteGraphSample<T: FloatLike> {
    ...
    /// A point generated by the canonical compiled sampling-channel bridge.
    /// The bridge has already applied the map Jacobian and map-density
...
impl<T: FloatLike> DiscreteGraphSample<T> {
    ...
    fn rotate(
        ...
        match self {
            ...
            DiscreteGraphSample::Advanced { channel_id, sample } => DiscreteGraphSample::Advanced {
                channel_id: *channel_id,
                sample: sample.rotate(rotation, loop_mom_cache_id, external_mom_cache_id),
            },
        }
