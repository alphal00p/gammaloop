...
impl CrossSectionGraph {
    ...
    fn resolve_threshold_counterterm_directives(
        ...
        // An explicit group id is a user assertion about one solve space. Validate it after
        // parent-LMB rebasing so incompatible cuts can never silently share an SOCP center.
        let mut explicit_group_members = BTreeMap::<
            (ThresholdCountertermSide, usize),
            (
                CutGroupId,
                CutId,
                Vec<EdgeIndex>,
        ...
        for draft in &physical_drafts {
            for variant in &draft.variants {
                ...
                if let Some((
                    ...
                    old_name,
                    old_subspace,
                )) = explicit_group_members.get(&key)
                {
                    if old_subspace.solve_signature(all_lmbs)
                        != variant.subspace.solve_signature(all_lmbs)
                    {
                        return Err(eyre!(
                            "Graph '{}' explicit threshold group_id={} on {:?} contains incompatible solve subspaces. First member: cut_group={}, cut={}, variant='{}', cut_edges={:?}, threshold_edges={:?}, parent={:?}, basis={:?}; conflicting member: cut_group={}, cut={}, variant='{}', cut_edges={:?}, threshold_edges={:?}, parent={:?}, basis={:?}",
                            self.graph.name,
                            group_id,
                            draft.side,
                            old_cut_group.0,
                            ...
                            old_cut_edges,
                            old_threshold_edges,
                            old_subspace.parent_lmb_index(),
                            old_subspace.solve_signature(all_lmbs),
                            draft.cut_group_id.0,
                            draft.cut_id.0,
                            variant.name,
                            draft.cut_edges,
                            draft.threshold_edges,
                            variant.subspace.parent_lmb_index(),
                            variant.subspace.solve_signature(all_lmbs),
                        ));
                    }
                } else {
                    explicit_group_members.insert(
                        key,
                        (
                            draft.cut_group_id,
                            draft.cut_id,
                            draft.cut_edges.clone(),
                            draft.threshold_edges.clone(),
                            variant.name.clone(),
                            variant.subspace.clone(),
                        ),
                    );
                }
            }
