                ...
                for (mode, explicit_sum, projected) in [
                    ...
                    for phase in ["generated", "loaded", "selected"] {
                        if phase == "selected" {
                            ...
                            assert!(cross_section.integrand.is_some());
                            assert_eq!(cross_section.supergraphs.len(), 2);
                            assert_eq!(cross_section.graph_group_structure.len(), 2);
                            let retained = &cross_section.supergraphs[1];
                            assert_eq!(retained.graph.group_id, Some(GroupId(1)));
                            assert!(retained.graph.is_group_master);
                            let removed_name = graph_name.clone();
                            graph_name = retained.graph.name.clone();
                            assert_ne!(graph_name, removed_name);
                            let expected_cuts = bincode::encode_to_vec(
                                (
                                    &retained.cuts,
                                    &retained.derived_data.cut_group_data.cut_groups,
                                ),
                                bincode::config::standard(),
                            )?;
                            let mut expected_source = retained.graph.clone();
                            expected_source.group_id = Some(GroupId(0));
                            production_bytes = Some(bincode::encode_to_vec(
                                retained
                                    .derived_data
                            ...
                            assert!(cross_section.integrand.is_none());
                            assert_eq!(cross_section.supergraphs.len(), 1);
                            assert_eq!(cross_section.graph_group_structure.len(), 1);
                            assert_eq!(
                                (&cross_section.graph_group_structure[GroupId(0)])
                                    .into_iter()
                                    .collect::<Vec<_>>(),
                                vec![0],
                            );
                            ...
                            assert_eq!(
                                ...
                                "persistent selection changed the retained physical source",
                            );
                            assert_eq!(
                                bincode::encode_to_vec(
                                    retained
                                ...
                                *production_bytes.as_ref().unwrap(),
                                "persistent selection changed the retained production CFF",
                            );
                            assert!(
                            ...
                            assert_eq!(
                                bincode::encode_to_vec(
                                    (&term.cuts, &term.cut_group_data.cut_groups),
                                    bincode::config::standard(),
                                )?,
                                expected_cuts,
                        ...
                        // Persistence includes every exact map and component sign;
                        // transient source degree reports are intentionally excluded.
                        // Selection/regeneration must also retain the old graph 1 source.
                        let bytes =
                            bincode::encode_to_vec(production, bincode::config::standard())?;
                        if let Some(expected) = &production_bytes {
                            assert_eq!(
                                &bytes, expected,
                                "stored production CFF changed after {phase}"
                            );
                        } else {
                            production_bytes = Some(bytes);
                        }
                        ...
                        for forest in fs::read_dir(graph_dir.join(format!("{graph_name}_nodes")))? {
                            ...
                            for node in fs::read_dir(forest.path())? {
                                ...
                                node_indices
                                    .insert(file.split('_').nth(1).unwrap().parse::<usize>()?);
                                assert!(
                        ...
                        assert!(
                            node_indices.len() > 1,
                            "{folder}/{orchestrator}/{mode}: no nonempty computed node"
                        );
                        if phase == "generated" {
