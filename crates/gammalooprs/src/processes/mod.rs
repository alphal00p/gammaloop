...
pub struct DotExportSettings {
    ...
    /// Emit a `reduced_num` attribute: the full numerator reduced to master integrals via the
    /// `oneloop` IBP reducer. Forces gamma/color algebra on
    #[serde(default, skip_serializing_if = "is_false")]
    pub reduce: bool,
}
...
impl Default for DotExportSettings {
    fn default() -> Self {
        Self {
            ...
            with_uv: false,
            include_autogenerated_fields: false,
            reduce: false,
        }
    }
}
