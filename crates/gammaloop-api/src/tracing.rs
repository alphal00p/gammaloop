...
use eyre::Context;
use gammaloop_tracing_filter::{
    strip_ansi_escape_codes, GammaDisplayFormat, GammaLogFilter, LogFormat, LogSink, LogStyle,
    RoutedFieldsVisitor,
};
...
pub(crate) fn init_tracing(dir: impl AsRef<Path>, log_file_name: Option<String>) {
    ...
    FILTER_HANDLES.get_or_init(|| {
        ...
        let status_layer = fmt::layer()
            .with_target(false)
            .event_format(GammaDisplayFormat::dynamic(|| {
                LOG_STYLE_STATE.lock().unwrap().effective_style()
            }))
