...
mod test {
    ...
    #[test]
    fn split_command_list_preserves_escaped_quotes_and_semicolons() {
        let parts =
            split_command_list(r#"run block_a -c "display \"semi;quoted\""; display model"#)
                .unwrap();
        assert_eq!(
            parts,
            vec![
                r#"run block_a -c "display \"semi;quoted\"""#,
                "display model",
            ]
        );
    }
