...
pub struct IntegrandGraphInfo {
    ...
    pub name: String,
    pub is_master: bool,
    /// Compact, explicit graph directives, flattened only for display/API consumption.
    ///
    /// Autogenerated legacy associations are deliberately absent. An explicit threshold with no
    /// counterterm blocks contributes one `implicit_default` row because that shorthand is part of
    /// the graph document's semantics.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub threshold_counterterm_directives: Vec<IntegrandThresholdCountertermDirectiveInfo>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub threshold_counterterms: Option<ThresholdCountertermMetadataRegistry>,
}

#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct IntegrandThresholdCountertermDirectiveInfo {
...
impl IntegrandThresholdCountertermDirectiveInfo {
    fn explicit_for_graph(graph: &Graph) -> Vec<Self> {
        ...
        graph
            ...
            .flat_map(|cut| {
                cut.thresholds.iter().flat_map(move |threshold| {
                    ...
                    let variants = if threshold.counterterms.is_empty() {
                        std::slice::from_ref(&implicit_default)
                    } else {
                    ...
                    variants
                        .iter()
                        .map(move |variant| Self {
                            cut_edge_ids: cut.edges.iter().map(|edge| edge.0).collect(),
                            threshold_edge_ids: threshold.edges.iter().map(|edge| edge.0).collect(),
                            name: variant
                                .name
                                .clone()
                                .unwrap_or_else(|| "default".to_string()),
                            implicit_default: threshold.counterterms.is_empty(),
        ...
        .map(|(group_id, group)| {
            ...
            Ok(IntegrandGraphGroupInfo {
                ...
                graphs: group
                    .into_iter()
                    .map(|graph_id| {
                        let graph_term = &integrand.data.graph_terms[graph_id];
                        IntegrandGraphInfo {
                            graph_id,
                            name: graph_term.graph.name.clone(),
                            is_master: graph_id == master_graph_id,
                            threshold_counterterm_directives:
                                IntegrandThresholdCountertermDirectiveInfo::explicit_for_graph(
                                    &graph_term.graph,
                                ),
                            threshold_counterterms: graph_term
