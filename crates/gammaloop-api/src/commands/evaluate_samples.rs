...
use eyre::eyre;
use gammalooprs::{
    integrands::{process::MomentumSpaceEvaluationInput, HasIntegrand},
...
impl<'a> EvaluateSamples<'a> {
    pub fn run(&self, state: &mut State) -> Result<Vec<Complex<f64>>> {
        ...
        for sample_index in 0..batch_len {
            ...
            let evaluation = if self.momentum_space {
            ...
            results.push(evaluation.integrand_result.map(|component| component.0));
        }
...
fn evaluate_x_point(
    ...
    integrand.evaluate_sample(&sample, model, F(0.0), 1, use_arb_prec, Complex::new_zero())
}
