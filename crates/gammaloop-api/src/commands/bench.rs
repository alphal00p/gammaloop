...
use crate::{
    commands::evaluate_samples::{evaluate_samples, EvaluateSamples},
...
const BENCH_WARMUP_SAMPLES: usize = 10;
const BENCH_DEFAULT_BATCHES: usize = 10;
const BENCH_OTHER_RELATIVE_THRESHOLD: f64 = 5.0e-3;
const BENCH_OTHER_ABSOLUTE_THRESHOLD_SECONDS: f64 = 1.0e-9;
...
pub struct Bench {
    ...
    /// Fixed x-space or momentum-space point to benchmark
    #[arg(
        short = 'x',
        long = "point",
        required = true,
    ...
    /// Target benchmark duration
    #[arg(long, value_name = "DURATION")]
    pub duration: String,

    /// Evaluate in arbitrary precision
    #[arg(short = 'f', long = "use-arb-prec")]
    pub use_arb_prec: bool,

    /// Interpret the point as momentum-space coordinates
    #[arg(short = 'm', long)]
    pub momentum_space: bool,

    /// Discrete dimensions of the fixed sample
    #[arg(
        short = 'd',
        long = "discrete-dim",
        value_name = "DIMS",
    ...
    /// Select a specific graph in momentum-space evaluation
    #[arg(long = "graph-id", value_name = "GRAPH_ID")]
    pub graph_id: Option<usize>,

    /// Select a specific orientation of the selected graph
    #[arg(
        long = "orientation-id",
    ...
    /// Number of batches used to estimate uncertainty
    #[arg(long = "n-batches", value_name = "N", value_parser = parse_positive_usize)]
    ...
    #[arg(long = "minimal-integrand")]
    #[serde(default)]
    pub minimal_integrand: bool,

    /// Write the benchmark result as pretty JSON to this path
...
struct BenchOutput {
    ...
    batches: Vec<BenchBatchTiming>,
    summary: Vec<BenchSummaryRow>,
}
...
impl Bench {
    fn validate_selector_mode(&self) -> Result<()> {
        if !self.momentum_space && (self.graph_id.is_some() || self.orientation_id.is_some()) {
            return Err(eyre!(
                "Graph and orientation selectors are only supported in momentum-space benchmarks."
            ));
        }
        Ok(())
    }
    ...
    fn displayed_result_from_evaluation(
        ...
        let displayed_result = if self.momentum_space {
            ...
            let jacobian = jacobian.ok_or_else(|| {
                eyre!("An x-space benchmark requires a parameterization jacobian, but none was returned")
            })?;
    ...
    fn write_json_output<T: Serialize>(&self, value: &T) -> Result<()> {
        ...
        fs::write(&path, serde_json::to_vec_pretty(value)?)
            .with_context(|| format!("Could not write benchmark JSON output {}", path.display()))?;
        info!(
            "Wrote benchmark JSON output to {}",
    ...
    fn run_benchmark(
        ...
        let target = parse_bench_target_duration(&self.duration)?;
        let n_batches = self.n_batches.unwrap_or(BENCH_DEFAULT_BATCHES);
        if n_batches == 0 {
            return Err(eyre!("bench requires at least one batch"));
        }

        let warmup_points = self.repeated_point_array(BENCH_WARMUP_SAMPLES)?;
        let warmup_discrete_dims = self.repeated_discrete_dim_array(BENCH_WARMUP_SAMPLES)?;
        ...
        let progress_bar = bench_progress_bar(n_batches);
        ...
        let batch_timings = (|| -> Result<Vec<_>> {
            ...
            for (batch_index, batch_size) in batch_sizes.into_iter().enumerate() {
                ...
                batch_timings.push(bench_batch_timing(total_timing, &batch)?);
                update_bench_progress(
                    &progress_bar,
                    batch_index + 1,
                    n_batches,
                    &batch_timings,
        ...
        let batch_timings = batch_timings?;

        let warmup_evaluation = warmup
            .samples
            .first()
            .ok_or_else(|| eyre!("bench warmup returned no sample"))?;
        ...
        let bench_output = BenchOutput {
            ...
            summary: bench_summary_rows(&batch_timings),
        };
        ...
        info!(
            "\n{}\n{}",
            format!(
                "Benchmark for integrand '{}' at fixed input point: target {}, warmup {} sample(s), profiled {} sample(s) in {} batch(es).",
            ...
            render_bench_table(&batch_timings)
        );
    ...
    pub fn run(&self, state: &mut State) -> Result<()> {
        ...
        self.with_optional_minimal_integrand_settings(
            state,
            process_id,
            &integrand_name_for_restore,
            |state| self.run_benchmark(state, process_id, integrand_name, graph_name),
        )
        .map(drop)
    }
...
fn parse_bench_target_duration(input: &str) -> Result<Duration> {
    ...
    if trimmed.is_empty() {
        return Err(eyre!("bench requires a non-empty duration"));
    }
    ...
    let seconds = number
        .trim()
        .parse::<f64>()
        .with_context(|| format!("Could not parse bench duration '{input}'"))?
        * multiplier;
    if !seconds.is_finite() || seconds <= 0.0 {
        return Err(eyre!("bench duration must be positive and finite"));
    }
    Duration::try_from_secs_f64(seconds)
        .map_err(|_| eyre!("bench duration is too large to represent"))
}
...
fn bench_summary_rows(batch_timings: &[BenchBatchTiming]) -> Vec<BenchSummaryRow> {
    let rows = bench_rows(batch_timings);
...
fn bench_progress_bar(n_batches: usize) -> ProgressBar {
    let bar = if io::stderr().is_terminal() {
        ProgressBar::new(n_batches as u64)
    } else {
        ProgressBar::hidden()
    };
...
fn update_bench_progress(
    progress_bar: &ProgressBar,
    completed_batches: usize,
    n_batches: usize,
    batch_timings: &[BenchBatchTiming],
    total_samples: usize,
) {
    let total_values = batch_timings
        .iter()
        ...
        .collect::<Vec<_>>();
    let (total_mean, total_standard_error) = mean_and_standard_error(&total_values);
    let projected_total = Duration::try_from_secs_f64((total_mean * total_samples as f64).max(0.0))
...
fn bench_rows(batch_timings: &[BenchBatchTiming]) -> Vec<BenchRow> {
    ...
    if other_mean
        > (total_mean * BENCH_OTHER_RELATIVE_THRESHOLD).max(BENCH_OTHER_ABSOLUTE_THRESHOLD_SECONDS)
    {
        rows.push(BenchRow {
...
fn render_bench_table(batch_timings: &[BenchBatchTiming]) -> String {
    let rows = bench_rows(batch_timings);
    let total_values = batch_timings
        .iter()
        .map(|timing| timing.total)
...
mod tests {
    ...
    use super::{
        bench_batch_timing, format_timing_with_uncertainty, parse_bench_target_duration,
        split_samples_into_batches,
    };

    #[test]
    fn bench_duration_parser_accepts_common_units() {
        assert_eq!(
        ...
        assert!(parse_bench_target_duration("1e300").is_err());
    }

    #[test]
    fn bench_batches_cover_all_samples() {
        let batches = split_samples_into_batches(23, 10);
        assert_eq!(batches.len(), 10);
        assert_eq!(batches.iter().sum::<usize>(), 23);
        assert!(batches.iter().all(|size| (2..=3).contains(size)));
    }

    #[test]
    fn bench_timing_formatter_uses_shared_unit() {
        assert_eq!(
    ...
    #[test]
    fn bench_table_includes_total_row_after_separator() {
