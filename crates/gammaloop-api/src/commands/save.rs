...
fn selected_export_root(
    path: Option<PathBuf>,
    global_settings: &CLISettings,
    read_only_base_name: &str,
) -> PathBuf {
    path.unwrap_or_else(|| global_settings.default_active_state_output_path(read_only_base_name))
}
    ...
    pub fn run(
        ...
        match self {
            Save::Dot {
                ...
                let target_dir = selected_export_root(path, global_settings, "dot_files");
                global_settings
            ...
            Save::Standalone {
                ...
                let target_dir = selected_export_root(path, global_settings, "standalone");
...
#[cfg(test)]
mod tests {
    use super::{selected_export_root, Save, SaveState};
    use crate::{
    ...
    #[test]
    fn save_dot_defaults_to_cwd_target_in_read_only_mode() {
        let mut cli_settings = CLISettings::default();
        cli_settings.state.folder = PathBuf::from("/tmp/read_only_state");
        cli_settings.state.name = Some("demo state".to_string());
        cli_settings.session.read_only_state = true;

        let target = selected_export_root(None, &cli_settings, "dot_files");

        assert_eq!(target, PathBuf::from("./dot_files_demo_state"));
    }
