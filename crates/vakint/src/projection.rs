...
impl ScalarTerms {
    ...
    pub(crate) fn epsilon_pole_order(
        ...
        // Absolute order zero includes every pole without searching for a
        // nonzero leading term in an exactly cancelling coefficient. Retain
        // the complete coefficient for the independent final Laurent series.
...
mod tests {
    ...
    fn whole_numerical_kernel_preserves_exact_complex_coefficients() {
        ...
        let (kernel, _, common_power) = terms
    ...
    fn projected_zero_external_sector_preserves_the_nonzero_spectator() {
        ...
        let result = VacuumProjection::new(&settings, std::slice::from_ref(&input))
            .project(input.as_view(), |_| Ok(Atom::zero()))
            .unwrap();
    ...
    fn scalar_zero_certificate_handles_cycles_and_accumulated_cancellation() {
        ...
        assert!(terms.expression().is_zero());
    ...
    #[test]
    fn projected_sums_preserve_the_complete_factorized_spectator() {
        let settings = VakintSettings::default();
        ...
        assert_eq!(
            result.expression(),
            &spectator
                * atom("dot(p(1),p(3))+dot(p(1),p(4))+dot(p(2),p(3))+dot(p(2),p(4))")
                * atom("dot(k(1),k(1))")
                / &projection.dimension,
        );
    ...
    fn closed_spin_tensor_projection_preserves_coefficients_up_to_dummy_renaming() {
        ...
        assert!(
            LorentzTensor::parse(expression.as_view(), &projection.dimension)
                .unwrap()
                .is_scalar()
        );
        // A closed contraction may retain either original dummy or a projector
        // slot. Compare the complete tensor expression after alpha-renaming.
        ...
        assert_eq!(
            restored,
            &spectator
                * atom("tensor(gamma(mink(4,fresh_dummy)),mink(4,fresh_dummy))^2*dot(k(1),k(1))")
                / &projection.dimension
        );
    ...
    #[test]
    fn invariant_powers_preserve_the_projected_tensor_value() {
        let settings = VakintSettings::default();
    ...
    #[test]
    fn scalar_sectors_preserve_complete_values_and_external_products() {
        let input = atom("(a+b)*(c+d)*(dot(k(1),k(2))+dot(k(1),k(1)))");
        let terms = ScalarTerms::parse(input.as_view()).unwrap();
        assert_eq!(
            terms.expression(),
            atom("(a+b)*(c+d)*dot(k(1),k(2))+(a+b)*(c+d)*dot(k(1),k(1))")
        );
        ...
        assert_eq!(
            terms.expression(),
            &external * atom("dot(k(1),p(1))") + &external * atom("dot(k(1),k(2))")
        );
    }
    ...
    fn projection_is_idempotent_for_internal_scalar_products() {
        ...
        let mut projection = VacuumProjection::new(&settings, std::slice::from_ref(&input));
        let result = projection
            .project(input.as_view(), |_| Ok(Atom::one()))
            .unwrap();
    ...
    fn rational_epsilon_series_uses_existing_atom_coefficient_ring() {
        ...
        // Compare the complete Laurent value: scalar ASTs remain products of
        // sums, while epsilon alone is series-expanded.
        assert_eq!(result.to_atom(), atom("(a+b)*(c+d)/(4*eps)+(a+b)*(c+d)/8"));
    }
    ...
    fn rank_four_kernel_has_the_fixed_minkowski_0011_component() {
        ...
        let result = projection
            .project(input.as_view(), |_| Ok(kernel.clone()))
            .unwrap();
        let mut expression = result.expression();
        ...
        assert_eq!(expression, atom("-dot(k(1),k(1))^2/24"));
    }
    ...
    fn mixed_rank_two_two_has_both_fixed_minkowski_components() {
        ...
        for (values, expected) in [
            ...
            assert_eq!(
                expression,
                atom("dot(k(1),k(1))*dot(k(2),k(2))") * atom(expected[0])
                    + atom("dot(k(1),k(2))^2") * atom(expected[1])
            );
        }
    }
    ...
    #[test]
    fn epsilon_pole_query_includes_nested_poles_and_regular_coefficients() {
        let settings = VakintSettings {
            epsilon_symbol: "eps".into(),
            ..VakintSettings::default()
        };
        for (input, expected) in [
            ...
            assert_eq!(
                ScalarTerms::epsilon_pole_order(atom(input).as_view(), &settings).unwrap(),
                expected,
                "{input}"
            );
        }
        for input in ["f(eps)", "log(f(eps))", "log(eps)", "eps^(-1/2)"] {
            assert!(
                ScalarTerms::epsilon_pole_order(atom(input).as_view(), &settings).is_err(),
                "{input}"
            );
        }
    }
    ...
    #[test]
    fn bounded_epsilon_zero_preserves_complete_coefficients_for_final_series() {
        let settings = VakintSettings {
            epsilon_symbol: "eps".into(),
            ..VakintSettings::default()
        };
        let a = Atom::i() * atom("8*GC_10*GC_11^3");
        let dimension = atom("-4+2*eps");
        let zero = &a
            + Atom::num(4) * &a / &dimension
            + (-Atom::num(4) * &a - &a * &dimension) / &dimension;
        assert!(!zero.is_zero());
        let spectator = atom("(a+b)*(c+d)*tensor(p(1,mu),mu)");
        // This independent scalar master keeps the terms required by a
        // coefficient pole; evanescent coefficients still multiply its poles.
        let master = atom("eps^-2+3*eps^-1+5+7*eps");
        for (coefficient, pole_order, expected) in [
            (zero, 0, "0"),
            (atom("eps^2"), 0, "spectator"),
            (atom("exp(eps)-1-eps"), 0, "spectator/2"),
            (
                atom("1/(eps*(4-2*eps))"),
                1,
                "spectator/(4*eps^3)+7*spectator/(8*eps^2)+27*spectator/(16*eps)+83*spectator/32",
            ),
        ] {
            let coefficient = &spectator * coefficient;
            assert_eq!(
                ScalarTerms::epsilon_pole_order(coefficient.as_view(), &settings).unwrap(),
                pole_order
            );
            let terms = ScalarTerms(BTreeMap::from([(Atom::one(), coefficient.clone())]));
            let (kernel, aliases, extra_orders) = terms.backend_kernel(&settings, &[]).unwrap();
            assert_eq!(extra_orders, pole_order);
            let restored = aliases.iter().fold(kernel, |kernel, (alias, coefficient)| {
                kernel
                    .replace(alias.to_pattern())
                    .with(coefficient.to_pattern())
            ...
            assert_eq!(
                result.to_atom(),
                atom(expected)
                    .replace(atom("spectator").to_pattern())
                    .with(spectator.to_pattern())
            );
        }
    }
