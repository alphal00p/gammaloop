...
fn analytic_backends_preserve_coefficients_and_epsilon_pole_orders() {
    ...
    for evaluation_order in [
        ...
        // The Laurent value may omit or retain exact zero coefficients.
        for (power, value) in result.get_epsilon_coefficients() {
            assert!(
                value.is_zero() || (-2..=0).contains(&power),
                "unexpected coefficient at epsilon^{power}: {value}"
            );
        }
            ...
            for (power, value) in result.get_epsilon_coefficients() {
                assert!(
                    value.is_zero()
                        || expected
                            .iter()
                            .any(|(expected_power, _)| *expected_power == power),
                    "{coefficient}: unexpected coefficient at epsilon^{power}: {value}"
                );
            }
