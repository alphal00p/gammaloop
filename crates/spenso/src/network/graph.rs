...
use linnet::{
    half_edge::{
        ...
        subgraph::{Inclusion, ModifySubSet, SuBitGraph, SubGraphLike, SubSetLike, SubSetOps},
...
impl ScalarRef {
    pub fn index(self) -> usize {
        match self {
            ScalarRef::Store(index) | ScalarRef::Alias(index) => index,
        }
    }

    pub fn map_index(self, f: impl FnOnce(usize) -> usize) -> Self {
        match self {
            ScalarRef::Store(index) => ScalarRef::Store(f(index)),
            ScalarRef::Alias(index) => ScalarRef::Alias(f(index)),
        }
    }

    pub fn rebase_from(self, base: usize, offset: usize) -> Self {
    ...
    pub fn with_alias_state(self, should_alias: impl FnOnce(usize) -> bool) -> Self {
        let index = self.index();
        if should_alias(index) {
            ScalarRef::Alias(index)
        } else {
            ScalarRef::Store(index)
        }
    }

    pub fn display_label(self) -> String {
        match self {
            ScalarRef::Store(index) => index.to_string(),
            ScalarRef::Alias(index) => format!("scalar({index})"),
        }
    }

    pub fn display_with(self, scalar_disp: impl FnOnce(usize) -> String) -> String {
        match self {
            ScalarRef::Store(index) => scalar_disp(index),
            ScalarRef::Alias(index) => format!("scalar({index})"),
        }
    }
}

impl Display for ScalarRef {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
...
impl<K: Debug, FK: Debug, Aind: AbsInd> NetworkGraph<K, FK, Aind> {
    ...
    /// Close tensor boundaries before materializing the arms of a tensor sum.
    ///
    /// This is finite tensor execution preparation, not symbolic parsing: callers
    ...
    pub fn shift_scalars(&mut self, shift: usize) {
        ...
        self.map_scalar_refs(|scalar| scalar.map_index(|index| index + shift));
    }
    ...
    pub fn delete<S: SubSetLike<Base = SuBitGraph>>(&mut self, subgraph: &S) {
        ...
        self.slot_order.truncate(self.graph.n_hedges());
    }
            ...
            &|n| match n {
                NetworkNode::Leaf(l) => match l {
                    ...
                    NetworkLeaf::Scalar(s) => {
                        let scalar = s.display_with(&scalar_disp);
                        Some(format!("label = \"S:{}\"", scalar))
                    }
                },
                NetworkNode::Op(o) => {
...
// where
    ...
    pub fn scalar(pos: usize) -> Self {
        ...
        let head = graph.add_node(NetworkNode::Leaf(NetworkLeaf::Scalar(pos.into())));
        graph.add_external_edge(head, NetworkEdge::Head, true, Flow::Source);
...
pub mod test {
    ...
    use super::{NAdd, NMul, NetworkGraph, NetworkNode, NetworkOp};

    #[test]
    fn deleting_hedges_keeps_tensor_slot_order_aligned() {
        let mut graph = NetworkGraph::<i8>::mul_graph(3);
        graph.slot_order = vec![0, 1, 2, 3];
        let mut removed = SuBitGraph::empty(graph.graph.n_hedges());
        removed.add(super::Hedge(1));
        graph.delete(&removed);
    ...
    #[test]
    fn ready_sum_boundaries_preserve_dual_spectators_and_shared_library_references() {
        type Graph = NetworkGraph<OrderedStructure>;
        let matrix = |index| {
            Graph::tensor(
                &PermutedStructure::<OrderedStructure>::from_iter([
                    Minkowski {}.new_slot(3, 2).to_lib(),
                    Minkowski {}.new_slot(2, 1).to_lib(),
                ])
                .structure,
                NetworkLeaf::LocalTensor(index),
            )
        };
        let vector = Graph::key(PermutedStructure::<OrderedStructure>::from_iter([
            Minkowski {}.new_slot(2, 1).to_lib(),
        ]));
        let spectator = |index, slot| {
            Graph::tensor(
                &PermutedStructure::<OrderedStructure>::from_iter([slot]).structure,
                NetworkLeaf::LocalTensor(index),
            )
        };
        let mut graph = vector.n_mul([
            matrix(0).n_add([matrix(1)]),
            Graph::scalar(7),
            spectator(2, Lorentz {}.new_slot(2, 3).to_lib()),
            spectator(3, Lorentz {}.dual().new_slot(2, 3).to_lib()),
        ]);
        assert_eq!(graph.n_dangling(), 1);
        assert_eq!(graph.contract_ready_sum_boundaries(), 1);
        assert_eq!(graph.n_dangling(), 1);
        ...
        let nodes: Vec<_> = tree.iter_preorder_tree_nodes(&graph.graph, root).collect();
        assert_eq!(
            nodes
                .iter()
                .filter(|node| matches!(
        ...
        assert_eq!(
            nodes
                ...
                .count(),
            1,
            "scalar spectator stays outside the sum",
        );
        assert_eq!(
            nodes
                .iter()
        ...
        let mut dual = spectator(0, Lorentz {}.new_slot(2, 3).to_lib()).n_mul([spectator(
            1,
            Lorentz {}.dual().new_slot(2, 3).to_lib(),
        )
        .n_add([spectator(2, Lorentz {}.dual().new_slot(2, 3).to_lib())])]);
        dual.merge_ops();
        let unchanged = dual.clone();
        assert_eq!(dual.contract_ready_sum_boundaries(), 0);
        assert_eq!(dual, unchanged);

        let mixed = |index| {
            Graph::tensor(
                &PermutedStructure::<OrderedStructure>::from_iter([
                    Minkowski {}.new_slot(2, 1).to_lib(),
                    Lorentz {}.new_slot(2, 3).to_lib(),
                ])
                .structure,
                NetworkLeaf::LocalTensor(index),
            )
        ...
        for outside_first in [false, true] {
            let sum = mixed(0).n_add([mixed(1)]);
            let closing = spectator(2, Minkowski {}.new_slot(2, 1).to_lib());
            let outside = spectator(3, Lorentz {}.dual().new_slot(2, 3).to_lib());
            let mut graph = if outside_first {
                outside.n_mul([closing, sum])
            } else {
                sum.n_mul([closing, outside])
            };
            graph.merge_ops();
    ...
    #[test]
    fn ready_sum_boundaries_do_not_distribute_additive_or_partly_open_factors() {
        ...
        let sum = || tensor(0, &[1]).n_add([tensor(1, &[1])]);
        for mut graph in [
            sum().n_mul([sum()]),
            tensor(2, &[1, 2]).n_mul([sum()]),
            NetworkGraph::scalar(0).n_mul([sum()]),
        ] {
            graph.merge_ops();
            let unchanged = graph.clone();
            assert_eq!(graph.contract_ready_sum_boundaries(), 0);
        ...
        let mut graph = tensor(2, &[1]).n_mul([
            sum(),
            tensor(3, &[2]),
            tensor(4, &[2]).n_add([tensor(5, &[2])]),
        ]);
        assert_eq!(graph.contract_ready_sum_boundaries(), 2);
        let tree = graph.expr_tree().cast::<ChildVecStore<()>>();
        let root = graph.graph.node_id(graph.head());
        assert_eq!(
            tree.iter_preorder_tree_nodes(&graph.graph, root)
                .filter(|node| matches!(graph.graph[*node], NetworkNode::Op(NetworkOp::Sum)))
                .count(),
            2,
        );
    ...
    #[test]
    fn sparse_ready_batches_match_dense_conflict_filter_with_hidden_edges() {
        let tensor_a = NetworkGraph::<i8>::tensor(
            &PermutedStructure::<OrderedStructure>::from_iter([
                Minkowski {}.new_slot(1, 2),
                Minkowski {}.new_slot(2, 2),
        ...
        let mut expr = (tensor_a + tensor_b) * (NetworkGraph::scalar(2) + NetworkGraph::scalar(3));
        expr.merge_ops();
        expr.cache_expr_tree_roots();

        let tree = expr.expr_tree().cast::<ChildVecStore<()>>();
        let root = expr.graph.node_id(expr.head());
        let child = tree.iter_children(root, &expr.graph).next().unwrap();
        let expression_hedge = tree.root_hedge(child);
        let slot_hedge = expr
            .graph
            .iter_hedges()
            .map(|(hedge, _)| hedge)
            .find(|hedge| expr.graph[[hedge]].is_slot() && expr.graph.inv(*hedge) != *hedge)
            .unwrap();

        for hidden_hedges in [
            vec![],
            vec![expression_hedge],
            vec![expression_hedge, expr.graph.inv(expression_hedge)],
            vec![slot_hedge],
            vec![slot_hedge, expr.graph.inv(slot_hedge)],
        ] {
            let mut graph = expr.clone();
            ...
            let mut used: SuBitGraph = graph.graph.empty_subgraph();
            let mut expected = Vec::new();
            for node in graph.cached_expr_preorder_nodes_ignoring(&hidden) {
                let Some((op, children)) = graph.ready_operation_parts_ignoring(node, &hidden)
                else {
                    continue;
                };
            ...
            for operations in [
                graph.ready_operations_from_tree_ignoring(&hidden),
                graph
                    .ready_operation_tree_refs_ignoring(&hidden)
                    .iter()
                    .map(super::NetworkOperation::from)
                    .collect(),
                graph
                    .ready_operation_refs_ignoring(&hidden)
                    .iter()
                    .map(super::NetworkOperation::from)
                    .collect(),
            ] {
