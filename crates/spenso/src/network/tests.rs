...
fn ready_sum_boundary_closure_matches_coordinates_across_execution_strategies() {
    ...
    let lorentz = OrderedStructure::new(vec![Lorentz {}.new_slot(2, 9).to_lib()]).structure;
    let dual = OrderedStructure::new(vec![Lorentz {}.dual().new_slot(2, 9).to_lib()]).structure;
    let left_dual =
        Net::from_tensor(DenseTensor::from_data(vec![2.0, 3.0], lorentz.clone()).unwrap());
    let right_dual =
        Net::from_tensor(DenseTensor::from_data(vec![5.0, 7.0], dual.clone()).unwrap());
    ...
    let expected_structure =
        OrderedStructure::new(vec![Euclidean {}.new_slot(2, 2).to_lib()]).structure;
    for (name, original, expected) in [
        (
    ...
    let dual_sum = left_dual.clone()
        * (right_dual
            + Net::from_tensor(DenseTensor::from_data(vec![11.0, 13.0], dual.clone()).unwrap()));
    let additive =
    ...
    let right = Net::from_tensor(DenseTensor::from_data(vec![5.0, 6.0], lorentz).unwrap());
    let mixed_sum = (tensor(&[1], 1) * left_dual).n_add([tensor(&[1], 3) * right]);
    let closing = tensor(&[1], 7);
    let outside = Net::from_tensor(DenseTensor::from_data(vec![11.0, 13.0], dual).unwrap());
    for original in [
