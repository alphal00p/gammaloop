...
/// allocation measurement. Dense volume remains meaningful for tensor backends
/// which do not provide symbolic profiles.
...
#[cfg(test)]
mod intermediate_cost_tests {
    use super::*;

    #[test]
    fn intermediate_cost_prefers_small_entries_over_a_lower_rank_payload() {
        let profile = |total_bytes, total_terms| TensorContractionProfile {
            entries: 4,
            total_terms,
            max_terms: total_terms / 4,
            total_bytes,
            max_bytes: total_bytes / 4,
            common_factor_count: 0,
            simple_tensor: false,
        };
        let pair = |estimated_products| super::super::TensorContractionPairEstimate {
            estimated_products,
            estimated_output_entries: 4,
            output_dense_size: 4,
            max_output_entry_products: estimated_products.div_ceil(4),
            simple_tensor_penalty: 1,
            common_factor_penalty: 1,
        };
        // Four products of large symbolic entries are more expensive than
        // eight products of tiny entries, despite the former's smaller rank.
        let bulky = ResultRankPairScore::new(1, 1, profile(4000, 400), profile(4000, 400), pair(4));
        let small = ResultRankPairScore::new(2, 1, profile(16, 4), profile(16, 4), pair(8));
        assert!(bulky.less_by::<PAIR_SCORE_SPARSE_ATOM_AWARE>(&small));
        assert!(small.less_by::<PAIR_SCORE_INTERMEDIATE_COST>(&bulky));
        assert!(!bulky.less_by::<PAIR_SCORE_INTERMEDIATE_COST>(&small));
    }
}
