...
impl<R: RepName, Aind: AbsInd> OrderedStructure<R, Aind> {
    ...
    ///
    /// The returned [`Canonicalized`] contains the finalized structure and
    ...
    /// permuted with the recorded mappings; constructing a [`DenseTensor`] with
    /// `from_storage_data` and `.into_canonical()` does not reorder the component
    /// vector.
    ///
