...
#let native-cubics(segments, stroke: black + 0.6pt, dx: 0, dy: 0) = place(
  ...
  curve(
    stroke: stroke,
    ..segments
      .map(segment => native-cubic-items(segment, dx: dx, dy: dy))
      .flatten(),
  ),
)
  ...
  if points.len() < 2 {
    ...
    place(
      ...
      curve(
        ...
        ..points
          .slice(1)
          .map(point => curve.line(native-point(point, dx: dx, dy: dy))),
      ),
    )
  }
}
...
#let manual = [
  ...
  #context {
    ...
    let _ = tidy-style.insert(
      "show-example",
      tidy-style.show-example.with(scale-preview: 100%),
    )
