...
impl<T> Tree<T> {
    ...
    pub fn remove_node(&mut self, node_id: NodeId) {
        ...
        // From largest to smallest, so shifting indices does not affect removal.
...
mod tests {
    ...
    fn test_remove_node() {
        ...
        assert!(
            (tree.map(LinearSurfaceID).to_atom_inv()
                - expected_tree.map(LinearSurfaceID).to_atom_inv())
            .expand()
            .is_zero()
        );
