...
pub(crate) enum ContourClosure {
    Above,
    Below,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub(crate) struct EnergyResidue {
    pub basis: Vec<usize>,
    pub sigmas: Vec<i32>,
    pub sign: i32,
...
type Result<T> = std::result::Result<T, CutStructureError>;

pub(crate) fn energy_residues(
    loop_line_signatures: &[Vec<i32>],
    contour_closure: &[ContourClosure],
) -> Result<Vec<EnergyResidue>> {
...
mod tests {
    ...
    fn one_loop_residues_match_two_cut_signs() {
        let residues = energy_residues(&[vec![1]], &[ContourClosure::Below]).unwrap();
    ...
    #[test]
    fn two_loop_triangle_has_energy_residues() {
        let residues = energy_residues(
