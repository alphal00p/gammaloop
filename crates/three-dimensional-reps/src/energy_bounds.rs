...
pub fn normalize_energy_degree_bounds(
    ...
    for (edge, degree) in bounds {
        if *edge >= n_internal {
            return Err(EnergyBoundsError::EdgeOutOfRange(*edge, n_internal));
        }
        out[*edge] = *degree;
    }
    Ok(out)
...
mod tests {
    ...
    fn auto_numerator_expr_saturates_sparse_energy_bounds() {
        ...
        let actual = parse!(expr.replace('[', "_").replace(']', ""));
        let expected = parse!(
            "dot(edges_0,ext_0)*dot(edges_1,ext_1)*dot(edges_3,ext_0)^2*dot(edges_3,ext_1)*dot(edges_3,ext_2)"
        );
