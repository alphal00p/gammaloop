...
fn short_origin_label(origin: &str) -> String {
    origin
        .replace("confluent_residue", "cflt_res")
...
fn signed_named_term(name: &str, coeff: &Rational, use_color: bool) -> SignedTerm {
...
mod tests {
    ...
    fn display_renders_summary_and_orientation_details() {
        ...
        assert!(rendered.contains("den tree"));
    ...
    fn surface_list_reports_all_branch_powers() {
        ...
        expression.orientations[OrientationID(0)].variants = vec![variant];
        let rendered = render_expression_summary(
            &expression,
            &graph_info(&parsed),
            None,
        ...
        assert!(rendered.contains("[5,5^2]"));
    }
}
