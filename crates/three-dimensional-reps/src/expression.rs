    ...
    fn clear_selected_denominator_surface_sign(
        ...
        for esurface_id in raised_esurface_group.esurface_ids() {
            if let Some(sign) = self
                ...
                && sign < 0
            {
                self.prefactor *= Rational::from(sign);
            }
        }
    }
...
impl OrientationExpression {
    ...
    pub fn fuse_compatible_variants(&mut self) {
        let mut chain_contributions = BTreeMap::<VariantChainKey, Rational>::new();
        for mut variant in std::mem::take(&mut self.variants) {
            ...
            for chain in denominator_tree_chains(&variant.denominator) {
                ...
                *chain_contributions
                    .entry(key)
                    .or_insert_with(Rational::zero) += &variant.prefactor;
            }
        }
        ...
        self.variants.sort_by(|lhs, rhs| {
            lhs.half_edges
                ...
                .then(lhs.prefactor.cmp(&rhs.prefactor))
        });
    }
...
fn linear_energy_expr_key(expr: &LinearEnergyExpr) -> String {
    let internal = expr
        .internal_terms
        .iter()
        .map(|(edge_id, coeff)| format!("{}:{coeff}", edge_id.0))
        .join(",");
    let external = expr
        .external_terms
        .iter()
    ...
    format!(
        ...
        expr.uniform_scale_coeff, expr.constant
    )
}
...
mod tests {
    ...
    #[test]
    fn fusion_preserves_zero_from_a_filtered_denominator_tree() {
        let data = OrientationData::new(EdgeVec::from_iter([Orientation::Undirected]));
        let mut orientation = OrientationExpression::lower_sector_unit(data);
        let mut zero = cut_variant(&[], &[]);
        zero.denominator.remove_node(NodeId::root());
        orientation.variants.push(zero);
        assert_eq!(orientation.to_atom(), Atom::one());
        orientation.fuse_compatible_variants();
        assert_eq!(orientation.to_atom(), Atom::one());
    }
    ...
    fn orientation_id_selection_preserves_selector_shapes_and_factorized_bodies() {
        ...
        for id in [0, 1, 4, 9, 99].map(OrientationID) {
            let selected_value = Atom::num(i32::from(id == OrientationID(4)));
            let unknown_value = Atom::num(i32::from(id == OrientationID(99)));
            let cases = [
                (selected.clone(), selected_value.clone()),
                (unknown.clone(), unknown_value.clone()),
                (function!(sigma, -1), Atom::Zero),
                (function!(sigma, &x), Atom::Zero),
                (Atom::var(sigma), Atom::var(sigma)),
                (function!(sigma), function!(sigma)),
                (
                    function!(sigma, &selected, &unknown),
                    function!(sigma, &selected_value, &unknown_value),
                ),
                (function!(sigma, &selected), Atom::Zero),
                (function!(sigma, OrientationID(0).atom()), Atom::Zero),
                (function!(sigma, OrientationID(1).atom()), Atom::Zero),
                (
                    (&selected * &body + &unknown * &x) / (Atom::one() + &unknown * &x),
                    (&selected_value * &body + &unknown_value * &x)
                        / (Atom::one() + &unknown_value * &x),
                ),
            ];
    ...
    fn cut_alternatives_are_disjunctive_and_consume_only_selected_provenance() {
        ...
        for (variant, coefficient) in orientation.variants.iter_mut().zip([2, 3, 7]) {
            variant.prefactor = Rational::from(coefficient);
        }
        ...
        assert_eq!(restricted.to_atom(AllOrientations), Atom::num(5));
        for variant in variants {
            let expected = if variant.denominator_edges.contains(&EdgeIndex(4)) {
                BTreeMap::from([(vec![EdgeIndex(4)], -1)])
            } else {
                BTreeMap::new()
            };
            assert_eq!(variant.denominator_edge_support_signs, expected);
        }
