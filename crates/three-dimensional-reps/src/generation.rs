        ...
        let variants = groups
            ...
            .map(|((thermal_weight, prefactor), denominator_chains)| {
                crate::expression::CFFVariant {
                    prefactor: rational_coeff_atom(Rational::from(overall_sign) * prefactor),
                    thermal_weight,
...
impl<'a> BoundedCffBuilder<'a> {
    fn new(parsed: &'a ParsedGraph, options: &Generate3DExpressionOptions) -> Result<Self> {
        ...
        let mut builder =
            Self::for_bounds(parsed, bounds, options.medium_mode).with_options(options);
    ...
    fn build_quadratic_recursive(
        ...
        let mut remainder_builder =
            BoundedCffBuilder::for_bounds(self.parsed, remainder_bounds, self.medium_mode);
        ...
        let sub_bounds = sub_to_orig
            ...
            .collect::<Vec<_>>();
        let mut contact_builder =
            BoundedCffBuilder::for_bounds(&subparsed, sub_bounds, self.medium_mode);
...
impl<'a> KnownFactorCffBuilder<'a> {
    ...
    fn sample_occurrences(
        ...
        for (node_idx, sample) in nodes.iter().enumerate() {
            ...
            for term in Self::occurrence_normal_form_terms(&basis_poly, use_uniform_scale) {
                ...
                let (subparsed, sub_to_local) = BoundedCffBuilder::for_bounds(
                    parsed,
    ...
    fn append_known_polynomial_branch(
        ...
        let delete = parsed
            ...
            .collect::<Vec<_>>();
        let (branch_parsed, branch_to_local) = BoundedCffBuilder::for_bounds(
            parsed,
            vec![0; parsed.internal_edges.len()],
            self.medium_mode,
        )
        ...
        let mut lower_sector = LowerSectorCffBuilder::new(&branch_parsed, self.medium_mode);
    ...
    fn build(mut self) -> Result<ThreeDExpression<OrientationID>> {
        if self.parsed.internal_edges.is_empty() {
            ...
            for variant in &mut orientation.variants {
                variant.thermal_weight.medium_mode = self.medium_mode;
                if let Some(source_prefactor) = &self.source_prefactor {
                    variant.prefactor = rational_to_coefficient(
                        rational_from_coefficient(&variant.prefactor) / source_prefactor.clone(),
                    )?;
                }
            }
...
mod causal_generation_tests {
    ...
    fn known_factor_projection_preserves_survivors_and_reconstructs_only_deleted_variables() {
        ...
        let (subparsed, sub_to_parent) = BoundedCffBuilder::for_bounds(
            &parsed,
            vec![0; parsed.internal_edges.len()],
            crate::MediumMode::Vacuum,
        )
        ...
        let (without_carrier, remaining) = BoundedCffBuilder::for_bounds(
            &parsed,
            vec![0; parsed.internal_edges.len()],
            crate::MediumMode::Vacuum,
        )
        ...
        let (opposite_subparsed, opposite_sub_to_parent) = BoundedCffBuilder::for_bounds(
            &opposite,
            vec![0; opposite.internal_edges.len()],
            crate::MediumMode::Vacuum,
        )
    ...
    fn inherited_full_rank_terminal_keeps_single_signed_below_residue() {
        ...
        let builder = BoundedCffBuilder::for_bounds(&parsed, vec![1], crate::MediumMode::Vacuum);
        let source_prefactor = builder.source_prefactor.clone();
        let expression = builder.build_quadratic_recursive(true).unwrap();
        ...
        // The private base is source-normalized: beta=(-1)^N*chi=-1
        // for this one-line source. Chi alone is +1 and misses that conversion.
        assert_eq!(source_prefactor, Rational::from(-1));
        assert_eq!(
            rational_from_coefficient(&variant.prefactor) * source_prefactor,
            Rational::from(-1)
        );
    }
    ...
    fn embedded_full_rank_factor_keeps_single_below_residue() {
        ...
        assert!(
            standalone
                ...
                .iter()
                .any(|orientation| {
                    orientation.data.orientation == factorized_orientation.data.orientation
                        && orientation.loop_energy_map == factorized_orientation.loop_energy_map
                        && orientation.edge_energy_map == factorized_orientation.edge_energy_map
                }),
            "the factorized Below residue must retain an existing standalone routing"
        );
        assert_eq!(
    ...
    fn gl04_outer_unequal_mass_quadratic_obeys_complete_cut_identity() {
        ...
        assert!(repeated_groups(&parent).is_empty());
        ...
        let select_cut = |mut expression: ThreeDExpression<OrientationID>| {
            ...
            .canonical();
            let target_id = expression
                .surfaces
                .linear_surface_cache
                .iter_enumerated()
                .filter_map(|(id, surface)| (surface.expression == target).then_some(id))
                .exactly_one()
                .expect("one positive E1+E2-P surface belongs to the physical cut");
            let target_id = HybridSurfaceID::Linear(target_id);
            for orientation in &mut expression.orientations {
                orientation.variants.retain_mut(|variant| {
                    assert!(!variant.numerator_surfaces.contains(&target_id));
                    variant
                        .denominator
                        .keep_branches_with_value_count_mut(&target_id, 1);
    ...
    fn attached_tadpole_repeated_bubble_matches_independent_convergent_contour() {
        ...
        assert_eq!(denominator_connected_components(&parsed).len(), 1);
        for context in [
            CffGenerationContext::Standalone,
            CffGenerationContext::EmbeddedCffFactor,
        ] {
    ...
    fn mixed_carrier_contact_matches_independent_convergent_contour() {
        ...
        for context in [
            ...
            for external_energy in [2.4, 0.7] {
                ...
                let x_pure_raw = crate::eval::evaluate_expression(&x_graph, &x_pure, "1", &x_input)
                    .unwrap()
                    .value;
                let x_signed_raw =
                    crate::eval::evaluate_expression(&x_graph, &x_signed, "1", &x_input)
                        .unwrap()
                        .value;
                let y_raw = crate::eval::evaluate_expression(&y_graph, &y_pure, "1", &y_input)
                    .unwrap()
                    .value;
                // The existing signed terminal constructor includes the Below
                // closure sign. The free pure terminal contains both samples,
                // hence its conversion is -1/2. The repeated y scalar already
                // has the signed Below frame. Certify both scalar contours
                // independently before their product in the generalized parent.
                for (label, actual, expected) in [
                    ("signed terminal", x_signed_raw, -1.0 / (2.0 * ex)),
                    (
                        "pure terminal contour",
                        -x_pure_raw / 2.0,
                        -1.0 / (2.0 * ex),
                    ),
                    ("repeated y native +1", y_raw, -y_positive_residues),
                    (
    ...
    fn two_loop_full_rank_terminal_keeps_all_below_residue_sign() {
        ...
        let expression =
            BoundedCffBuilder::for_bounds(&parsed, vec![1, 1], crate::MediumMode::Vacuum)
    ...
    fn projected_contact_preserves_unrelated_quadratic_source_bound() {
        ...
        let expression =
            BoundedCffBuilder::for_bounds(&parsed, vec![2, 0, 2, 0, 0], crate::MediumMode::Vacuum)
                .build()
                .unwrap();
    ...
    fn nested_inherited_contours_use_latest_closure_on_a_shared_terminal_pivot() {
        ...
        let terminal_component = |inherited_contour_rows| {
            let mut builder = LowerSectorCffBuilder::new(&parsed, crate::MediumMode::Vacuum);
    ...
    fn terminal_duplicate_contact_sign_is_routing_and_order_invariant() {
        ...
        for (label, edge_data, active_edge) in [
            ...
            let expression =
                BoundedCffBuilder::for_bounds(&parsed, bounds, crate::MediumMode::Vacuum)
    ...
    fn quadratic_hermite_remainder_retains_powered_denominator_parity() {
        ...
        let expression =
            BoundedCffBuilder::for_bounds(&powered, vec![2, 0], crate::MediumMode::Vacuum)
                .build_quadratic_recursive(false)
                .unwrap();
    ...
    fn nonterminal_quadratic_contact_matches_independent_lower_cff_frame() {
        ...
        .unwrap();
        let (lower, lower_to_orig) = BoundedCffBuilder::for_bounds(
            &parsed,
            vec![0; parsed.internal_edges.len()],
            crate::MediumMode::Vacuum,
        )
        ...
        let (unit_lower, unit_lower_to_orig) =
            BoundedCffBuilder::for_bounds(&unit_parent, vec![0; 3], crate::MediumMode::Vacuum)
                .project_parsed_edges(&[0]);
        ...
        let rational_component_count = |graph: &ParsedGraph| {
            let signatures = graph
                ...
                .map(|edge| edge.signature.clone())
                .collect::<Vec<_>>();
            LowerSectorCffBuilder::new(graph, crate::MediumMode::Vacuum)
                .vector_matroid_components(&signatures)
                .len()
        };
    ...
    fn lower_sector_constructors_return_direct_expressions_for_all_bases() {
        ...
        let pure_component = LowerSectorCffBuilder::new(&pure_repeated, crate::MediumMode::Vacuum)
            .build()
            .unwrap();
        ...
        let affine = LowerSectorCffBuilder::new(&affine, crate::MediumMode::Vacuum)
            .build()
            .unwrap();
        ...
        let disconnected = LowerSectorCffBuilder::new(&disconnected, crate::MediumMode::Vacuum)
            .build()
            .unwrap();
    ...
    fn factorized_lower_sector_restores_global_core_sign_and_orientation_sum() {
        ...
        let lower = LowerSectorCffBuilder::new(&parsed, crate::MediumMode::Vacuum)
            .build()
            .unwrap();
        let input = crate::eval::EvaluationInput {
            external_momenta: Vec::new(),
    ...
    fn attached_repeated_factor_selector_branches_match_lower_sector_cff() {
        ...
        let lower_sector = LowerSectorCffBuilder::new(&parsed, crate::MediumMode::Vacuum)
            .build()
            .unwrap();
        let input = crate::eval::EvaluationInput::deterministic(
            &parsed,
    ...
    fn gl24_lower_sector_cff_is_invariant_under_edge_reversal() {
        ...
        let build_and_evaluate = |label: &str, graph: &ParsedGraph| {
            let signatures = graph
                ...
                .map(|edge| edge.signature.clone())
                .collect::<Vec<_>>();
            let builder = LowerSectorCffBuilder::new(graph, crate::MediumMode::Vacuum);
            assert_eq!(
        ...
        let signatures = reversed
            ...
            .map(|edge| edge.signature.clone())
            .collect::<Vec<_>>();
        let builder = LowerSectorCffBuilder::new(&reversed, crate::MediumMode::Vacuum);
    ...
    fn lower_sector_particular_lift_keeps_initial_cut_external() {
        ...
        assert!(crate::validate_parsed_graph(&parsed).ok);
        let expression = LowerSectorCffBuilder::new(&parsed, crate::MediumMode::Vacuum)
            .build()
            .unwrap();
        assert!(!expression.orientations.is_empty());
    ...
    fn ordinary_terminal_sources_match_signed_contours() {
        ...
        for (label, parsed, masses) in [
            ...
            for context in [
                ...
                let native_signed = source_factor * native_raw;
                eprintln!(
                    "RAW_ORDINARY_TERMINAL label={label} context={context:?} orientations={} source_factor={source_factor} native_signed={native_signed:.17e} public_signed={public_signed:.17e} contour={contour:.17e}",
                    generated.expression.orientations.len()
                );
                if (public_signed - contour).abs() > 1.0e-12 * contour.abs() {
                    failures.push(format!("{label} {context:?}: public={public_signed:.17e}, native={native_signed:.17e}, contour={contour:.17e}"));
                }
            }
        }
        assert!(
...
mod graph_source_tests {
    ...
    #[test]
    fn public_initial_cut_labels_preserve_complete_residue_maps() {
        for external_sign in [1, -1] {
            for mode in [
                ...
                if matches!(mode, "repeated" | "bounded_repeated") {
                    ...
                    assert_eq!(repeated_groups(&parsed)[0].edge_ids, vec![1, 2]);
                    ...
                    parsed.node_name_to_internal.insert("n2".to_string(), 2);
                    assert_eq!(denominator_connected_components(&parsed).len(), 2);
                ...
                for remapped in [false, true] {
                    ...
                    let mut original =
                        generate_3d_expression_from_parsed_generated(&parsed, &options).unwrap();
                    let generated = if remapped {
                        original.expression = original
                            .expression
                            .remap_energy_edge_indices(&edge_map)
                            .fuse_compatible_variants();
                        assign_numerator_map_labels(&mut original.expression.orientations);
                    ...
                    assert!(!generated.expression.orientations.is_empty(), "{mode}");
                    assert_eq!(
                        generated.expression.orientations.len(),
                        original.expression.orientations.len(),
                        "{mode}: canonical labels must not merge complete residue keys"
                    );
                    assert_eq!(
                        generated.energy_factor_ownership,
                    ...
                    let mut restored = generated.expression;
                    for (old, new) in original
                        .expression
                        .orientations
                        .iter()
                        .zip(&mut restored.orientations)
                    {
                        assert_eq!(
                            new.data.orientation[EdgeIndex(cut_edge)],
                            Orientation::Default,
                            "{mode}"
                        );
                        assert_eq!(
                            new.data.label.as_deref().unwrap().as_bytes()[cut_edge],
                            b'+',
                            "{mode}: public label retained the old initial-cut prefix"
                        );
                        assert_eq!(
                            new.edge_energy_map[cut_edge],
                        ...
                        for (edge, direction) in &old.data.orientation {
                            if edge.0 != cut_edge {
                                assert_eq!(new.data.orientation[edge], *direction, "{mode}");
                            }
                        }
                        let old_prefix = old
                            .data
                            ...
                            .next()
                            .unwrap();
                        if old_prefix.len() == old.data.orientation.len().0
                            && old_prefix
                                .bytes()
                                .all(|value| matches!(value, b'+' | b'-' | b'0' | b'x'))
                        {
                            for (edge, value) in old_prefix.bytes().enumerate() {
                                if edge != cut_edge {
                                    assert_eq!(
                                        new.data.label.as_deref().unwrap().as_bytes()[edge],
                                        value,
                                        "{mode}: a non-cut display label changed"
                                    );
                                }
                            }
                            assert_eq!(
                                new.data.numerator_map_index, old.data.numerator_map_index,
                                "{mode}: the unchanged energy maps changed their N indices"
                            );
                        }
                        // Restore only public labels and the certified cut slot.
                        // Exact serialization then checks every map, surface,
                        // factor, sign, denominator and variant in the old order.
                        new.data = old.data.clone();
                    }
                    assert_eq!(
                        bincode::encode_to_vec(&restored, bincode::config::standard()).unwrap(),
                        bincode::encode_to_vec(&original.expression, bincode::config::standard())
                            .unwrap(),
                        "{mode}: cut-label normalization changed the complete source expression"
                    );
                }
            }
        }
...
mod cff_tests {
    ...
    fn connected_generalized_product_tracks_lifted_global_duplicates() {
        ...
        let signatures = parsed_with_pure_duplicate
            ...
            .collect::<Vec<_>>();
        let builder =
            LowerSectorCffBuilder::new(&parsed_with_pure_duplicate, crate::MediumMode::Vacuum);
        let components = builder.vector_matroid_components(&signatures);
        let (ordinary_component, _) = builder
            .project_component_parsed(&signatures, &components[0], &[0])
            .unwrap();
