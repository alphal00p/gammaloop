...
use spenso::{
    ...
    structure::{
        ...
        slot::IsAbstractSlot,
        OrderedStructure, ScalarTensor, TensorStructure,
    },
                    ...
                    HedgePair::Paired { source, sink } => {
                        if let Some(lmb_id) = e.data.lmb_id {
                            if let Some(old_value) = lmb_ids.insert(lmb_id, eid) {
                                return Err(eyre!(
                                    "lmb_id {:?} already exists with value {:?}",
                                    lmb_id,
                                    old_value
                                ));
                            }
                            ...
                            )
                        {
                            if i.rep().matches(&j.rep()) {
                                color_num_e[eid] *= j.rep().id(i.aind, j.aind);
                            } else {
