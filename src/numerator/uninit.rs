...
use spenso::{
    ...
    network::{library::TensorLibraryData, Network},
    structure::{
        representation::{Euclidean, LibraryRep},
...
impl Numerator<UnInit> {
    ...
    pub fn from_new_graph<S: SubGraph>(
        ...
        let mut colorless_builder = prefactor.colorless.clone();

        let mut colorful_builder = prefactor.color.clone();
