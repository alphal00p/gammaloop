...
fn ratio_compare() {
    ...
    // -----------------------------------------------------------------------------
    // n        E             px             py              pz               m
    // 1   0.58600000000000000E+03  0.00000000000000000E+00  0.00000000000000000E+00  0.57358172913718238E+03  0.11999999999999976E+03
    // 2   0.58600000000000000E+03  0.00000000000000000E+00  0.00000000000000000E+00 -0.57358172913718238E+03  0.11999999999999976E+03
    // 3   0.19538871635897596E+03 -0.22666188279138453E+02  0.41105903024355840E+02 -0.77745090686520300E+02  0.17299999999999997E+03
    // 4   0.37857156244115407E+03 -0.10650684775112998E+03 -0.30965943861484556E+03  0.78452223346396792E+02  0.17300000000000011E+03
    // 5   0.15625654900769729E+03 -0.10859017233120495E+03 -0.10020976857176890E+03  0.50816196863467042E+02  0.20230487285229133E-05
    // 6   0.44178317219217269E+03  0.23776320836147340E+03  0.36876330416225863E+03 -0.51523329523343548E+02  0.65380617145548286E-05
    // -----------------------------------------------------------------------------
    // Helicity=           0           0          -1           1          -1          -1
    // AMP(1)= (-9.34344742081958172E-012,-1.65023639802701860E-010)
    // Helicity=           0           0          -1           1          -1           1
    // AMP(1)= (-3.64496636602360159E-009,-1.01464000053425044E-009)
    // Helicity=           0           0          -1           1           1          -1
    // AMP(1)= (-3.10205438285936560E-010,-1.60174383754970851E-010)
    // Helicity=           0           0          -1           1           1           1
    // AMP(1)= (-2.49599716763135805E-010,-6.60375406746473258E-009)
    // Helicity=           0           0          -1          -1          -1          -1
    // AMP(1)=  (1.57120709528938986E-008,-2.62057723129549968E-008)
    // Helicity=           0           0          -1          -1          -1           1
    // AMP(1)=  (1.02357269890128478E-009,-2.68691234759103913E-010)
    // Helicity=           0           0          -1          -1           1          -1
    // AMP(1)=  (-4.67143137690823360E-009,4.49811411445219046E-010)
    // Helicity=           0           0          -1          -1           1           1
    // AMP(1)= (-8.93989423627355454E-011,-9.64051266161532750E-010)
    // Helicity=           0           0           1           1          -1          -1
    // AMP(1)=  (8.93989423627356230E-011,-9.64051266161532750E-010)
    // Helicity=           0           0           1           1          -1           1
    // AMP(1)=   (4.67143137690823360E-009,4.49811411445219511E-010)
    // Helicity=           0           0           1           1           1          -1
    // AMP(1)= (-1.02357269890128499E-009,-2.68691234759102310E-010)
    // Helicity=           0           0           1           1           1           1
    // AMP(1)= (-1.57120709528938986E-008,-2.62057723129549935E-008)
    // Helicity=           0           0           1          -1          -1          -1
    // AMP(1)=  (-2.49599716763135857E-010,6.60375406746473093E-009)
    // Helicity=           0           0           1          -1          -1           1
    // AMP(1)=  (-3.10205438285936664E-010,1.60174383754970671E-010)
    // Helicity=           0           0           1          -1           1          -1
    // AMP(1)=  (-3.64496636602360118E-009,1.01464000053425003E-009)
    // Helicity=           0           0           1          -1           1           1
    // AMP(1)=  (-9.34344742081992261E-012,1.65023639802702040E-010)
    // Matrix element =    6.6715984428709583E-012  GeV^          -4
    // -----------------------------------------------------------------------------

    let all_plus_gl = Complex::new(3.397311122593196e-8, -2.329600746857571e-9);
    let mppp_gl = Complex::new(2.4124167266079924e-9, -1.483635833524474e-8);

    let all_plus_target = Complex::new(-1.57120709528938986E-008, -2.62057723129549935E-008);
    let mppp_target = Complex::new(-2.49599716763135805E-010, -6.60375406746473258E-009);

    println!("{}", all_plus_gl.norm() / all_plus_target.norm());

    println!("{}", mppp_gl.norm() / mppp_target.norm());

    /// AMP(1)=  (-1.11798214070957178E-002,4.48335439600943478E-002)
    //  AMP(2)=  (-2.28783480574661705E-002,9.17472100867136181E-002)
    let all_plus_ta_ta_gl = Complex::new(-1.0186412591747754e-2, -2.540112670050997e-3);
    let all_plus_ta_ta_target = Complex::new(-2.28783480574661705E-002, 9.17472100867136181E-002);

    let mppp_ta_ta_gl = Complex::new(-5.886339770564276e-20, 1.194430814775141e-1);
    let mppp_ta_ta_target = Complex::new(1.00678888368575629E-002, 5.35403944803097744E-003);

    // AMP(1)=              (0.0000000000000000,-0.0000000000000000)
    //  AMP(2)=   (1.00678888368575629E-002,5.35403944803097744E-003)

    println!(
        "{}",
        all_plus_ta_ta_target.norm() / all_plus_ta_ta_gl.norm()
    );

    println!("{}", mppp_ta_ta_target.norm() / mppp_ta_ta_gl.norm());

    let mp_th_th_vxhhh_gl = Complex::new(-1.337658898292158e-1, -3.3356235814372484e-2);
    let mp_th_th_vxhhh_target = Complex::new(3.07823431007100690E-002, -0.12344396948277445);

    println!(
        "(-.+.)t h->t h:{}",
        mp_th_th_vxhhh_target.norm() / mp_th_th_vxhhh_gl.norm()
    );

    let pp_th_th_vxhhh_gl = Complex::new(0e0, -2.637453235128374e-2);
    let pp_th_th_vxhhh_target = Complex::new(2.43393595706816734E-002, 0.0000000000000000);

    println!(
        "(+.+.)t h->t h:{}",
        (pp_th_th_vxhhh_target.norm() / pp_th_th_vxhhh_gl.norm())
    );

    let pp_th_th_3_gl = Complex::new(-2.4854801171330698e-1, 2.0498453296948058e-1);

    let pp_th_th_3_target = Complex::new(-0.71162129569519772, 3.59283476711791996E-017);
    ...
    println!(
        "(-.+.)t h->t h:{}",
        (mp_th_th_3_target.norm() / mp_th_th_3_gl.norm())
    );
}
