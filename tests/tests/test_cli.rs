...
#[test]
#[serial]
fn run_history_load_accepts_command_block_templates_that_need_late_parsing() -> Result<()> {
    let mut cli =
        new_cli("run_history_load_accepts_command_block_templates_that_need_late_parsing")?;
    let path = run_card_path("late_parsing.toml");
...
[[command_blocks]]
name = "quit_template"
commands = ["quit -n --override-state $(overwrite)"]
"#,
    )?;
    let history = RunHistory::load(&path)?;
    assert!(matches!(
        cli.apply_boot_run_history(&history)?,
        ControlFlow::Continue(())
    ));
    cli.save_state()?;
    cli.run_history = RunHistory::load(cli.cli_settings.state.folder.join("run.toml"))?;
    let flow = cli
        .run_command_flow("run quit_template -D overwrite=true")?
        .flow;
    assert!(
        matches!(flow, ControlFlow::Break(save) if save.no_save_state && save.override_state == Some(true))
    );
    Ok(())
}
...
#[test]
#[serial]
fn nested_inline_runs_apply_defines_in_their_own_scope() -> Result<()> {
    let mut cli = new_cli("nested_inline_runs_apply_defines_in_their_own_scope")?;
    cli.run_command(
        r#"run -c 'run -D level=warn -c "set global kv global.display_directive=$(level)"'"#,
    )?;
    assert_eq!(cli.cli_settings.global.display_directive, "warn");
    cli.run_command(r#"run -D level=warn -c 'run -D level=error -c "set global kv global.display_directive=$(level)"'"#)?;
    assert_eq!(cli.cli_settings.global.display_directive, "error");
    cli.run_command(
        r#"run -D level=warn -c 'run -c "set global kv global.display_directive=$(level)"'"#,
    )?;
    assert_eq!(cli.cli_settings.global.display_directive, "warn");
    Ok(())
}
...
#[test]
#[serial]
fn boot_nested_blocks_resolve_inherited_variables_when_invoked() -> Result<()> {
    let mut cli = new_cli("boot_nested_blocks_resolve_inherited_variables_when_invoked")?;
    let history = RunHistory {
        ...
        commands: vec![command("run outer -D level=warn")],
        ..Default::default()
    };
    assert!(matches!(
        cli.apply_boot_run_history(&history)?,
    ...
    assert_eq!(cli.cli_settings.global.display_directive, "warn");
    cli.run_command("run outer -D level=error")?;
    assert_eq!(cli.cli_settings.global.display_directive, "error");
    Ok(())
}
...
#[test]
#[serial]
fn boot_parameterized_blocks_reject_missing_and_recursive_calls() -> Result<()> {
    for (name, nested_commands, expected) in [
        (
            "missing",
            vec![block("inner", &["run absent"])],
            "Unknown command block 'absent'",
        ),
        (
            "recursive",
            vec![block("inner", &["run outer"])],
            "Command block recursion detected",
        ),
    ] {
        let mut cli = new_cli(&format!("boot_parameterized_blocks_reject_{name}"))?;
        let original_display = cli.cli_settings.global.display_directive.clone();
        let mut command_blocks = vec![block(
            "outer",
            &[
                "set global kv global.display_directive=$(level)",
                "run inner",
        ...
        command_blocks.extend(nested_commands);
        let history = RunHistory {
            command_blocks,
            ..Default::default()
        };
        let error = cli.apply_boot_run_history(&history).unwrap_err();
        assert!(format!("{error:?}").contains(expected), "{error:?}");
        assert!(cli.run_history.command_block("outer").is_none());
        assert_eq!(cli.cli_settings.global.display_directive, original_display);
    }
    Ok(())
}
...
#[test]
#[serial]
fn three_d_rep_defaults_keep_artifacts_outside_unsaved_state() -> Result<()> {
    let mut cli = new_cli("three_d_rep_defaults_keep_artifacts_outside_unsaved_state")?;
    let cwd = cli_state_path("three_d_rep_default_workspace");
    clean_test(&cwd);
    let _cwd_guard = CurrentDirGuard::enter(&cwd)?;
    cli.cli_settings.state.name = None;
    cli.run_command("import model scalars")?;
    cli.run_command(&format!(
        "import graphs {} -p box",
        graph_fixture_path("scalar_box.dot").display()
    ))?;
    cli.run_command("3Drep build -p box -i default -g 0 --no-pretty")?;
    let flow = cli.run_command_flow("quit -n")?.flow;
    assert!(matches!(flow, ControlFlow::Break(save) if save.no_save_state));
    let latest =
        fs::read_to_string(cwd.join("threed_workspace/latest_oriented_expression_path.txt"))?;
    let output: serde_json::Value = serde_json::from_slice(&fs::read(latest.trim())?)?;
    assert_eq!(output["graph_name"], "scalar_box");
    assert_eq!(output["validation"]["ok"], true);
    assert!(
        !cli.cli_settings
            .state
            .folder
