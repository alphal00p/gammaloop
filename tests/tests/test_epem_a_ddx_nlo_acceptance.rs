...
#[test]
#[serial]
fn epem_a_ddx_nlo_is_alpha_s_over_pi_times_lo_in_both_local_uv_routes() -> Result<()> {
    ...
    let test_root = get_tests_workspace_path()
        .join("epem_a_ddx_nlo_is_alpha_s_over_pi_times_lo_in_both_local_uv_routes");
    ...
    for (process, _) in ROUTES {
        ...
        assert!(
            ...
            "the {process} NLO probe is not phase-pure enough for a single-phase magnitude acceptance: {nlo_probe:e}",
        );
    ...
    for (route_index, (process, _)) in ROUTES.into_iter().enumerate() {
        ...
        assert!(
            ...
            "{process} inclusive NLO normalization mismatch: |NLO|={nlo_value:e} ± {nlo_error:e}, (aS/pi)*LO={expected_nlo:e} ± {:e}, |delta|={delta:e}, delta/sigma={:e}",
    ...
    let [
        ...
        unreachable!("both local UV routes are always evaluated")
    };
    let route_delta = (first_value - second_value).abs();
    let route_error = first_error.hypot(*second_error);
    assert!(
        route_delta <= 3.0 * route_error,
        "local UV route mismatch: {first_process}={first_value:e} ± {first_error:e}, {second_process}={second_value:e} ± {second_error:e}, delta/sigma={:e}",
