...
mod slow {
    use super::*;
    #[test]
    fn box_4e_self_energy_bulk_profile_passes() -> Result<()> {
        let state_path = get_tests_workspace_path().join("box_4e_self_energy_bulk_profile");
        let run_history = run_card("box_4e_self_energy_generate.toml")?;
        let setup_commands = run_history
            .commands
            .iter()
            .map(|command| {
        ...
        assert!(
            run_history
                ...
                .iter()
                .any(|command| command.raw_string.as_deref() == Some("profile bulk")),
            "expected tests/resources/run_cards/box_4e_self_energy_generate.toml to contain the profile bulk command"
        );
        assert_eq!(setup_commands.last().copied(), Some("generate"));
