...
fn run_scalar_3l_cross_section_case_impl(
    ...
    let certify_zero = |cli: &gammaloop_integration_tests::CLIState,
     ...
     -> Result<bool> {
        ...
        let coefficient = (original / &expected).collect_factors();
        ...
        assert_eq!(
            ...
            (&expected * coefficient).collect_factors()
        );
    ...
    for (evaluation_index, (process, integrand, label)) in evaluations.into_iter().enumerate() {
        ...
        if let Some(localized_3d_results) = &localized_3d_results {
            ...
            for (route, actual) in [
                ...
                let relative_distance = if IsZero::is_zero(&scale) {
                    distance.clone()
                } else {
                    distance.clone() / scale.clone()
                };
                // Samples supplied as f64 may cancel differently between routes.
                // Only the exact source-zero certificate permits an absolute bound;
                // every nonzero result must satisfy the Arb relative comparison.
                let finite = [actual, &explicit_3d_arb]
                    .into_iter()
