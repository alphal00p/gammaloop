...
fn cff_cli_validate_and_build_use_gammaloop_graph_state() -> Result<()> {
    ...
    let process_ref = ProcessRef::Unqualified("threedreps_box_build".to_string());
    let integrand_name = "default".to_string();
    let (process_id, integrand_name) = cli
        .state
        .find_integrand_ref(Some(&process_ref), Some(&integrand_name))?;
    let ProcessCollection::Amplitudes(amplitudes) =
        ...
        panic!("the scalar box must import as an amplitude")
    };
    let graph = &amplitudes[&integrand_name].graphs[0].graph;
    let parsed = graph.to_three_d_parsed_graph()?;
    let source_map = graph.energy_edge_index_map(&parsed).unwrap();
    let local_map = EnergyEdgeIndexMap {
        internal: source_map
            .internal
            .into_iter()
    ...
    let exported =
        serde_json::from_value::<ThreeDExpression<OrientationID>>(artifact["expression"].clone())?
            .remap_energy_edge_indices(&local_map);
    let reference = three_dimensional_reps::generate_3d_expression(&parsed, &Default::default())?;
    for seed in [17, 41] {
        let input = EvaluationInput::deterministic(&parsed, seed, &Default::default(), None)?;
        let actual = evaluate_expression(&parsed, &exported, "1", &input)?.value;
        let expected = evaluate_expression(&parsed, &reference.expression, "1", &input)?.value;
        let scale = actual.abs().max(expected.abs()).max(f64::MIN_POSITIVE);
        assert!(
            actual.is_finite()
                && expected.is_finite()
                && (actual - expected).abs() <= 1.0e-12 * scale,
            "the exported scalar source changed at seed {seed}: {actual:e} != {expected:e}"
        );
    }
    assert_eq!(
        cli.cli_settings
