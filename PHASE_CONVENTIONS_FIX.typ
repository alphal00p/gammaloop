...
<gammaloop-phase-conventions-fix-proposed-implementation-plan>
#quote(block: true)[
Historical phase-stack plan and execution record. All results below apply to the recorded source checkpoints, including superseded intermediate steps. They do not certify the reconstructed reviewed stack; see its #link("docs/architecture/raised-energy-cff-stack-review.typ")[review] and #link("docs/architecture/raised-energy-cff-stack-review-validation.typ")[fresh validation];. Current work follows #link("CONTRIBUTING.typ") and #link("docs/architecture/architecture-current.typ")[current architecture];.
]

#quote(block: true)[
All final-source local gates pass: 2,084 exact `just test_gammaloop` tests, 114 focused tests, and all 166 scalar LU cases. The four signed ddx/ttx LO/NLO acceptances, fresh API/schema/parity, check, Clippy, formatting, and Hakari pass. Four fast numerical PySecDec tests were run manually and pass; all 24 physical PySecDec tests remain excluded from automatic local/CI coverage. CP symmetrization remains an explicit opt-in with warnings. The originally accepted plan is preserved below. The accepted research-driven revision after the verbatim addenda supersedes the proposed numerator-conjugation modes and their implementation steps.
]

This is an investigation-backed draft for acceptance. Implementation has not started. At implementation start, copy the accepted plan verbatim to PHASE\_CONVENTIONS\_FIX.md at the repository root, including these addenda, and set the goal statement below as the active task goal. Keep later implementation evidence in an appended execution log so the accepted plan remains intact.

=== Goal statement
<goal-statement>
Make GammaLoop return the complete textbook Minkowski Feynman-rule amplitude contribution A\_Gamma = -i M\_Gamma, with a consistent scalar UFO/JSON model, and physical LU cross-sections obtained by correctly conjugating the right-hand amplitude. Establish this through independent scalar contour and phase-space oracles across loop counts, exact and scalar\_only numerator-conjugation tests, and signed ddx/ttx LO and NLO acceptances matching the published values without compensating phases or absolute-value comparisons. Only after those conventions are certified, migrate all affected tests, snapshots, examples, and documentation, complete the required checks, and push the finished changes to the current branch name with suffix \_phase\_fix.

=== Findings that determine the plan
<findings-that-determine-the-plan>
+ The shared energy representation is J = integral product\[dq0/(2#emph[pi];i)\] N/product(D^a), with D = q^2 - m^2 + i0. GammaLoop's existing source-frame and inverse-energy signs correctly recover J. Production then multiplies by (-i)#super[L/(2#emph[pi)^(3L), whereas the physical Minkowski measure requires i^L/(2];pi)];(3L). The current uncut amplitude therefore differs from the Feynman-rule result by (-1)^L. This statement holds for the audited scalar contour boundary; each subtraction branch must be verified consistently during implementation.
+ The independent tadpole and tadpole-times-bubble contour check in crates/gammalooprs/src/cff/mod.rs:6879 explicitly strips the production (-i)^L factor at line 6946 before comparison with J. This distinguishes loop parity from denominator parity. The three production normalization sites are cff/mod.rs:392, :699, and :828. The real source-frame adapter at :224 and the 1/product(-2E) convention in graph/mod.rs:724 must not be independently flipped.
+ Integrated UV terms have the same historical convention: uv/settings.rs:425 defaults additional\_normalization to "-1", and uv/approx/integrated.rs:977 applies it once per integrated loop. The amplitude correction must change this default coherently to "1", including its serialization-default predicate. Preserve forest subtraction signs and actual causal prescriptions.
+ The vendored scalar UFO uses +i#emph[lambda, while the standard interaction L\_int = -lambda];phi^4/4! requires -i#emph[lambda. Its full JSON export has +i scalar propagator numerators, as expected. The actively used scalars\_2p\_3p.json instead has propagator numerators 1; its coupling expression and cached numerical value also use +i];lambda. Both JSON variants and the UFO must be corrected consistently.
...
+ The present scalar LU matrix's "no\_numerator" cases retain complete UFO Feynman rules; the label means no additional momentum numerator. A graph-level DOT num="1" also does not clear edge and vertex factors. Reuse Graph::with\_global\_numerator\_only for a genuinely complete unit numerator.
+ RHS ownership first becomes insufficient when CrossSectionCut is reduced to LuCutSelection: physical cut alternatives survive, but the side-dependent numerator information does not. UV forest values are also computed and shared before the current cut loop. Applying conjugation only to the final numerator would therefore leave UV counterterms incorrect.
+ This checkout has no method literally named complex\_conjugate. Existing spenso\_conj, scalar/tensor network mapping, and Dirac simplifiers provide the required operations. dirac\_adjoint additionally inserts gamma0 and transposes open spin-line endpoints; it must not be substituted blindly for elementwise conjugation of an amputated RHS tensor.
+ Current ddx/ttx acceptances contain phase-selecting probes, norm-based LO/NLO comparisons, and manually supplied +/-i generation prefactors. Some individual graph comparisons already retain signs, but their inputs still contain these compensating conventions.

=== Convention to impose
<convention-to-impose>
Use metric (+,-,-,-), positive-energy physical cuts, and the ordinary measure product\[d^4k/(2\*pi)^4\]. Return the complete amputated Feynman graph, including its vertex and propagator numerator factors, as A\_Gamma = -i M\_Gamma in the user's notation. The operational anchor is the rule itself: a phi^4 contact graph is -i#emph[lambda and an internal scalar propagator is i/(q#super[2-m];2+i0). Some textbooks name the complete graph i];M instead; no numerical ambiguity remains once this anchor is fixed.

For mixed scalar interactions with occupation numbers n\_a, use L\_int = -lambda#emph[product(phi\_a^n\_a)/product(n\_a!). Their vertex is -i];lambda without another factorial; feyngen independently owns diagram and identical-state symmetry factors.

For a literal complete unit numerator and P = sum of denominator powers, a Wick-rotatable integral obeys I\_M = i^L#emph[(-1)^P];I\_E. I\_E is real and is positive for a convergent positive Euclidean scalar integrand. A no-threshold condition alone does not guarantee UV convergence or the sign of a renormalized finite remainder. The phase axis still alternates between real and imaginary with loop parity. In an ordinary four-point phi^4 topology P = 2L, so the unit-numerator sequence at L = 0,1,2,3,4 is 1,i,-1,-i,1. Physical UFO amplitudes include additional vertex and propagator phases; they must be tested separately.

For LU, impose d sigma\_cut = positive physical normalization \* A\_left \* conjugate(A\_right) \* dPhi\_n.~For identical scalar Born halves this is nonnegative, and strictly positive at nonzero interior points. There is no residual phase depending on the forward loop count. Do not impose positivity on individual interference, UV-counterterm, threshold-counterterm, or gauge-projector graph contributions.

The source-derived LU conversion to certify is: after fixing the shared contour normalization to i^L, retain each cut-line propagator numerator exactly once, conjugate RHS numerator factors and the RHS virtual contour, and replace the current 2#emph[pi LU residue factor by 2];pi#emph[i. In this representation the additional RHS contour factor is (-1)^L\_R, where cut legs are external and L\_R counts RHS virtual loops. Thus the combined LU conversion is i];(-1)^L\_R relative to the physical forward contour. This is an analytic residue-to-cut conversion, not a process-dependent fitted prefactor. Verify it with an independently sewn amplitude and phase-space measure before making it the production contract.

References: - David Tong's Feynman rules and phi^4 example: https:\/\/www.damtp.cam.ac.uk/user/tong/qft/qfthtml/S3.html - Local Unitarity: cutting raised propagators and localising renormalisation, arXiv:2203.11038, especially Eq. (7.1) and the signed MSbar results table on PDF page 55: https:\/\/arxiv.org/pdf/2203.11038

=== Implementation sequence
<implementation-sequence>
+ Record the accepted plan and establish the branch and goal.
  - Confirm the current clean/dirty state and preserve any user work.
  - The inspected current branch is codex/raised\_energy\_cff\_wip\_optimized. Create codex/raised\_energy\_cff\_wip\_optimized\_phase\_fix from its current tip.
  - Write the accepted plan verbatim into repository-root PHASE\_CONVENTIONS\_FIX.md, including the goal statement, original request, and follow-up instructions.
...
+ Make the scalar model a textbook oracle.
  - Change assets/models/ufo/scalars/couplings.py to -i\*lambda.
  - Regenerate/update assets/models/json/scalars/scalars.json and scalars\_2p\_3p.json, including symbolic expressions and cached coupling values \[0,-1\] at lambda=1.
  - Restore scalar propagator numerators +i in scalars\_2p\_3p.json while preserving its restricted interaction inventory.
  ...
  - Do not apply a universal potential-coupling sign change to the separate scalar\_gravity model.
+ Correct the amplitude measure at its existing owner.
...
+ Introduce cut-aware numerator conjugation before UV construction.
  - Add global.generation.complex\_conjugation = "exact" | "scalar\_only", with exact as the default, on the existing GenerationSettings owner.
  - Reuse CrossSectionCut::amplitude\_side\_subjects for side membership. Cut spin sums/projectors and globally supplied complete-forward prefactors remain outside the amputated RHS conjugation.
...
+ Implement and certify the two modes using existing tensor APIs.
  ...
  - Exact must remain correct under subsequent numerical model updates. Scalar\_only is specialized to generation-time coupling reality; record that assumption and require regeneration when an update invalidates it.
...
+ Complete the LU phase and causal-conjugation conversion.
  - Implement the certified i\*(-1)^L\_R residue/contour conversion at the existing cut boundary, retaining cut-propagator numerator factors once.
  ...
  - Add a one-virtual-loop RHS example and its left/right mirror: Born-only cuts have L\_R=0 and cannot validate this part of the convention.
...
+ Establish independent scalar LU acceptance across multiplicities.
  ...
  - At deterministic interior points, compare the complete cut weight to an independently evaluated |A\_Gamma|^2 times the LU phase-space Jacobian. Check a vanishing imaginary component and positive real value, in both conjugation modes.
  - Run MC integrations for each multiplicity against independent massless n-body phase-space normalization: Phi\_n(s) = s^(n-2) / \[2#emph[(4];pi)^(2#emph[n-3)];Gamma(n)\*Gamma(n-1)\]. For one scalar of mass M decaying to n identical massless scalars, the rate oracle is |g|^2#emph[Phi\_n(M^2)/(2];M\*n!) when the physical flux is enabled and unit conversion is disabled.
...
+ Require signed ddx and ttx physical acceptances.
  ...
  - In the paper's direct-current normalization and MSbar scheme, check:
    ...
    #figure(
      align(center)[#table(
        ...
        table.header([Channel], [LO], [NLO vertex], [NLO self-energy], [Total NLO correction],),
        table.hline(),
  ...
  - For the lepton processes, apply the established Eq. (7.1) real conversion and picobarn units; compare signed absolute LO/NLO values as well as ddx NLO/LO = alpha\_s/pi and the ttx published ratio.
  ...
  - Use exact as the physical acceptance default. Compare scalar\_only where its algebraic assumptions are justified; it must not become a workaround for a failure of exact.
+ Migrate the affected tests only after independent certification.
  ...
  - Update docs/architecture/architecture-current.typ and related convention/state documentation; update differential\_lu.md if event or cut-group behavior changes.
+ Complete validation and push the finished branch.
  ...
  - Run the full 166-case just test\_LU\_scalar\_xs matrix, including slow cases, under its existing 30 GB watchdog.
  ...
  - Commit with the complete user request in the description as required by CONTRIBUTING.typ, push codex/raised\_energy\_cff\_wip\_optimized\_phase\_fix to origin, and report the branch/commit and test evidence.
...
The independent second audit covers intrinsic complex CKM phases, the complex-mass scheme, cut-line numerator factors, and the complete marking/contour ledger. Its implementation contract and primary references are documented in #link("docs/architecture/phase-conventions.typ")[phase-conventions.typ];:
...
- A fresh SM UFO import exposed an infinite square-root normalization in `ufo_model_loader`: Symbolica canonicalizes the replacement to the same expression, so a repeat-until-no-match rewrite never terminates. The source-owner fixed-point repair passes its bounded regression and the full wrapped SM import. A separate compatibility run exposed the loader's obsolete `symbolica>=0.18.0` declaration: its existing replacement/evaluation API requires Symbolica 2. The final #link("https://github.com/alphal00p/ufo_model_loader/pull/1")[loader PR \#1];, commit `0bdd60af9927114a38df73d4823da07aafb1d158`, passes the bounded regression and full wrapped SM import on public Symbolica 2.0.0 and 2.2.0 (0.031 seconds per import). Both GammaLoop dependency declarations pin that revision and require Symbolica 2.0 or newer; `uv.lock` selects tested public 2.2.0. No installed-package or dependency-cache source patch is used.
...
- A fresh Python extension is now building in the isolated locked environment with the required compile-time NO\_SYMBOLICA\_OEM\_LICENSE=1 setting. It will be selected explicitly for the exact full suite; this switch is scoped to the extension build. No full-suite success is claimed yet.
