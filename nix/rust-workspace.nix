{
  ...
  nixCiConfiguration = let
    enable = (import (workspaceRoot + "/nix-ci.nix")).enable;
  in assert lib.assertMsg (builtins.isBool enable) "nix-ci.nix: top-level enable must be a boolean";
    pkgs.writeText "nix-ci.nix" (
      "# Generated by just ci-update from nix/ci.nix and nix/ci-workspace-graph.json.\n"
      + "# Edit only top-level enable manually; regeneration preserves it.\n"
      + lib.generators.toPretty {} (ci.configuration // {inherit enable;}) + "\n"
    );
