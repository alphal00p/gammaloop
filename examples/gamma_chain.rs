...
fn pslash(indices: (usize, usize), p: [Complex64; 4]) -> DenseTensor<Complex64> {
    ...
    let  pslash = gamma(minkindex, indices).contract_dT(&p).unwrap();
...
fn benchmark_chain(
    ...
    for m in minkindices {
        if *m > 0 {
            ...
            result = pslash.contract_dT(&result).unwrap();
        } else {
            result = gamma(
    ...
    result
        .contract_dT(&eucl_four_vector(contracting_index, u))
        .unwrap()
}
...
fn main() {
    ...
    //     .contract_dT(&identity((3, 4), Euclidean(4)))
    //     .unwrap();

    // // println!("{:?}", gammamu);

    // let gammunu = gamma(1, (4, 5))
    //     .contract_dT(&identity((5, 6), Euclidean(4)))
    //     .unwrap();

    // println!("{:?}", gammamu.contract_dT(&gammunu).unwrap());
    ...
    // println!("{:?}", a.contract_dT(&b));

    // let p11 = pslash((1, 2), [one, zero, zero, zero])
    //     .contract_dT(&pslash((2, 1), [one, zero, zero, zero]));
