...
def get_integrand_results(log_file_path):
    ...
    has_term_name = bool(
        span_dt is not None and span_dt.is_(pl.Struct) and "term.name" in span_dt.to_schema()  # dict of field -> dtype
    )

    spans_dt = schema.get("spans")
    has_term_name_from_spans = bool(
        spans_dt is not None and spans_dt.is_(pl.List) and spans_dt.inner.is_(pl.Struct) and "term.name" in spans_dt.inner.to_schema()
    )
    ...
    norm = df.select("span", "spans").match_to_schema(
        {
            "span": pl.Struct([Field("term.name", pl.Utf8), Field("name", pl.Utf8)]),
            "spans": pl.List(pl.Struct([Field("term.name", pl.Utf8), Field("name", pl.Utf8)])),
        },
    ...
    for g_name in graph_names:
        ...
        pred_spans = pl.col("spans").list.eval(pl.element().struct.field("term.name") == g_name).list.any().fill_null(False)
        ...
        if g_original_integrand.height > 1:
            raise ValueError(f"Multiple original integrand entries found for graph '{g_name}', expected only one")
        ...
        if g_ct_eval.height > 1:
            raise ValueError(f"Multiple evaluated threshold counterterm entries found for graph '{g_name}', expected only one")
    ...
    if total_df.height > 1:
        raise ValueError("Multiple entries found for total integrand, expected only one")
    ...
    if jac_df.height > 1:
        raise ValueError("Multiple entries found for Jacobian, expected only one")
...
def eval_integrand(k_input, debug=False, gammaloop_state="./GL_QQX_AAA_euclidean", gammaloop_executable="../../target/dev-optim/gammaloop"):
    ...
    cmd = (
        f"{gammaloop_executable} -s {gammaloop_state} -n -t inspect inspect "
        f"-p 0 -i default --discrete-dim 0 -m -x "
        f"{k_input[0]:.16f} {k_input[1]:.16f} {k_input[2]:.16f}"
    )
    if debug:
    ...
    r = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, shell=True, env=tmp_env)
    if debug:
        print(f"Output:\n{r.stdout}")
    all_res, total, jac = get_integrand_results(f"{gammaloop_state}/logs/gammalog-inspect.jsonl")

    reconstructed_total = sum(v["I"] + v["threshold_CT"] for v in all_res.values())
...
def run_limit_test(
    ...
    tests_to_run=None,
    gammaloop_state="./GL_QQX_AAA_euclidean",
    gammaloop_executable="../../target/dev-optim/gammaloop",
    debug=False,
    threshold_to_study="A",
):
    ...
    p5: list[float] = [p1[i] + p2[i] - p3[i] - p4[i] for i in range(4)]
    ...
    match threshold_to_study:
        case "A":
            ...
            on_threshold_point = [on_threshold_point[i] + p1[i + 1] for i in range(3)]
            # on_threshold_point = [on_threshold_point[i] * 2. for i in range(3)]
        case _:
            raise ValueError(f"Unknown threshold to study '{threshold_to_study}'")
    ...
    for test_name, k_base, scaling, pref_power in [
        ...
        if test_name == "IR":
            k = k_base
            scaled_k = [k[0] * scaling_factor, k[1] * scaling_factor, k[2]]
        elif test_name == "UV":
            k = k_base
            scaled_k = [k[0] * scaling_factor, k[1] * scaling_factor, k[2] * scaling_factor]
        elif test_name == "THRES":
            eps = 0.001
            shift = [eps, eps, eps]
            k = [k_base[0] + shift[0], k_base[1] + shift[1], k_base[2] + shift[2]]
            scaled_k = [k_base[0] + shift[0] * scaling_factor, k_base[1] + shift[1] * scaling_factor, k_base[2] + shift[2] * scaling_factor]
            # print(k)
            # print(scaled_k)
        else:
        ...
        scaled_all_res, scaled_jac, scaled_total, scaled_reconstructed_total = eval_integrand(scaled_k, debug, gammaloop_state, gammaloop_executable)
        for i_g, g_name in enumerate(graph_names):
            ...
            if "I" in elements_to_display:
                print(
                    f"{'Evaluation result for graph "' + g_name + '" [I]':60s}: {res:50.16e} | scaled: {
                        res_scaled:50.16e} | log10(abs(scaled / orig)) = {log10(abs(res_scaled / res)) if res != 0.0 else float('nan'):.3f} "
                )
            res = eval_res["threshold_CT"]
            res_scaled = eval_res_scaled["threshold_CT"]
            if "th.CT" in elements_to_display:
                print(
                    f"{'Evaluation result for graph "' + g_name + '" [th.CT]':60s}: {res:50.16e} | scaled: {
                        res_scaled:50.16e} | log10(abs(scaled / orig)) = {log10(abs(res_scaled / res)) if res != 0.0 else float('nan'):.3f} "
                )
            ...
            if "tot" in elements_to_display:
                print(
                    f"{'Evaluation result for graph "' + g_name + '" [tot]':60s}: {res:50.16e} | scaled: {
                        res_scaled:50.16e} | log10(abs(scaled / orig)) = {log10(abs(res_scaled / res)) if res != 0.0 else float('nan'):.3f} "
                )

        print("-" * 100)
        print(
            f"{'Total evaluation result from non-CT':60s}: {running_sum_non_ct:50.16e} | scaled: {
                running_sum_non_ct_scaled:50.16e}  | log10(abs(scaled / orig)) = {
                log10(abs(running_sum_non_ct_scaled / running_sum_non_ct)) if running_sum_non_ct != 0.0 else float('nan'):.3f} "
        )
        print(
            f"{'Total evaluation result from CT':60s}: {running_sum_ct:50.16e} | scaled: {
                running_sum_ct_scaled:50.16e}  | log10(abs(scaled / orig)) = {
                log10(abs(running_sum_ct_scaled / running_sum_ct)) if running_sum_ct != 0.0 else float('nan'):.3f} "
        )
        print(
            f"{'Total evaluation result':60s}: {running_sum_ct + running_sum_non_ct:50.16e} | scaled: {
                running_sum_ct_scaled + running_sum_non_ct_scaled:50.16e}  | log10(abs(scaled / orig)) = {
                log10(abs((running_sum_ct_scaled + running_sum_non_ct_scaled) / (running_sum_ct + running_sum_non_ct)))
                if running_sum_ct + running_sum_non_ct != 0.0
                else float('nan'):.3f} "
        )
...
parser.add_argument("--display_elements", "-d", nargs="*", choices=["I", "th.CT", "tot"], help="Which elements to display per graph", default=["tot"])
parser.add_argument("--tests", "-t", nargs="+", choices=["IR", "UV", "THRES"], help="Which limits to test", default=["IR", "UV", "THRES"])
parser.add_argument("--threshold_to_study", "-thres", type=str, choices=["A"], help="Which threshold to study", default="A")
parser.add_argument(
    "--gammaloop_state", "-s", type=str, help="Path to the GammaLoop state directory. Default=%(default)s", default="./GL_QQX_AAA_euclidean"
)
parser.add_argument("--graph_names", "-g", type=str, nargs="*", default=None, help="Graph names to consider. Default=all")

parser.add_argument(
    "--gammaloop_executable", "-e", type=str, help="Path to the GammaLoop executable. Default=%(default)s", default="../../target/dev-optim/gammaloop"
)
...
if __name__ == "__main__":
    ...
    run_limit_test(args.display_elements, args.tests, args.gammaloop_state, args.gammaloop_executable, args.debug, args.threshold_to_study)
