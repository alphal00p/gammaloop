...
- **Threshold structure edited:** no. The clean graph has no explicit `threshold_counterterms` document. A projected directive was not guessed because the decisive high-precision/threshold-off diagnosis could not be completed inside the single lease.
- **All limits OK:** **no (unresolved)**. The ordinary-precision complete sum formally fails the `e12/e13` hard-collinear path (`p=3.925..3.994` at rank 2). Two correlated `e12` families and generic `e13` soft pass; the other required paths are inconclusive under the strict fit rules.
...
| final run card | `cfbf6d6f5883979a18edced5b45ff55915ddee73e35b2242fa9281a0cdd0efac` |
...
Vectors are `(K0,K1,K2,K3)`, three components per loop momentum. Every accepted block uses 50 signed logarithmic points, `10^-6 <= |lambda| <= 10^-2`, a skipped midpoint, three cores, graph ID 0, and no orientation ID.
...
Threshold subtraction was active and nonzero on every recorded path. The clean graph was left unchanged because the evidence does not yet isolate a smallest common projected subspace. The exact 50-point all-orientation Arb replay of `soft_e12` was attempted through the watchdog, but did not finish in 238.974 s and was interrupted for the integration/documentation window; it produced no partial JSON. The intended exact Arb replay of the decisive collinear path is present in the card but was not started. The ordinary-precision threshold-off collinear diagnostic is also explicit in the card, but its watchdog expired after 90.033 s waiting for a shared heavy slot (`slot_deadline`); GammaLoop was never launched for that diagnostic.

This is an unresolved result, not evidence that removing threshold subtraction is an acceptable cure. No threshold-only path is required because no association was edited.

## Generation

- Full-UV internal generation time: **153.302972 s**.
- Evaluators: 28; compile time: 0.
- Internal peak RAM: 6,265,929,728 bytes.
- Spenso time: 11.766762 s; Symbolica time: 108.929550 s.
- Full local/integrated UV succeeded on the first attempt; no local-only result exists.
- Guard duration / peak recursive RSS: 154.833 s / 6,265,929,728 bytes; minimum free disk exceeded 154 GB.
...
GL286 remains unresolved at its one-lease boundary. The complete f64 sum formally fails one hard-collinear sector with active threshold subtraction, while the mandatory targeted Arb confirmation exceeded the available diagnostic window. No unsupported directive was added. `all limits OK = no` reflects the formal f64 fail; the result text retains the precision caveat and all inconclusive sectors.
