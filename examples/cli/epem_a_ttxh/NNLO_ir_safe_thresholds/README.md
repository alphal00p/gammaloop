...
Every multiplier E-surface call uses the mandatory form
`eta(effective, eset(...))` or `eta(star, eset(...))`.  The `eset` symbol is
registered as symmetric before expression parsing, so its edge arguments need
not be sorted; the former flat `eta(view, edge, ...)` form is intentionally
invalid.  The curing GL638 partition deliberately uses `effective`: for a
local component this is the local sample `r`, while for an integrated
component GammaLoop's normative context replaces it by `r_star`.  It therefore
implements the requested `f(r)`/`f(r_star)` distinction without spoiling the
integrated CT's PV-zero property.  An explicit `star` view remains available
when a function genuinely needs root kinematics even for a local component.
