...
independently named `[7]` and `[3,7]` variants and the same complementary
squared-E-surface ratios as the dressed graph.  It also forces `(8,12,14)`
under cut `(2,6,10)` into the shared one-loop subspace `[7]`; the legacy
maximal subspace there is `[3,7]`, unlike the `[7]` associations of the same
threshold under cuts `(2,6,12,13)` and `(2,4,12)`.  This common treatment and
the multiplier partition implement the cure demonstrated for the
selected-orientation correlated `q13` soft/threshold limit of cut `(2,4,12)`;
their sufficiency for every other cut has not yet been demonstrated.  Other
...
`eta(view, edge, ...)` form is intentionally invalid.  Pinning these E-surface
samples to the threshold-root view also preserves the integrated CT's PV-zero
property.  The generic ABI still exposes `effective`, but the tracked GL638
prescription and its physics regression deliberately exercise `star`.

The curated DOT files present the embedded TOML as literal multiline quoted
attributes.  GammaLoop's `save dot`, including
`--include-autogenerated-fields`, uses the same readable multiline form while
still escaping quotes, backslashes, tabs, and carriage returns.

For a clean checkout, use the dressed graphs and runnable `approach` cards
under `../NNLO/analyses/gl297_forced_subspace_cure`,
`../NNLO/analyses/gl638_multiplier_decomposition`, and
`../NNLO/analyses/gl638_correlated_soft_threshold_cure`.  Their local
`plot.py` wrappers render every page through the shared
`assets/plot_approach_result.py` script.  The absolute paths and local state
directories retained later in this document are historical audit provenance,
not prerequisites or current runnable instructions.
...
The cure audit additionally found that `(8,12,14)` resolves maximally in
`[3,7]` under cut 1 but in `[7]` under cuts 0 and 3.  The dressed graph makes
cut 1 explicit as `shared_1l` in `[7]`; this supplies the missing member of the
cross-cut LU cancellation.
...
the multiplier carrying the multichannel partition, and forces the
`(8,12,14)` association under cut `(2,6,10)` into the common `[7]` frame.  For
the selected `q13`/two-right-threshold trajectory, the two-sided fitted slopes
of the original, CT sum, and total are all `-1.000` at the four smallest
scales.  That is the demonstrated scope of this prescription; extending or
retuning it for every other GL638 cut remains follow-up research.
