...
fn ensure_workspace_cwd() {
    SET_WORKSPACE_CWD.call_once(|| {
        let workspace_root = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .expect("integration-tests must be in workspace root")
            .to_path_buf();
...
pub fn run_card(resource_path: impl AsRef<Path>) -> Result<RunHistory> {
    ...
    RunHistory::load(PathBuf::from("./integration-tests/resources/run_cards").join(resource_path))
}

pub const TESTS_WORKSPACE: &str = "./integration-tests/workspace";
