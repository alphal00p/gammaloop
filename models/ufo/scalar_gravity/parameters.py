...
# Specify here how many scalar particles coupled to graviton are needed to be considered
if os.environ.get('UFO_GRAVITY_MODEL_N_SCALARS') is not None:
    try:
        N_SCALARS = int(os.environ['UFO_GRAVITY_MODEL_N_SCALARS'])
    except ValueError:
        raise ValueError(
            "Environment variable UFO_GRAVITY_MODEL_N_SCALARS must be an integer")
    print("Loading UFO scalars model with %d scalars" % N_SCALARS)
else:
    N_SCALARS = 3

# Add all possible n-graviton interactions. User can filter undesired ones in vertices.py if needed.
if os.environ.get('UFO_GRAVITY_MODEL_N_POINT_INTERACTIONS') is not None:
    try:
        N_GRAVITON_INTERACTIONS = [
            int(x) for x in os.environ['UFO_GRAVITY_MODEL_N_POINT_INTERACTIONS'].split(',')]
    except ValueError:
        raise ValueError(
            "Environment variable UFO_GRAVITY_MODEL_N_POINT_INTERACTIONS must be a comma-separated list of integers")
    print("Loading UFO scalars model with n-point interactions, n=[%s]" %
          '|'.join(str(x) for x in N_POINT_INTERACTIONS))
else:
    N_GRAVITON_INTERACTIONS = [3, 4, 5, 6, 7, 8, 9, 10]
