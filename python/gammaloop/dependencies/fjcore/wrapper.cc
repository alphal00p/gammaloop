...
namespace fwrapper {
  ...
  void transfer_input_particles(struct FastjetWorkspace * w, const double * p, const int & npart) {
    w->input_particles.resize(0);
    w->input_particles.reserve(npart);
    for (int i=0; i<npart; i++) {
      ...
      w->input_particles.push_back(psjet);    
    }
  }

  /// helper routine to help transfer jets -> f77jets[4*ijet+0..3]
  void transfer_jets(struct FastjetWorkspace * w, double * f77jets, int & njets) {
    njets = w->jets.size();
    for (int i=0; i<njets; i++) {
      for (int j=0;j<=3; j++) {
      // *f77jets = jets[i][j];
      // RF-MZ: reorder the arguments because in madfks energy goes first; in fastjet last.
        *f77jets = w->jets[i][(j+3) % 4];
        f77jets++;
      } 
    }
  }
  ...
  void transfer_cluster_transfer(struct FastjetWorkspace * w, const double * p, const int & npart, 
    ...
    for (unsigned int kk=0; kk<njets; ++kk) {   
      vector<PseudoJet> constit = w->cs->constituents(w->jets[kk]);
...
struct FastjetWorkspace* fastjet_workspace() {
  return new FastjetWorkspace;
}

void fastjet_free(struct FastjetWorkspace * w) {
  delete w;
}
...
// (particle mass).
//
void fastjetppgenkt_etamax_(struct FastjetWorkspace * w,
                     const double * p, const int & npart,                   
    ...
    // prepare jet def
    if (palg == 1.0) {
      w->jet_def = JetDefinition(kt_algorithm, R);
    }  else if (palg == 0.0) {
      w->jet_def = JetDefinition(cambridge_algorithm, R);
    }  else if (palg == -1.0) {
      w->jet_def = JetDefinition(antikt_algorithm, R);
    } else {
      w->jet_def = JetDefinition(genkt_algorithm, R, palg);
    }
    // do everything
    transfer_cluster_transfer(w, p,npart, w->jet_def,ptjetmin,etamax,f77jets,njets,whichjet);
}


void fastjetppgenkt_(struct FastjetWorkspace * w,
                     const double * p, const int & npart,                   
                     const double & R, const double & ptjetmin,
                     const double & palg,
                     double * f77jets, int & njets, int * whichjet) {
    ...
    double etamax = -1.;
    fastjetppgenkt_etamax_(w, p,npart,R,ptjetmin,etamax,palg,
...
//
void fastjetconstituents_(struct FastjetWorkspace * w, const int & ijet, 
   	                  int * constituent_indices, int & nconstituents) {
  assert(w->cs.get() != 0);
  assert(ijet > 0 && ijet <= w->jets.size());

  vector<PseudoJet> constituents = w->cs->constituents(w->jets[ijet-1]);
...
//   INTEGER N
//   
double fastjetdmerge_(struct FastjetWorkspace * w, const int & n) {
  assert(w->cs.get() != 0);
  return w->cs->exclusive_dmerge(n);
}
...
//   INTEGER N
//   
double fastjetdmergemax_(struct FastjetWorkspace * w, const int & n) {
  assert(w->cs.get() != 0);
  return w->cs->exclusive_dmerge_max(n);
}
